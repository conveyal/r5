...
// Basic test of one-to-many profile routing served up at http://localhost:8075/profile_analyst_tester.html
...
req.open('GET', 'http://localhost:8075/r5', false); // async=false, define center before setting up map
...
function profileRouteThreeIsochrones() {
    ...
    // Fetch http://localhost:8075/otp/routers/default/index/routes
    req.open('GET', 'http://localhost:8075/r5/profile' + qstring, true); // async=true, background call
    req.setRequestHeader("Accept", "application/json");
    req.onload = function() {
        var resp = JSON.parse(req.responseText);
        console.log(resp);
...
function profileRouteIsochrones(banAgency) {
    ...
    var url = 'http://localhost:8075/r5/profile';
    var fromPos = fromMarker.getLatLng();
    var qstring = '?from=' + fromPos.lat + ',' + fromPos.lng;
    qstring += '&analyst=true';
    qstring += '&modes=WALK,BICYCLE,TRANSIT'; // note that TRANSIT or specific transit modes must be included
    // Fetch bannable routes from http://localhost:8080/r5fault/index/routes
...
function profileRouteTiles(bannedAgency) {
    ...
    qstring += '&analyst=true';
    qstring += '&modes=WALK,BICYCLE,TRANSIT'; // note that TRANSIT or specific transit modes must be included
    // Fetch bannable routes from http://localhost:8075/r5/index/routes
