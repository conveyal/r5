...
public class BackendConfig
        implements TaskScheduler.Config,
                AnalysisDB.Config,
                Broker.Config,
    ...
    // This is used for testing that automated builds and JAR packaging are producing a usable
    // artifact.
    public final boolean immediateShutdown;

    // For use in testing - setting this field will activate alternate code paths that cause
    ...
    /**
     * Load configuration from the given properties file, overriding from environment variables and
     * system properties. In the latter two sources, the keys may be in upper or lower case and use
     * dashes, underscores, or dots as separators. The usual config file keys must be prefixed with
     * "conveyal", e.g. CONVEYAL_HEAVY_THREADS=5 or conveyal.heavy.threads=5. Precedence of
     * configuration sources is: system properties > environment variables > config file.
     */
    public BackendConfig(String filename) {
        ...
        // This could also be done with the Properties constructor that specifies defaults, but by
        // manually
        // overwriting items we are able to log these potentially confusing changes to
        // configuration.
        setPropertiesFromMap(System.getenv(), "environment variable");
        setPropertiesFromMap(System.getProperties(), "system properties");

        // We intentionally don't supply any defaults here - any 'defaults' should be shipped in an
        // example config file.
        ...
        if (!missingKeys.isEmpty()) {
            LOG.error(
                    "You must provide these configuration properties: {}",
    ...
    /**
     * This needs to work with both properties and environment variable conventions, so case and
     * separators are normalized. Properties are Object-Object Maps so key and value are cast to
     * String.
     */
    private void setPropertiesFromMap(Map<?, ?> map, String sourceDescription) {
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            if (key.startsWith(CONVEYAL_PROPERTY_PREFIX)) {
                ...
                if (existingKey != null) {
                    LOG.info(
                            "Overwriting existing config key {} to '{}' from {}.",
                            key,
                            value,
                            sourceDescription);
                } else {
                    LOG.info(
                            "Setting configuration key {} to '{}' from {}.",
                            key,
                            value,
                            sourceDescription);
                }
    ...
    @Override
    public String resultsBucket() {
        return resultsBucket;
    }

    @Override
    public boolean testTaskRedelivery() {
        return testTaskRedelivery;
    }

    @Override
    public String gridBucket() {
        return gridBucket;
    }

    @Override
    public String seamlessCensusRegion() {
        return seamlessCensusRegion;
    }

    @Override
    public String seamlessCensusBucket() {
        return seamlessCensusBucket;
    }
