package com.conveyal.analysis.broker;
...
import com.conveyal.eventbus.RegionalAnalysisEvent;
import com.conveyal.eventbus.WorkerEvent;
...
import static com.conveyal.eventbus.RegionalAnalysisEvent.State.CANCELED;
import static com.conveyal.eventbus.RegionalAnalysisEvent.State.COMPLETED;
import static com.conveyal.eventbus.RegionalAnalysisEvent.State.STARTED;
import static com.conveyal.eventbus.WorkerEvent.Action.REQUESTED;
import static com.conveyal.eventbus.WorkerEvent.Role.REGIONAL;
import static com.conveyal.eventbus.WorkerEvent.Role.SINGLE_POINT;
...
public class Broker implements Component {
    ...
    public synchronized void enqueueTasksForRegionalJob (RegionalAnalysis regionalAnalysis) {
        ...
        WorkerTags workerTags = new WorkerTags(
    ...
    private RegionalTask templateTaskFromRegionalAnalysis (RegionalAnalysis regionalAnalysis) {
        ...
        try {
            ...
            JsonUtils.objectMapper.writeValue(localScenario, scenario);
    ...
    public void createWorkersInCategory (WorkerCategory category, WorkerTags workerTags, int nOnDemand, int nSpot) {
        ...
        if (workerCatalog.totalWorkerCount() + nOnDemand + nSpot >= config.maxWorkers()) {
            ...
            throw HttpServerRuntimeException.forbidden(message);
        }
