...
public class LocalComponents extends Components {
    ...
    public LocalComponents() {
        ...
        // Instantiate the HttpControllers last, when all the components except the HttpApi are
        // already created.
        httpApi =
                new HttpApi(
                        fileStorage,
                        ...
                        eventBus,
                        config,
                        standardHttpControllers(this));
        // compute = new LocalCompute();
        // persistence = persistence(local_Mongo)
    }

    /**
     * Create the standard list of HttpControllers used in local operation. The Components parameter
     * should already be initialized with all components except the HttpApi. We pass these
     * controllers into the HttpApi (rather than constructing them in the HttpApi constructor) to
     * allow injecting custom controllers in other deployment environments. This also avoids
     * bulk-passing the entire set of components into the HttpApi constructor, ensuring clear
     * declaration of each component's dependencies. Such bulk-passing of components should only
     * occur in this wiring-up code, not in component code.
     */
    public static List<HttpController> standardHttpControllers(Components components) {
        final List<HttpController> httpControllers =
                Arrays.asList(
                        ...
                        new OpportunityDatasetController(
                                components.fileStorage,
                                components.taskScheduler,
                                components.config),
                        new RegionalAnalysisController(
                                components.broker, components.fileStorage, components.config),
                        ...
                        // This broker controller registers at least one handler at URL paths
                        // beginning with /internal, which
                        // is exempted from authentication and authorization, but should be hidden
                        // from the world
                        // outside the cluster by the reverse proxy. Perhaps we should serve
                        // /internal on a separate
                        // port so they can't be accidentally exposed by the reverse proxy. It could
                        // even be a separate
                        // InternalHttpApi component with its own spark service, renaming this
                        // ExternalHttpApi.
