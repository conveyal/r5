...
/**
 * A catalog of all the workers this broker has been contacted by recently. Ideally this would also
 * manage target quantities of workers by category and migrate workers from one category to another.
 * But for now we just leave workers on a single graph / r5 commit and don't migrate them.
 */
public class WorkerCatalog {
    ...
    /**
     * The information supplied by workers the last time they polled for more tasks. We store these
     * observations keyed on the worker ID so new observations replace old ones for the same worker.
     */
    private Map<String, WorkerObservation> observationsByWorkerId = new HashMap<>();

    /**
     * Keeps the workers sorted into categories depending on which network and R5 commit they are
     * running. The values are worker IDs instead of observations because the observation objects
     * are being constantly updated, so are not practical for equality tests and hashing.
     */
    private Multimap<WorkerCategory, String> workerIdsByCategory = HashMultimap.create();

    /**
     * Tracks which worker within each category we are directing single point requests to. This is a
     * trade-off: single point work is not balanced between workers, but once a worker applies
     * scenarios and links pointsets that work will not need to be redone for subsequent single
     * point requests.
     */
    ...
    public synchronized void catalog(WorkerStatus workerStatus) {
        ...
        // Associate the worker with its currently reported category. According to Guava docs,
        // Multimap does not store
        // duplicate key-value pairs. Adding a new key-value pair equal to an existing key-value
        // pair has no effect.
        workerIdsByCategory.put(observation.category, workerId);
        // If this worker's current category has no assigned single point worker, assign this
        // worker.
    ...
    /**
     * Before fetching worker information, call this method to remove any workers that we haven't
     * heard from for a while. Perhaps we should be calling this method on a timer instead of every
     * time read-oriented methods are called.
     */
    ...
    /**
     * If a single-point worker has shut down, single-point work for a given workerCategory should
     * be re-assigned. If no other workers of the requested workerCategory exist, the caller in the
     * Broker should start one.
     *
     * @param workerCategory of worker that could not be reached for a single-point request (and has
    ...
    /**
     * Return the address of a worker machine on a given category that can be used to handle a
     * single point request. Attempt to repeatedly return the same active worker for a given
     * category. This helps direct single point requests to the same worker and avoid relinking. The
     * HashMultimap does not return values in a predictable order, so we maintain a separate map for
     * these workers.
     */
    public synchronized String getSinglePointWorkerAddressForCategory(
    ...
    /**
     * Count active workers for all jobs and return a protective copy of the list for further
     * processing in the caller, in order to minimize the time spent in this synchronized block.
     *
     * <p>A worker is considered active on a job if the worker reported a task complete for that job
     * over the minute before it last polled the backend.
     *
    ...
    public synchronized boolean noWorkersAvailable(
            WorkerCategory category, boolean ignoreWorkerVersion) {
        purgeDeadWorkers();
        if (ignoreWorkerVersion) {
            // Look for workers on the right network ID, independent of their worker software
            // version.
            return observationsByWorkerId.values().stream()
                    .noneMatch(obs -> obs.category.graphId.equals(category.graphId));
        }
