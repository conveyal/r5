...
/**
 * Updated each time the worker contacts the broker. This class is used by the broker to keep track
 * of the pool of workers contacting it, but also used to serialize worker information in the HTTP
 * API so the web client can visualize the worker pool.
 */
public class WorkerObservation {

    public final String workerId;

    @JsonIgnore public final WorkerCategory category;

    /** The last time this worker contacted the broker, in milliseconds since the epoch. */
    @JsonIgnore public final long lastSeen;

    @JsonUnwrapped // The workerStatus fields will be serialized as if they were part of this root
                   // object
