...
/**
 * Stores vector aggregationAreas (used to define the region of a weighted average accessibility
 * metric).
 */
public class AggregationAreaController implements HttpController {
    ...
    /**
     * Arbitrary limit to prevent UI clutter from many aggregation areas (e.g. if someone uploads
     * thousands of blocks). Someone might reasonably request an aggregation area for each of
     * Chicago's 50 wards, so that's a good approximate limit for now.
     */
    ...
    public interface Config {
        // TODO this could be eliminated by hard-wiring file types to bucket subdirectories in the
        // FileStorage.
        String gridBucket();
    }
    ...
    /**
     * Create binary .grid files for aggregation (aka mask) areas, save them to S3, and persist
     * their details.
     *
     * @param req Must include a shapefile on which the aggregation area(s) will be based. If HTTP
     *     query parameter union is "true", features will be merged to a single aggregation area,
     *     named using the value of the "name" query parameter. If union is false or if the
     *     parameter is missing, each feature will be a separate aggregation area, named using the
     *     value for the shapefile property specified by the HTTP query parameter "nameAttribute."
     */
    private List<AggregationArea> createAggregationAreas(Request req, Response res)
            throws Exception {
        ...
        // ======================================================
        Map<String, FileItem> filesByName =
                query.get("files").stream().collect(Collectors.toMap(FileItem::getName, f -> f));

        String fileName =
                filesByName.keySet().stream()
                        .filter(f -> f.endsWith(".shp"))
                        .findAny()
                        .orElse(null);
        if (fileName == null) {
            throw AnalysisServerException.fileUpload(
                    "Shapefile upload must contain .shp, .prj, and .dbf");
        }
        ...
        if (!unionRequested && features.size() > MAX_FEATURES) {
            throw AnalysisServerException.fileUpload(
                    MessageFormat.format(
                            "The uploaded shapefile has {0} features, "
                                    + "which exceeds the limit of {1}",
                            features.size(), MAX_FEATURES));
        }
        ...
        if (unionRequested) {
            ...
            // Don't union. Name each area by looking up its value for the name property in the
            // request.
            String nameProperty = query.get("nameProperty").get(0).getString("UTF-8");
            features.forEach(
                    f ->
                            areas.put(
                                    readProperty(f, nameProperty),
        ...
        areas.forEach(
                (String name, Geometry geometry) -> {
                    ...
                    weights.forEach(
                            pixel -> maskGrid.grid[pixel.x][pixel.y] = pixel.weight * 100_000);
                    ...
                    try {
                        ...
                        // Create the aggregation area before generating the S3 key so that the
                        // `_id` is generated
                        ...
                        throw new AnalysisServerException(
                                "Error processing/uploading aggregation area");
                    }
    ...
    private String readProperty(SimpleFeature feature, String propertyName) {
        try {
            ...
            String message =
                    String.format(
                            "The specified property '%s' was not present on the uploaded features. "
                                    + "Please verify that '%s' corresponds to a shapefile column.",
    ...
    private Object getAggregationArea(Request req, Response res) {
        ...
        AggregationArea aggregationArea =
                Persistence.aggregationAreas.findByIdIfPermitted(maskId, accessGroup);
    ...
    public void registerEndpoints(spark.Service sparkService) {
        sparkService.path(
                ...
                () -> {
                    sparkService.get(
                            "/:regionId/aggregationArea", this::getAggregationAreas, toJson);
                    sparkService.get(
                            "/:regionId/aggregationArea/:maskId", this::getAggregationArea, toJson);
                    sparkService.post(
                            "/:regionId/aggregationArea", this::createAggregationAreas, toJson);
                });
    }
}
