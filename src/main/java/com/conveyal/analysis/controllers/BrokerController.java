 ...
 * requesting work. It also handles connections from the front end for single-point requests. This
 * API replaces what used to be a separate broker process running outside the Analysis backend. The
 * core task queueing and distribution logic is supplied by an instance of the Broker object. The
 * present class should just wrap that functionality with an HTTP API.
 *
 * <p>Workers used to long-poll and hold connections open, allowing them to receive tasks instantly,
 * as soon as the tasks are enqueued. However, this adds a lot of complexity since we need to
 * suspend the open connections and clear them out when the connections are closed. The workers now
 * short-poll, which allows a simple, standard HTTP API. We don't care if it takes a few seconds for
 * workers to start consuming regional tasks.
 *
 * <p>Single point (high priority) tasks are instead handled with a proxy-like push approach. TODO
 * refactor into TaskBroker and WebAPIServer components
 */
public class BrokerController implements HttpController {
    ...
    /**
     * Spark handler functions return Objects. Spark's DefaultSerializer calls toString on those
     * objects and converts the resulting string to UTF-8 bytes. The class
     * spark.webserver.serialization.Serializer only has a few really basic subclasses. In the past
     * we've supplied a ResponseTransformer to these mapping functions to serialize our results. It
     * seems like we should define a Serializer, but Spark provides no setters for
     * MatcherFilter.serializerChain. The other solution (which I've adopted here) is to explicitly
     * serialize our results to JSON and just return a JSON string from our HTTP handler methods.
     */
    @Override
    public void registerEndpoints(spark.Service sparkService) {
        ...
        sparkService.post(
                "/api/analysis",
                this::singlePoint); // TODO rename HTTP path to "single" or something
    }

    /**
     * Handler for single-origin requests. This endpoint is contacted by the frontend rather than
     * the worker, and causes a single-point task to be pushed to an appropriate worker for
     * immediate processing. These requests typically come from an interactive session where the
     * user is moving the origin point around in the web UI. Unlike regional jobs where workers pull
     * tasks from a queue, single point tasks work like a proxy or load balancer. Note that we are
     * using an Apache HTTPComponents client to contact the worker, within a Spark (Jetty) handler.
     * We could use the Jetty HTTP client (which would facilitate exactly replicating request and
     * response headers when we forward the request to a worker), but since Spark wraps the internal
     * Jetty request/response objects, we don't gain much. We should probably switch to the Jetty
     * HTTP client some day when we get rid of Spark. There is also a Jetty proxy module that may be
     * too simple for what we're doing here.
     *
    ...
    private Object singlePoint(Request request, Response response) {
        ...
        // Perhaps we should allow both travel time surface and accessibility calculation tasks to
        // be done as single points.
        // AnalysisRequest (backend) vs. AnalysisTask (R5)
        // The accessgroup stuff is copypasta from the old single point controller.
        // We already know the user is authenticated, and we need not check if they have access to
        // the graphs etc,
        // as they're all coded with UUIDs which contain significantly more entropy than any human's
        // account password.
        ...
        // Transform the analysis UI/backend task format into a slightly different type for R5
        // workers.
        TravelTimeSurfaceTask task =
                (TravelTimeSurfaceTask)
                        analysisRequest.populateTask(new TravelTimeSurfaceTask(), project);
        ...
        if (notNullOrEmpty(analysisRequest.destinationPointSetIds)) {
            // Look up all destination opportunity data sets from the database and derive their
            // storage keys.
            // This is mostly copypasted from the code to create a regional analysis.
            // Ideally we'd reuse the same code in both places.
            // We should refactor the populateTask method (and move it off the request) to take care
            // of all this.
            ...
            task.destinationPointSetKeys =
                    opportunityDatasets.stream()
            ...
            // regional tasks.
            // Don't validate cutoffs because those are implied to be [0...120) and generated by the
            // worker itself.
        ...
        if (address == null) {
            ...
            WorkerTags workerTags =
                    new WorkerTags(accessGroup, userEmail, project._id, project.regionId);
            ...
            return jsonResponse(
                    ...
                    "Starting routing server. Expect status updates within a few minutes.");
        } else {
        ...
        try {
            // Serialize and send the R5-specific task (not the original one the broker received
            // from the UI)
            ...
            // Mimic headers sent by the worker. We're mostly interested in Content-Type, maybe
            // Content-Encoding.
            ...
            LOG.info(
                    "Returning worker response to UI with status code {} and content type {}",
                    workerResponse.getStatusLine(),
                    contentTypeHeader.getValue());
            // This header will cause the Spark Framework to gzip the data automatically if
            // requested by the client.
            response.header("Content-Encoding", "gzip");
            entity = workerResponse.getEntity();
            // Only record activity on successful requests, to avoid polling noise.
            // All other eventbus usage is in Broker, a sign that most of this method should be
            // factored out of the controller.
            if (response.status() == 200) {
                int durationMsec = (int) (System.currentTimeMillis() - startTimeMsec);
                eventBus.send(
                        new SinglePointEvent(
                                        task.scenarioId,
            ...
            // If you return a stream to the Spark Framework, its SerializerChain will copy that
            // stream out to the
            // client, but does not then close the stream. HttpClient waits for the stream to be
            // closed to return the
            // connection to the pool. In order to be able to close the stream in code we control,
            // we buffer the
            // response in a byte buffer before resending it. NOTE: The fact that we're buffering
            // before re-sending
            ...
            // Aborting the request might help release resources - we had problems with exhausting
            // connection pools here.
            httpPost.abort();
            return jsonResponse(
                    response,
                    HttpStatus.BAD_REQUEST_400,
                    "Routing server timed out. For the complexity of this scenario, your request"
                        + " may have too many simulated schedules. If you are using Routing Engine"
                        + " version < 4.5.1, your scenario may still be in preparation and you"
                        + " should try again in a few minutes.");
        } catch (NoRouteToHostException nrthe) {
            LOG.info(
                    "Worker in category {} was previously cataloged but is not reachable now. This"
                        + " is expected if a user made a single-point request within"
                        + " WORKER_RECORD_DURATION_MSEC after shutdown.",
                    workerCategory);
            httpPost.abort();
            ...
            // If the HTTP response entity is non-null close the associated input stream, which
            // causes the HttpClient
            // to release the TCP connection back to its pool. This is critical to avoid exhausting
            // the pool.
    ...
    /**
     * TODO respond to HEAD requests. For some reason we needed to implement HEAD, for proxies or
     * caches?
     */
    ...
    /**
     * This method factors out a few lines of common code that appear in all handlers that produce
     * JSON responses. Set the response status code and media type, then serialize an object to JSON
     * to serve as a response.
     *
     ...
     * @return the serialized object as a String, which can then be returned from the Spark handler
     *     function
     */
    ...
    private String getAllWorkers(Request request, Response response) {
        ...
        for (WorkerObservation observation : observations) {
            ...
            for (String networkId : observation.status.networks) {
                ...
                if (bundle == null) {
                    DBCursor<Bundle> cursor =
                            Persistence.bundles.find(QueryBuilder.start("_id").is(networkId).get());
    ...
    /**
     * Workers use this endpoint to fetch tasks from job queues. At the same time, they also report
     * their version information, unique ID, loaded networks, etc. as JSON in the request body. They
     * also supply the results of any completed work via this same object. The broker should
     * preferentially send them work they can do efficiently using already loaded networks and
     * scenarios. The method is POST because unlike GETs (which fetch status) it modifies the
     * contents of the task queue.
     */
    private Object workerPoll(Request request, Response response) {
        ...
        // Record any regional analysis results that were supplied by the worker and mark them
        // completed.
