...
public class BrokerController implements HttpController {
    ...
    private Object singlePoint(Request request, Response response) {
        ...
        // If the client used HTTP headers to request a Geotiff (for exporting results to GIS) or a PNG (for
        // colorization in GL rendering), signal this to the worker using a field on the request object.
        if (request.headers("Accept").equals("image/tiff")) {
            task.setFormat(TravelTimeSurfaceTask.Format.GEOTIFF);
        } else if (request.headers("Accept").equals("image/png")) {
            task.setFormat(TravelTimeSurfaceTask.Format.PNG);
        } else {
        ...
        try {
            ...
            for (String headerName : new String[] {"Content-Type", "X-Conveyal-Bounding-Box"}) {
                Header header = workerResponse.getFirstHeader(headerName);
                if (header != null) {
                    response.header(header.getName(), header.getValue());
                }
            }
            LOG.debug("Returning worker response to UI with status code {}");
