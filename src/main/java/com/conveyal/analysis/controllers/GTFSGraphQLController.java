...
/**
 * GraphQL interface for fetching GTFS feed contents (generally used for scenario editing). For now
 * it just wraps the GTFS API graphql response with a bundle object.
 */
public class GTFSGraphQLController implements HttpController {
    ...
    private Object handleQuery(Request req, Response res) throws IOException {
        ...
        Map<String, Object> variables =
                JsonUtil.objectMapper.readValue(
                        req.queryParams("variables"), new TypeReference<Map<String, Object>>() {});
    ...
    public GraphQLObjectType feedType =
            newObject()
                    .name("feed")
                    ...
                    // We have a custom wrapped GTFS Entity type for FeedInfo that includes feed
                    // checksum
                    .field(
                            newFieldDefinition()
                                    .name("checksum")
                                    .type(GraphQLLong)
                                    .dataFetcher(
                                            env -> ((WrappedFeedInfo) env.getSource()).checksum)
                                    .build())
                    .field(
                            newFieldDefinition()
                                    .name("routes")
    ...
    private GraphQLEnumType bundleStatus =
            newEnum()
                    .name("status")
    ...
    private GraphQLObjectType bundleType =
            newObject()
                    .name("bundle")
                    .field(string("_id"))
                    .field(string("name"))
    ...
    private GraphQLObjectType bundleQuery =
            newObject()
                    .name("bundleQuery")
                    .field(
                            newFieldDefinition()
    ...
    /**
     * Returns a list of wrapped FeedInfo objects. These objects have all fields null (default),
     * except feed_id, which is set from the cached copy of each requested feed. This method
     * previously returned the fully populated FeedInfo objects, but incremental changes led to
     * incompatibilities with Analysis (see analysis-internal #102). The current implementation is a
     * stopgap for backward compatibility.
     */
    private List<WrappedGTFSEntity<FeedInfo>> fetchFeeds(DataFetchingEnvironment environment) {
        ...
        // Old bundles were created without computing the service start and end dates. Will only
        // compute if needed.
        try {
        ...
        return bundle.feeds.stream()
                .map(
                        summary -> {
                            String bundleScopedFeedId =
                                    Bundle.bundleScopeFeedId(summary.feedId, bundle.feedGroupId);
                            try {
                                GTFSFeed feed = gtfsCache.get(bundleScopedFeedId);
                                FeedInfo ret = new FeedInfo();
                                ret.feed_id = feed.feedId;
                                return new WrappedFeedInfo(
                                        summary.bundleScopedFeedId, ret, summary.checksum);
                            } catch (UncheckedExecutionException nsee) {
                                Exception e =
                                        new Exception(
                                                String.format(
                                                        "Feed %s does not exist in the cache.",
                                                        summary.name),
                                                nsee);
                                context.addError(new ExceptionWhileDataFetching(e));
                                return null;
