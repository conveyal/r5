...
public class ProjectController implements HttpController {
    ...
    private Collection<Modification> importModifications(Request req, Response res) {
        ...
        final Project importProject =
                Persistence.projects.findByIdIfPermitted(importId, accessGroup);
        ...
        final Collection<Modification> modifications =
                Persistence.modifications.findPermitted(query.get(), accessGroup);

        // This would be a lot easier if we just used the actual `_id`s and dealt with it elsewhere
        // when searching. They
        ...
        return modifications.stream()
                .map(
                        modification -> {
                            ...
                            // Set `updatedBy` by manually, `createdBy` stays with the original
                            // author
                ...
                .map(
                        modification -> {
                            ...
                            if (modification.getType().equals(AddTripPattern.type)) {
                                ((AddTripPattern) modification)
                                        .timetables.forEach(
                                                tt -> {
                                                    String pft = tt.phaseFromTimetable;
                                                    if (pft != null && pft.length() > 0) {
                                                        String[] pfts = pft.split(":");
                                                        tt.phaseFromTimetable =
                                                                modificationIdPairs.get(pfts[0])
                                                                        + ":"
                                                                        + timetableIdPairs.get(
                                                                                pfts[1]);
                                                    }
                                                });
                            } else if (modification.getType().equals(ConvertToFrequency.type)) {
                                ((ConvertToFrequency) modification)
                                        .entries.forEach(
                                                tt -> {
                                                    String pft = tt.phaseFromTimetable;
                                                    if (pft != null && pft.length() > 0) {
    ...
    private Project deleteProject(Request req, Response res) {
        return Persistence.projects.removeIfPermitted(
    ...
    public void registerEndpoints(spark.Service sparkService) {
        ...
        sparkService.get(
                "/api/region/:region/projects", this::getAllProjects); // TODO response transformer?
    }
}
