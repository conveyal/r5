 ...
 *
 * <p>Previously the OSM was specified at the Region level, so every Bundle in the same Region used
 * the same OSM data.
 *
 * <p>All of the data is stored in S3, however some information is duplicated here for convenience.
 */
public class Bundle extends Model implements Cloneable {
    ...
    /**
     * Model for storing summary info in Mongo. Bundle contains one instance of FeedSummary per feed
     * in the Bundle.
     */
    public static class FeedSummary implements Cloneable {
        ...
        /**
         * The feed ID scoped with the bundle ID, for use as a unique identifier on S3 and in the
         * GTFS API
         */
         ...
         *
         * <p>If a FeedInfo file is present in the feed, the feed_id, feed_start_date, and
         * feed_end_date are used for the name. If not, dates from calendar/calendar_dates files and
         * agency_name values (up to a limit) are used.
         *
         * <p>This method should be called after setServiceDates().
         */
        private void createFeedName(GTFSFeed feed) {
            ...
            this.name =
                    name
                            ...
                            + " to "
                            + endingDate.toString(); // ISO-8601
        }

        /**
         * Set service start and end from the dates of service values returned from GTFSFeed. This
         * is calculated from the trip data in the feed. `feed_info.txt` is optional and many GTFS
         * feeds do not include the fields feed_start_date or feed_end_date. Therefore we instead
         * derive the start and end dates from the stop_times and trips. Also, at this time the only
         * usage of these fields is to explicitly show in the user interface that a date does or
        ...
        /**
         * This summarization could be done on the fly during loading. However some users will want
         * the whole pile of errors.
         */
