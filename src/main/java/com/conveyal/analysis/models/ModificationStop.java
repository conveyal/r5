...
class ModificationStop {
    ...
    /**
     * Convert a MongoDB GeoJSON Position to a JTS Geometry Coordinate.
     */
    static Coordinate fromPosition(Position p) {
        return new Coordinate(p.getValues().get(0), p.getValues().get(1));
    }
    ...
    static List<ModificationStop> getStopsFromSegments(List<Segment> segments, List<Integer> dwellTimes, int defaultDwellTime, List<Integer> segmentSpeedsKph) {
        ...
        if (firstSegment.stopAtStart) {
            Coordinate c = fromPosition(firstSegment.geometry.getCoordinates().get(0));
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            Coordinate[] coords = segment.geometry.getCoordinates().stream().map(ModificationStop::fromPosition).collect(Collectors.toList()).toArray(new Coordinate[]{});
