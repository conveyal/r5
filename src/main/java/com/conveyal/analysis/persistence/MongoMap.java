...
/**
 * An attempt at simulating a MapDB-style interface, for storing Java objects in MongoDB. Note this
 * used to implement Map, but the Map interface predates generics in Java, so it is more typesafe
 * not to implement it. TODO this is using org.mongojack.JacksonDBCollection. I believe Mongo Java
 * client library now provides POJO storage.
 */
public class MongoMap<V extends Model> {
    ...
    public V findByIdIfPermitted(String id, String accessGroup) {
        ...
        if (result == null) {
            throw AnalysisServerException.notFound(
                    String.format(
                            "The resource you requested (_id %s) could not be found. Has it been"
                                + " deleted?",
                            id));
        } else if (!accessGroup.equals(result.accessGroup)) {
            throw AnalysisServerException.forbidden(
    ...
    /**
     * Helper function that adds the `accessGroup` to the query if the user is not an admin. If you
     * want to query using the `accessGroup` as an admin it must be added to the query.
     */
    public Collection<V> findPermitted(DBObject query, String accessGroup) {
        DBCursor<V> cursor =
                find(
                        QueryBuilder.start()
                                .and(query, QueryBuilder.start("accessGroup").is(accessGroup).get())
                                .get());
    ...
    /**
     * Helper function that creates a query from the request query parameters. It will not filter by
     * `accessGroup` as an admin unless specifically added in the query.
     */
    ...
    /**
     * Note that this has side effects on the object passed in! It assigns it an ID and
    ...
    public V put(V value, DBObject optionalQuery) {
        ...
        BasicDBObject dbObject =
                JsonUtilities.objectMapper.convertValue(value, BasicDBObject.class);

        // Update
        V result =
                wrappedCollection.findAndModify(
                        query.get(), null, null, false, dbObject, true, false);
        ...
        if (result == null) {
            ...
            if (result == null) {
                throw AnalysisServerException.notFound(
                        "The data you attempted to update could not be found. ");
            } else if (!currentNonce.equals(result.nonce)) {
                throw AnalysisServerException.nonce();
            } else {
                throw AnalysisServerException.forbidden(
                        "The data you attempted to update is not in your access group.");
            }
        }
    ...
    public V removeIfPermitted(String key, String accessGroup) {
        ...
        if (result == null) {
            throw AnalysisServerException.notFound(
                    "The data you attempted to remove could not be found.");
        }
    ...
    public V remove(String key) {
        ...
        if (result.getN() == 0) {
            throw AnalysisServerException.notFound(
                    String.format("The data for _id %s does not exist", key));
        }
