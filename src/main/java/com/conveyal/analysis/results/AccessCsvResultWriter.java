...
public class AccessCsvResultWriter extends CsvResultWriter {
    ...
    public AccessCsvResultWriter(RegionalTask task, String outputBucket, FileStorage fileStorage)
            throws IOException {
    ...
    public String[] columnHeaders() {
        // We could potentially make the percentile and cutoff columns optional, but it's
        // unnecessary complexity.
        return new String[] {"origin", "destinations", "percentile", "cutoff", "access"};
    }

    /**
     * Check that each dimension of the 3D accessibility results array matches the expected size for
    ...
    protected void checkDimension(RegionalWorkResult workResult) {
        checkDimension(
                workResult,
                "destination pointsets",
                workResult.accessibilityValues.length,
                task.destinationPointSetKeys.length);
        for (int[][] percentilesForGrid : workResult.accessibilityValues) {
            checkDimension(
                    workResult, "percentiles", percentilesForGrid.length, task.percentiles.length);
            for (int[] cutoffsForPercentile : percentilesForGrid) {
                checkDimension(
                        workResult,
                        "cutoffs",
                        cutoffsForPercentile.length,
        ...
        for (int d = 0; d < task.destinationPointSetKeys.length; d++) {
            ...
            for (int p = 0; p < task.percentiles.length; p++) {
                ...
                for (int c = 0; c < task.cutoffsMinutes.length; c++) {
                    // Ideally we'd output the pointset IDs (rather than keys) which we have in the
                    // RegionalAnalysis
                    rows.add(
                            new String[] {
                                originId,
