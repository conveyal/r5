...
/**
 * Common supertype of classes that write information from incoming regional work results out into
 * CSV files. Subclasses are used to record origin/destination "skim" matrices, accessibility
 * indicators for non-gridded ("freeform") origin point sets, and cataloging paths between pairs of
...
public abstract class CsvResultWriter extends BaseResultWriter implements RegionalResultWriter {
    ...
    /**
     * Keep a reference to the task, for fetching percentiles, cutoffs, etc. in iteration over
     * multidimensional results. We could copy only the fields we need, but we end up copying six of
     * them to identically named fields.
     */
    protected final RegionalTask task;

    /**
     * Override to return an Enum (usable as String) identifying the kind of results recorded by
     * this CSV writer. This serves as a filename suffix to distinguish between different CSVs
     * generated by a single regional analysis.
     */
     ...
     * "origin", "destination", and the supplied indicator. FIXME it's strange we're manually
     * passing injectable components into objects not wired up at application construction.
     */
    CsvResultWriter(RegionalTask task, String outputBucket, FileStorage fileStorage)
            throws IOException {
        super(fileStorage);
        ...
        this.task = task;
        LOG.info(
                "Created CSV file to hold {} results for regional job {}",
                resultType(),
                task.jobId);
    }
    ...
    /**
     * Gzip the csv file and move it into permanent file storage such as AWS S3. Note: stored file
     * will undergo gzip compression in super.finish(), but be stored with a .csv extension. When
     * this file is downloaded from the UI, the browser will decompress, yielding a logically named
     * .csv file. Downloads through another channel (e.g. aws s3 cp), will need to be decompressed
     * manually.
     */
    @Override
    ...
    public void writeOneWorkResult(RegionalWorkResult workResult) throws Exception {
        // CsvWriter is not threadsafe and multiple threads may call this, so after values are
        // generated,
    ...
    static void checkDimension(
            RegionalWorkResult workResult, String dimensionName, int seen, int expected) {
        if (seen != expected) {
            LOG.error(
                    ...
                    seen,
                    expected);
            // This should be caught up in MultiOriginAssembler and the error flag set on the
            // regional analysis.
    ...
    /**
     * Look up the ID of a destination in the pointset, considering whether we're in one-to-one
     * mode.
     */
