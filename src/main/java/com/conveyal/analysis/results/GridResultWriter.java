 ...
 *
 * <p>During distributed computation of access to gridded destinations, workers return raw results
 * for single origins to the worker while polling. These results contain one accessibility
 * measurement per origin grid cell. This class assembles these results into a single large file
 * containing a delta-coded version of the same data for all origin points.
 *
 * <p>Access grids look like this:
 *
 * <ol>
 *   <li>Header (ASCII text "ACCESSGR", note that this header is eight bytes, so the full grid can
 *       be mapped into a Javascript typed array of 4-byte integers if desired)
...
public class GridResultWriter extends BaseResultWriter {
    ...
    /**
     * The number of different travel time cutoffs being applied when computing accessibility for
     * each origin. The number of values stored per origin cell in an accessibility results grid.
     * Note that we're storing only the number of different cutoffs, but not the cutoff values
     * themselves in the file. This means that the files can only be properly interpreted with the
     * Mongo metadata from the regional analysis. This is an intentional choice to avoid changing
     * the file format, and in any case these files are not expected to ever be used separately from
     * an environment where the Mongo database is available.
     */
    ...
    GridResultWriter(RegionalTask task, String outputBucket, FileStorage fileStorage) {
        ...
        LOG.info(
                "Expecting multi-origin results for grid with width {}, height {}, {} values per"
                    + " origin.",
                width,
                height,
                channels);
        ...
        try {
            ...
            // Initialize the temporary file where the accessibility results will be stored. Setting
            // this newly created
            // file to a larger size should just create a sparse file full of blocks of zeros (at
            // least on Linux).
            // The call to setLength is not strictly necessary (resizing will happen automatically
            // on write, see Javadoc
            // on RandomAccessFile.seek) but seems reasonable since we know the exact size of the
            // resulting file.
            // In the past we filled the file with zeros here, to "overwrite anything that might be
            // in the file already"
            // according to a code comment. However that creates a large burst of disk activity
            // which can run up against
            // IO limits on cloud servers with network storage. Even without initialization, any
            // complete regional analysis
            // would overwrite every byte in the file with a result for some origin point, so the
            // initial values are only
            ...
            randomAccessFile.setLength(
                    HEADER_LENGTH_BYTES + (width * height * channels * Integer.BYTES));
            LOG.info(
     ...
     * in a byte buffer instead of writing mini byte buffers into files? We should also be able to
     * use a filechannel with native order.
     */
    ...
    /**
     * Write all channels at once to the proper subregion of the buffer for this origin. The origins
     * we receive have 2d coordinates. Flatten them to compute file offsets and for the origin
     * checklist.
     */
    synchronized void writeOneOrigin(int taskNumber, int[] values) throws IOException {
        if (values.length != channels) {
            throw new IllegalArgumentException(
                    "Number of channels to be written does not match this writer.");
        }
        ...
        synchronized (this) {
            ...
            // FIXME should this be delta-coded? The Selecting grid reducer seems to expect it to
            // be.
            int lastValue = 0;
