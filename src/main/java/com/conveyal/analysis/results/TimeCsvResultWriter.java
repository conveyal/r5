...
public class TimeCsvResultWriter extends CsvResultWriter {
    ...
    public TimeCsvResultWriter(RegionalTask task, String outputBucket, FileStorage fileStorage)
            throws IOException {
    ...
    /**
     * Check that each dimension of the 2D results array matches the expected size for the job being
     * processed.
     */
    @Override
    protected void checkDimension(RegionalWorkResult workResult) {
        ...
        // In one-to-one mode, we expect only one value per origin, the destination point at the
        // same pointset index as
        ...
        checkDimension(
                workResult,
                "percentiles",
                workResult.travelTimeValues.length,
    ...
    public Iterable<String[]> rowValues(RegionalWorkResult workResult) {
        // Can nested iteration be made identical for all CSV writer subclasses, factoring it out to
        // the superclass?
