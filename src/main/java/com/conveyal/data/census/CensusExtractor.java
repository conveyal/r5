...
public class CensusExtractor {
    /**
     * The precision to use for output files. Set above 6 at your own risk; higher precision files
     * work fine with the reference implementation and with geobuf-java, but break with pygeobuf
     * (see https://github.com/mapbox/pygeobuf/issues/21)
     */
    ...
    public static void main(String... args) throws IOException {
        if (args.length < 3 || args.length > 6) {
            System.err.println(
                    "usage: CensusExtractor (s3://bucket|data_dir) n e s w [outfile.json]");
            System.err.println(
                    "   or: CensusExtractor (s3://bucket|data_dir) boundary.geojson"
                        + " [outfile.json]");
            return;
        }

        SeamlessSource source;
        if (!args[0].startsWith("s3://")) source = new FileSeamlessSource(args[0]);
        else source = new S3SeamlessSource(args[0].substring(5));
        ...
        if (args.length == 6) out = new FileOutputStream(new File(args[5]));
        else if (args.length == 3) out = new FileOutputStream(new File(args[2]));
        else out = System.out;
        ...
        if (out instanceof FileOutputStream) out.close();
    }
