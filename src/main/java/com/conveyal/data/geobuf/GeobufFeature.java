...
public class GeobufFeature implements Cloneable, Serializable {
    ...
    public GeobufFeature(SimpleFeature simpleFeature) {
        ...
        for (Property p : simpleFeature.getProperties()) {
            if (p.getType() instanceof GeometryTypeImpl) continue;
    ...
    /**
     * decode a feature from GeoBuf, passing in the keys in the file and the precision divison (e.g.
     * 1e6 for precision 6)
     */
    public GeobufFeature(Geobuf.Data.Feature feature, List<String> keys, double precisionDivisor) {
        ...
        for (int i = 0; i < feature.getPropertiesCount(); i += 2) {
            ...
            if (val.hasBoolValue()) valObj = val.getBoolValue();
            else if (val.hasDoubleValue()) valObj = val.getDoubleValue();
            else if (val.hasNegIntValue()) valObj = val.getNegIntValue();
            else if (val.hasPosIntValue()) valObj = val.getPosIntValue();
            else if (val.hasStringValue()) valObj = val.getStringValue();
        ...
        else LOG.warn("Unsupported geometry type {}", gbgeom.getType());

        // parse ID
        if (feature.hasIntId()) this.numericId = feature.getIntId();
        else this.id = feature.getId();
    }
    ...
    private Geometry decodeMultipolygon(Geobuf.Data.Geometry gbgeom, double precisionDivisor) {
        ...
        // first length is number of polygons, next is number of rigns, number of coordinates for
        // each ring,
        ...
        for (int poly = 0; poly < npoly; poly++) {
            ...
            for (int ring = 0; ring < nring; ring++) {
                ...
                for (int coordRingIdx = 0; coordRingIdx < ncoord; coordRingIdx++) {
                    ...
                    coords[coordRingIdx] =
                            new Coordinate(x / precisionDivisor, y / precisionDivisor);
                ...
                if (ring == 0) shell = theRing;
                else holes[ring - 1] = theRing;
            }
    ...
    /**
     * return a copy of this object (also makes a defensive copy of properties, but not of the
     * geometry as the geometry is considered immutable)
     */
