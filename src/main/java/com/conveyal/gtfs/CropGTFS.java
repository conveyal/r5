...
/**
 * Remove all stops outside the bounding box, then remove all stop_times outside the bounding box,
 * recording all trips with two or more stop_times inside the bounding box. Then remove all trips
 * with no stoptimes or one single stoptime, then remove all transfers whose stops have been
 * removed.
 *
 * <p>Note that this does not crop the GTFS shapes, only the stops and stoptimes. Therefore in some
 * tools like Transport Analyst, the data set will appear to extend beyond the bounding box because
 * the entire shapes are drawn.
 */
public class CropGTFS {

    // Logger is not super useful because as a library, gtfs-lib has no logger implementation
    // defined by default.
    ...
    private static final String outputFile =
            ""; // "/Users/abyrd/geodata/nl/NL-2016-08-23-noplatforms-noshapes.gtfs.zip";
    ...
    public static void main(String[] args) {
        ...
        // We keep two sets of trip IDs because we only keep trips that are referenced by two or
        // more stopTimes.
        ...
        Map<String, String> stopIdReplacements =
                new HashMap<>(); // Used when collapsing stops into stations.
        ...
        while (stopIterator.hasNext()) {
            ...
            if (MERGE_STATIONS) {
                ...
                // Redefine stations as stops in case GTFS consuming software doesn't allow vehicles
                // to stop at stations.
                // if (stop.location_type == 2) stop.location_type = 1;
            }
            if (!bounds.contains(
                    Geometries.geometryFactory.createPoint(
        ...
        if (MERGE_STATIONS) {
            System.out.println(
                    "Replacing stop_ids in stop_times with those of their parent stations...");
            for (Fun.Tuple2 key : feed.stop_times.keySet()) {
                ...
                if (replacementStopId != null) {
                    // Entry.setValue is an unsupported operation in MapDB, just re-put the
                    // StopTime.
        ...
        System.out.println(
                "Removing stop_times outside the bounding box and finding trips with two or more"
                    + " stop_times inside the bounding box...");
        ...
        while (stIterator.hasNext()) {
            ...
            if (feed.stops.containsKey(stopTime.stop_id)) {
                ...
                // Keep the stop_time, and also record the trip_id it belongs to so we can retain
                // those.
                if (referencedTripIds.contains(stopTime.trip_id)) {
                    ...
                    // This is the first time this trip has been referenced by a stopTime within the
                    // bounding box.
                    referencedTripIds.add(stopTime.trip_id);
                }
            } else {
                // Skip stops outside the bounding box, but keep those within the bounding box.
                // It is important to remove these or we'll end up with trips referencing stop IDs
                // that don't exist.
                stIterator.remove();
            }
        }
        ...
        System.out.println(
                "Removing stoptimes for trips with less than two stop_times inside the bounding"
                    + " box...");
        ...
        while (stIterator.hasNext()) {
            ...
            if (!retainedTripIds.contains(stopTime.trip_id)) {
                // This stop_time's trip is not referenced by two or more stopTimes within the
                // bounding box.
                stIterator.remove();
            }
        }
        ...
        System.out.println(
                "Removing trips that had less than two stop_times inside bounding box...");
        ...
        if (MERGE_STATIONS) {
            System.out.println(
                    "Replacing stop_ids in transfers with those of their parent stations...");
