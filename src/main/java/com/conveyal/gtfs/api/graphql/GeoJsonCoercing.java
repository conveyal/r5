...
public class GeoJsonCoercing implements Coercing {
    ...
    public Object serialize(Object input) {
        ...
        if (input instanceof LineString) {
            ...
            ret.coordinates =
                    Stream.of(((LineString) input).getCoordinates())
                            .map(c -> new double[] {c.x, c.y})
                            .toArray(i -> new double[i][]);

            return ret;
        }
        // Also handle legacy com.vividsolutons JTS LineStrings, which are serialized into our
        // MapDBs
        else if (input instanceof com.vividsolutions.jts.geom.LineString) {
            GeoJsonLineString ret = new GeoJsonLineString();
            ret.coordinates =
                    Stream.of(((com.vividsolutions.jts.geom.LineString) input).getCoordinates())
                            .map(c -> new double[] {c.x, c.y})
                            .toArray(i -> new double[i][]);
