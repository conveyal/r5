...
/**
 * TODO make hierarchy of feedError, tableError, lineError each adding fields and calling
 * constructors.
 *
 * <p>There's a good reason not to use a class hierarchy for errors: we need to instantiate errors
 * from a database. We don't want to use reflection to call different constructors based on the
 * error-type field in the database. We instead use an enum with final fields for severity and
...
public class NewGTFSError {
     ...
     * It is unlikely we'll see files with over 2 billion lines (3 orders of magnitude greater than
     * NL) so 31 bits is enough. Use Integer object rather than primitive to allow NULL because some
     * errors affect an entire table or feed.
     */
    public Integer lineNumber = null;

    // We could hold on to the Entity itself here, because these objects are very short lived.
    // However, extracting fields out of it allows this to serve as a general model for errors even
    // when
    // the entities are gone. We could then re-load them from the database if needed.
    // If it turns out we never need to load them back from the database into Java, then this class
    // is unnecessary
    // and all logic can be pushed down into the errorStorage methods.
    // One advantage of having this class represent errors is the ability to accumulate additional
    // key-value
    // info into the error before storing it.

    /**
     * The id part of the unique key of the line containing the error (convenient but redundant
     * info).
     */
    public String entityId = null;

    /**
     * The sequence number part of the unique key of the line containing the error (when needed for
     * uniqueness).
     */
     ...
     *
     * @return the modified object (builder pattern that allows adding info to newly created error
     *     object without assigning to a variable).
     */
     ...
     *
     * @param entityType should be supplied for a table, may be null if the error affects a whole
     *     feed.
    ...
    // Factory Builder for cases where an entity has not yet been constructed, but we know the line
    // number.
    public static NewGTFSError forLine(
            Table table, int lineNumber, NewGTFSErrorType errorType, String badValue) {
    ...
    // Factory Builder for cases where the entity has already been decoded and an error is
