...
/// place when its coordinates fall inside the polygon. The only slow part of containment testing
/// is the preparatory calculations, so one prepared tester is built per filter instance and reused
/// for all tests during a request.
public class PolygonPlaceFilter implements OnDemandPlaceFilter {
    ...
    public PolygonPlaceFilter (Polygon polygon, StreetLayer streetLayer) {
