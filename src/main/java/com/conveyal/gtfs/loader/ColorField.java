 ...
 *
 * <p>Created by abyrd on 2017-03-30
 */
public class ColorField extends Field {
    ...
    /**
     * From the Postgres manual, on character types: There is no performance difference among these
     * three types, apart from increased storage space when using the blank-padded type, and a few
     * extra CPU cycles to check the length when storing into a length-constrained column. While
     * character(n) has performance advantages in some other database systems, there is no such
     * advantage in PostgreSQL; in fact character(n) is usually the slowest of the three because of
     * its additional storage costs.
     *
     * <p>Arguably colors are integers expressed in hex though.
     */
    @Override
