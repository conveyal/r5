 ...
 *
 * <p>We could also have entity cursors with accessor functions that move along the results, and
 * only look at the fields we need. We could even select only the required fields from the backend
 * database but that gets messy. Anecdotal evidence suggests this would give about a 1/3 speedup.
 * But other observations show no speedup at all. The main benefit would be grabbing arbitrary
 * extension columns.
 *
 * <p>TODO associate EntityPopulator more closely with Entity types and Table instances, so you can
 * get one from the other. e.g. getEntityPopulator() and getTableSpec() on Entity classes.
 *
 * <p>TODO maybe instantiate EntityCreators with a columnForName map to avoid passing them around
 * and make them implement Iterable<T> Could even initialize with the resultSet and call it a
 * Factory
 *
 * <p>// FIXME URLs?
 *
 * <p>This might also be useable with Commons DBUtils as a result row processor.
 */
public interface EntityPopulator<T> {
    ...
    public static final EntityPopulator<Agency> AGENCY =
            (result, columnForName) -> {
                ...
                agency.agency_timezone =
                        getStringIfPresent(result, "agency_timezone", columnForName);
    ...
    public static final EntityPopulator<Route> ROUTE =
            (result, columnForName) -> {
                ...
                route.route_short_name =
                        getStringIfPresent(result, "route_short_name", columnForName);
                route.route_long_name =
                        getStringIfPresent(result, "route_long_name", columnForName);
                ...
                route.route_text_color =
                        getStringIfPresent(result, "route_text_color", columnForName);
                route.route_url = null;
                route.route_branding_url = null;
                return route;
            };

    public static final EntityPopulator<Stop> STOP =
            (result, columnForName) -> {
                Stop stop = new Stop();
                ...
                stop.stop_url =
                        null; // new URL(getStringIfPresent(result, "stop_url",  columnForName));
                stop.location_type = getIntIfPresent(result, "location_type", columnForName);
                stop.wheelchair_boarding =
                        Integer.toString(
                                getIntIfPresent(result, "wheelchair_boarding", columnForName));
                return stop;
            };

    public static final EntityPopulator<Trip> TRIP =
            (result, columnForName) -> {
                Trip trip = new Trip();
                ...
                trip.wheelchair_accessible =
                        getIntIfPresent(result, "wheelchair_accessible", columnForName);
                return trip;
            };

    public static final EntityPopulator<ShapePoint> SHAPE_POINT =
            (result, columnForName) -> {
                ...
                shapePoint.shape_pt_sequence =
                        getIntIfPresent(result, "shape_pt_sequence", columnForName);
                shapePoint.shape_dist_traveled =
                        getDoubleIfPresent(result, "shape_dist_traveled", columnForName);
                return shapePoint;
            };

    public static final EntityPopulator<StopTime> STOP_TIME =
            (result, columnForName) -> {
    ...
    // The reason we're passing in the columnForName map is that resultSet.getX(columnName) throws
    // an exception
    // when the column is not present.
    // Exceptions should only be used in exceptional circumstances (ones that should be logged as
    // errors).
    // Conceivably we could iterate over the fields present using ResultSetMetaData and set the
    // object fields only
    // for those fields present. Or we could create cursor objects that allow accessing the fields
    // of the ResultSet
    // in a typed way. Those cursor objects would make their own columnForName map when constructed.

    public static String getStringIfPresent(
            ResultSet resultSet, String columnName, TObjectIntMap<String> columnForName)
            throws SQLException {
    ...
    public static double getDoubleIfPresent(
            ResultSet resultSet, String columnName, TObjectIntMap<String> columnForName)
            throws SQLException {
        int columnIndex = columnForName.get(columnName);
    ...
    public static int getIntIfPresent(
            ResultSet resultSet, String columnName, TObjectIntMap<String> columnForName)
            throws SQLException {
        int columnIndex = columnForName.get(columnName);
        if (columnIndex == 0) return -1;
