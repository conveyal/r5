...
/**
 * This groups a table name with a description of the fields in the table. It can be normative
 * (expressing the specification for a CSV table in GTFS) or descriptive (providing the schema of an
 * RDBMS table).
 *
 * <p>TODO associate Table with EntityPopulator (combine read and write sides)
 */
public class Table {
    ...
    public static final Table AGENCY =
            new Table(
                    ...
                    new StringField(
                            "agency_id",
                            OPTIONAL), // FIXME? only required if there are more than one
                    new StringField("agency_name", REQUIRED),
                    new URLField("agency_url", REQUIRED),
                    new StringField(
                            "agency_timezone", REQUIRED), // FIXME new field type for time zones?
    ...
    // The GTFS spec says this table is required, but in practice it is not required if
    // calendar_dates is present.
    public static final Table CALENDAR =
            new Table(
                    ...
                    new DateField(
                            "start_date",
                            REQUIRED), // FIXME New field type for dates? Split string and check
                                       // each part.
    ...
    public static final Table ROUTES =
            new Table(
                    ...
                    new StringField(
                            "route_short_name", OPTIONAL), // one of short or long must be provided
                    ...
                    new ColorField(
                            "route_color", OPTIONAL), // really this is an int in hex notation
    ...
    public static final Table STOP_TIMES =
            new Table(
                    ...
                    new StringField("stop_id", REQUIRED),
                    // TODO verify that we have a special check for arrival and departure times
                    // first and last stop_time in a trip, which are reqiured
    ...
    /**
     * Create an SQL table with all the fields specified by this table object, plus an integer CSV
     * line number field in the first position.
     */
    public void createSqlTable(Connection connection) {
        ...
        // Adding the unlogged keyword gives about 12 percent speedup on loading, but is
        // non-standard.
        String createSql =
                String.format("create table %s (csv_line integer, %s)", name, fieldDeclarations);
        try {
    ...
    /**
     * Create an SQL table that will insert a value into all the fields specified by this table
     * object, plus an integer CSV line number field in the first position.
     */
    public String generateInsertSql() {
        ...
        return String.format(
                "insert into %s (csv_line, %s) values (?, %s)",
                name, joinedFieldNames, questionMarks);
    }

    /**
     * @param name a column name from the header of a CSV file
     * @return the Field object from this table with the given name. If there is no such field
     *     defined, create a new Field object for this name.
     */
