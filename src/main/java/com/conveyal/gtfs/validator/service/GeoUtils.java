...
/**
 * GeoUtils ported from old GTFS validator. Does everything with JTS MathTransforms and UTM, which
 * is needlessly complicated, so all functions are deprecated.
 */
public class GeoUtils {
    ...
    public static GeometryFactory projectedGeometryFactory =
    ...
    @Deprecated
    public static ProjectedCoordinate convertLonLatToEuclidean(Coordinate lonlat) {
        ...
        try {
            ...
            final CoordinateReferenceSystem dataCRS =
                    crsAuthorityFactory.createCoordinateReferenceSystem(
                            "EPSG:" + getEPSGCodefromUTS(refLatLon)); // EPSG:32618

            //      parameters = mtFactory.getDefaultParameters("Transverse_Mercator");
            //
            //      final double zoneNumber = zone;
            //      final double utmZoneCenterLongitude = (zoneNumber - 1) * 6 - 180 + 3; // +3 puts
            // origin
            ...
            try {
                distance +=
                        JTS.orthodromicDistance(
                                tripGeometry.getCoordinateN(i),
                                tripGeometry.getCoordinateN(i + 1),
