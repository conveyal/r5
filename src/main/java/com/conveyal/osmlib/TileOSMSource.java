...
public class TileOSMSource implements OSMEntitySource {
    ...
    public void setBoundingBox(double minLat, double minLon, double maxLat, double maxLon) {
        ...
        // Note that y tile numbers are increasing in the opposite direction of latitude (from north
        // to south)
    ...
    public void copyTo(OSMEntitySink sink) {
        // Avoid writing out shared/intersection nodes more than once. Besides being wasteful, the
        // first node in one way
        // may be the last node in the previous way output, which would create a node ID delta of
        // zero and prematirely
        // end the block.
        ...
        try {
            ...
            for (int pass = 0; pass < 2; pass++) {
                for (int x = minX; x <= maxX; x++) {
                    // SortedSet provides one-dimensional ordering and iteration. Tuple3 gives an
                    // odometer-like ordering.
                    // Therefore we must vary one of the dimensions "manually". Consider a set
                    // containing all the
                    // integers from 00 to 99 at 2-tuples. The range from (1,1) to (2,2) does not
                    // contain the four
                    // elements (1,1) (1,2) (2,1) (2,2). It contains the elements (1,1) (1,2) (1,3)
                    // (1,4) ... (2,2).
                    @SuppressWarnings("unchecked")
                    NavigableSet<Tuple3<Integer, Integer, Long>> xSubset =
                            osm.index.subSet(
                                    new Tuple3(x, minY, null),
                                            true, // inclusive lower bound, null tests lower than
                                                  // anything
                                    new Tuple3(x, maxY, Fun.HI),
                                            true // inclusive upper bound, HI tests higher than
                                                 // anything
                                    );
                    for (Tuple3<Integer, Integer, Long> item : xSubset) {
                        ...
                        if (pass == 0) { // Nodes
                            for (long nodeId : way.nodes) {
                                ...
                                if (node == null) {
                                    LOG.error(
                                            "Way references a node {} that was not loaded.",
                                            nodeId);
                                } else {
                                    ...
                                    Set<Fun.Tuple2<Long, Long>> relationsForNode =
                                            osm.relationsByNode.subSet(
                            ...
                            Set<Fun.Tuple2<Long, Long>> relationsForWay =
                                    osm.relationsByWay.subSet(
                                            new Fun.Tuple2(wayId, null),
                                            new Fun.Tuple2(wayId, Fun.HI));
                ...
                do {
                    ...
                    // we have to do this recursively as a relation could refer to a relation that
                    // in turn refers to a
                    // relation. Note that since we are using a set we can't loop infinitely if
                    // there is a cycle or self
                    // referential relation.
                    for (TLongIterator it = relationsSeen.iterator(); it.hasNext(); ) {
                        long relation = it.next();

                        Set<Fun.Tuple2<Long, Long>> relationsForRelation =
                                osm.relationsByRelation.subSet(
