 ...
 */
public class LongFlagsFST implements IenumSet, Serializable {
    TLongList flags;
    ...
    public enum LEdgeFlag {
        ...
        private LEdgeFlag (long bitNumber) {
            flag = 1L << bitNumber;
        }
    ...
    public LongFlagsFST(List<EnumSet<EdgeStore.EdgeFlag>> flags) {
        this.flags = new TLongArrayList(flags.size());
    ...
    public void serialize(OutputStream outputStream) throws IOException {
        LOG.info("Writing long flags...");
        FSTObjectOutput out = getConf().getObjectOutput(outputStream);
        out.writeObject(this, LongFlagsFST.class);
        out.flush();
        outputStream.close();
    ...
    public List<EnumSet<EdgeStore.EdgeFlag>> deserialize(InputStream inputStream) throws Exception {
        LOG.info("Reading long flags...");
        FSTObjectInput in = getConf().getObjectInput(inputStream);
        LongFlagsFST longFlagsFST = (LongFlagsFST) in.readObject(LongFlagsFST.class);
        LOG.info("Finished");

        return longFlagsFST.toList();
    }
