...
import com.conveyal.r5.transit.path.PathScorer;
import com.conveyal.r5.transit.path.PathWriter;
...
public class PerTargetPropagater {
    ...
    private StreetTimeAndMode[] perIterationEgress;
    ...
    public PerTargetPropagater(
            ...
            TravelTimeReducer travelTimeReducer,
            int[][] travelTimesToStopsForIteration,
            int[] nonTransitTravelTimesToTargets,
            List<Path[]> pathsToStopsForIteration
    ) {
        this.request = task;
        this.travelTimeReducer = travelTimeReducer;
        ...
        this.pathsToStopsForIteration = pathsToStopsForIteration;

        // Prepare a set of modes, all of which will simultaneously be used for on-street egress.
        this.modes = LegMode.toStreetModeSet(request.egressModes);
        ...
        if (task.makeTauiSite) {
            ...
            // them to the backend).
            pathWriter = new PathWriter(task.taskId, task.width * task.height, task.nPathsPerTarget);
        ...
        for (StreetMode streetMode : modes) {
            LinkedPointSet linkedTargetsForMode = network.linkageCache
                    .getLinkage(targets, network.streetLayer, streetMode);
    ...
    public OneOriginResult propagate () {
        ...
        if (savePaths != SavePaths.NONE) {
            ...
            perIterationEgress = new StreetTimeAndMode[nIterations];
        }
    ...
    private void propagateTransit (int targetIndex, LinkedPointSet linkedTargets) {
        ...
        RoutingVariable unit = egressCostTable.linkageCostUnit;
        ...
        if (pointToStopLinkageCostTable != null) {
            ...
            pointToStopLinkageCostTable.forEachEntry((stop, linkageCost) -> {
                ...
                if (unit == RoutingVariable.DISTANCE_MILLIMETERS) {
                    secondsFromStopToTarget = linkageCost / speedMillimetersPerSecond;
                } else if (unit == RoutingVariable.DURATION_SECONDS) {
                ...
                if (secondsFromStopToTarget < egressLegTimeLimitSeconds){
                    ...
                    StreetTimeAndMode egress = new StreetTimeAndMode(
