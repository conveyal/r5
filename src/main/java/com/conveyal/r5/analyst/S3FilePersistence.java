 ...
 *
 * <p>For any file whose length is not known in advance, the S3 client will buffer the whole thing
 * in memory. This obviously dangerous because it can exhaust memory. However, when we are going to
 * gzip files, we just don't know the size in advance. If the files are known to be reasonably
 * small, we can do all the layout and compression in memory buffers then upload from the buffer,
 * whose length is known.
 *
 * <p>From docs: "callers must supply the size of options in the stream through the content length
 * field in the ObjectMetadata parameter. If no content length is specified for the input stream,
 * then TransferManager will attempt to buffer all the stream contents in memory and upload the
 * options as a traditional, single part upload. Because the entire stream contents must be buffered
 * in memory, this can be very expensive, and should be avoided whenever possible."
 */
public class S3FilePersistence extends FilePersistence {
    ...
    /**
     * The common prefix of all accessed buckets. Example: if baseBucket is "analysis-staging",
     * files of type POLYGON will be in bucket "analysis-staging-polygons". NOTE: For now this only
     * applies to reading files using S3FilePersistence, not writing them. TODO if this works well,
     * extend to all other bucket names and file loading / saving.
     */
    ...
    // Low-level client, for now we're trying the high-level TransferManager TODO maybe use this so
    // we don't have to shut down the transferManager
    ...
    public S3FilePersistence(String region, String baseBucket) {
        ...
        this.amazonS3 =
                AmazonS3ClientBuilder.standard()
                        // .enableAccelerateMode() // this fails looking up
    ...
    public void saveData(String directory, String fileName, PersistenceBuffer persistenceBuffer) {
        try {
            ...
            // Set content encoding to gzip. This way browsers will decompress on download using
            // native deflate code.
            ...
            // We must setContentLength or the S3 client will re-buffer the InputStream into another
            // memory buffer.
            metadata.setContentLength(persistenceBuffer.getSize());
            //            amazonS3.putObject(directory, fileName,
            // persistenceBuffer.getInputStream(), metadata);
            final Upload upload =
                    transferManager.upload(
                            directory, fileName, persistenceBuffer.getInputStream(), metadata);
    ...
    public InputStream getData(FileCategory category, String name) {
        ...
        // TODO progress reporting, cacheing in local file using getObject(GetObjectRequest
        // getObjectRequest, File destinationFile) or TransferManager.download()
        InputStream inputStream = s3Object.getObjectContent();
        return inputStream;
    }

    /**
     * Example: for baseBucket "analysis-staging" and file type POLYGON, returns
     * "analysis-staging-polygons".
     */
        ...
        public void progressChanged(ProgressEvent progressEvent) {
            ...
            if (eventType == ProgressEventType.REQUEST_BYTE_TRANSFER_EVENT
                ...
                if (now > lastLogTime + LOG_INTERVAL_SECONDS * 1000
                    ...
                    LOG.info(
                            "{} transferred {} of {} ({} percent), duration {}, speed {})",
