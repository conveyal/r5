...
/**
 * A pointset that represents a grid of pixels from the web mercator projection. This PointSet
 * subclass does not yet support opportunity counts. TODO merge this class with Grid, which does
 * have opportunity counts.
 */
public class WebMercatorGridPointSet extends PointSet implements Serializable {
     ...
     *
     * @oaram basePointSet the super-grid pointset from which linkages will be copied or shared, or
     *     null if no such grid exists.
     */
    public WebMercatorGridPointSet(
            int zoom,
            int west,
            int north,
            int width,
            int height,
            WebMercatorGridPointSet basePointSet) {
        this.zoom = zoom;
        this.west = west;
        this.north = north;
        this.width = width;
    ...
    /**
     * Constructs a grid point set that covers the entire extent of the supplied transport network's
     * street network. This usually serves as the base supergrid pointset for other smaller grids in
     * the same region.
     */
    ...
    /**
     * The resulting PointSet will not have a null basePointSet, so should generally not be used for
     * linking.
     */
    ...
    public double sumTotalOpportunities() {
        // For now we are counting each point as 1 opportunity because this class does not have
        // opportunity counts.
        return featureCount();
    }
    ...
    public TIntList getPointsInEnvelope(Envelope envelopeFixedDegrees) {
        // Convert fixed-degree envelope to floating, then to world-scale web Mercator pixels at
        // this grid's zoom level.
        ...
        // Make the envelope's pixel values relative to the edges of this WebMercatorGridPointSet,
        // rather than
        ...
        // Pixels are truncated toward zero, and coords increase toward East and South in web
        // Mercator, so <= south/east.
    ...
    /**
     * Return the x pixel number containing the given longitude at this grid's zoom level, relative
     * to the left edge of the world (not relative to this grid or to any particular tile). TODO
     * This method could be reusable and static if zoom level was a parameter. And moved to
     * WebMercatorExtents.
     */
    ...
    /**
     * Return the y pixel number containing the given latitude at this grid's zoom level, relative
     * to the top edge of the world (not relative to this grid or to any particular tile). TODO This
     * method could be reusable and static if zoom level was a parameter. And moved to
     * WebMercatorExtents.
     */
    ...
    /**
     * Return the longitude in degrees of the west edge of any pixel at the specified x coordinate
     * relative to the left edge of the world (not relative to this grid or to any particular tile),
     * at this grid's zoom level. TODO This method could be reusable and static if zoom level was a
     * parameter. It should probably also be renamed to clarify that it doesn't return the center of
     * the pixel. Another equivalent method is found in Grid, and should probably be merged with
     * this one and WebMercatorExtents.
     */
    ...
    /**
     * Return the latitude in degrees of the north edge of any pixel at the specified y coordinate
     * relative to the top edge of the world (not relative to this grid or to any particular tile),
     * at this grid's zoom level. TODO This method could be reusable and static if zoom level was a
     * parameter. It should probably also be renamed to clarify that it doesn't return the center of
     * the pixel. Another equivalent method is found in Grid, and should probably be merged with
     * this one and WebMercatorExtents.
    ...
    public double pixelToLat(double y) {
        ...
        return Math.toDegrees(
                Math.atan(Math.sinh(Math.PI - tile * Math.PI * 2 / Math.pow(2, zoom))));
    }
