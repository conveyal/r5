...
/**
 * A fare calculator used in Analyst searches. The currency is not important as long as it is
 * integer and constant (i.e. the whole thing is in cents, yen, bitcoin or kina).
 *
 * <p>Fare calculator need not be greedy, see https://doi.org/10.1080/13658816.2019.1605075 and
 * summary/open-access preprint at
 * https://indicatrix.org/post/how-transit-fares-affect-accessibility/
 */
    ...
    public static Collater getCollator(ProfileRequest request) {
        return (states, maxClockTime) -> {
            ...
            for (McRaptorState state : states) {
                ...
                // We generally try not to impose cutoffs at calculation time, but leaving two free
                // cutoffs creates a grid
                // of possibilities that is too large to be stored.
                FareBounds fareAtState =
                        request.inRoutingFareCalculator.calculateFare(state, maxClockTime);
