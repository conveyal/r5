...
/**
 * An in-routing fare calculator for East-of-Hudson services in the NYC area, used for a specific
 * project with customized inputs. See note in NYCStaticFareData.
 *
 * @author mattwigway
 */
public class NYCInRoutingFareCalculator extends InRoutingFareCalculator {
     ...
     * weak so that it doesn't prevent GC of no longer used transport networks (though this is
     * largely hypothetical, as Analysis workers never load more than a single transport network.
     */
    private static final Map<TransitLayer, NYCFareDataCache> fareDataForTransitLayer =
            new WeakHashMap<>();

    /**
     * Map from NYCPatternType -> Integer for how much discount off the LIRR is implied by each
     * MetroCard Transfer Source
     */
    public Map<NYCPatternType, Integer> toLirrDiscounts = null;

    /**
     * Map from NYCPatternType -> Integer for how much discount off the MNR is implied by each
     * MetroCard Transfer Source
     */
    public Map<NYCPatternType, Integer> toMetroNorthDiscounts = null;

    /**
     * What type of MetroCard Transfer Allowance does riding the LIRR get you? This is used in
     * scenarios, we can make a full-fare LIRR ride give you the same transfer allowance as some
     * other type of ride that is already implemented, e.g. NICE_ONE_TRANSFER for a single free
     * transfer to NICE, or METROCARD_LOCAL_BUS for free transfers to most services and an upgrade
     * to the express bus. No LIRR MetroCard Transfer Allowance is granted if a discounted transfer
     * was used to board the LIRR.
     */
    ...
    /**
     * Map from stop -> stop -> fare for fare overrides coming from JSON, for Metro-North offpeak
     * fares
     */
    ...
    /**
     * Map from stop -> stop -> fare for fare overrides coming from JSON, for LIRR offpeak direct
     * fares
     */
    Map<LIRRStop, TObjectIntMap<LIRRStop>> lirrOffPeakDirectFareOverrides = null;

    /**
     * Map for from stop -> to stop -> via stop -> fare for LIRR peak via fare overrides coming from
     * JSON
     */
    Map<LIRRStop, Map<LIRRStop, TObjectIntMap<LIRRStop>>> lirrPeakViaFareOverrides = null;

    /**
     * Map for from stop -> to stop -> via stop -> fare for LIRR offpeak via fare overrides coming
     * from JSON
     */
    ...
    private TIntObjectMap<TIntIntMap> initializeMnrDirectFareOverrides(
            List<FareOverride> fareOverrides, boolean peak) {
        ...
        if (fareOverrides != null) {
            for (FareOverride fo : fareOverrides) {
                ...
                if (!fares.containsKey(fromStop)) fares.put(fromStop, new TIntIntHashMap());
    ...
    /**
     * Initialize the fare overrides for direct trips on the LIRR. Can't re-use Metro-North code b/c
     * it used int stop IDs and LIRR fare calculation uses enums
     */
    private Map<LIRRStop, TObjectIntMap<LIRRStop>> initializeLirrDirectFareOverrides(
            List<FareOverride> fareOverrides, boolean peak, NYCFareDataCache fareData) {
        ...
        if (fareOverrides != null) {
            for (FareOverride fo : fareOverrides) {
                ...
                fares.computeIfAbsent(fromLirrStop, k -> new TObjectIntHashMap<>())
                        .put(toLirrStop, fare);
            }
        }
        return fares;
    ...
    /**
     * Initialize the fare overrides for via trips on the LIRR - returns a map keyed as from -> to
     * -> via -> fare
     */
    private Map<LIRRStop, Map<LIRRStop, TObjectIntMap<LIRRStop>>> initializeLirrViaFareOverrides(
            List<FareOverride> fareOverrides, boolean peak, NYCFareDataCache fareData) {
    ...
    private void initializeFareOverrides(NYCFareDataCache fareData) {
        if (mnrPeakFareOverrides == null) {
            synchronized (this) {
                if (mnrPeakFareOverrides == null) {
                    mnrPeakFareOverrides =
                            initializeMnrDirectFareOverrides(metroNorthFareOverrides, true);
                    mnrOffPeakFareOverrides =
                            initializeMnrDirectFareOverrides(metroNorthFareOverrides, false);

                    lirrPeakDirectFareOverrides =
                            initializeLirrDirectFareOverrides(lirrFareOverrides, true, fareData);
                    lirrOffPeakDirectFareOverrides =
                            initializeLirrDirectFareOverrides(lirrFareOverrides, false, fareData);

                    lirrPeakViaFareOverrides =
                            initializeLirrViaFareOverrides(lirrFareOverrides, true, fareData);
                    lirrOffPeakViaFareOverrides =
                            initializeLirrViaFareOverrides(lirrFareOverrides, false, fareData);
                }
            }
        }
    }
    ...
    public FareBounds calculateFare(
        ...
        while (stateForTraversal != null) {
            ...
            boardStops.add(
                    transitLayer.tripPatterns.get(stateForTraversal.pattern)
                            .stops[stateForTraversal.boardStopPosition]);
            boardTimes.add(stateForTraversal.boardTime);

            // last condition is to avoid using loop transfer to throw away a negative Metro-North
            // or LIRR transfer allowance
            // check if back is an on-street transfer
            boolean onStreetTransfer =
                    stateForTraversal.back != null
                            && stateForTraversal.back.back != null
                            && stateForTraversal.back.pattern == -1
                            && stateForTraversal.back.stop != stateForTraversal.back.back.stop;
        ...
        // Metrocard fare information: where the fare came from (i.e. what mode was paid for), as
        ...
        boolean inSubwayPaidArea =
                false; // when inside the subway paid area, we can change to a new subway line for
                       // free
        ...
        // transfers from the New Haven line to the Harlem/Hudson lines, at least not with the
        // one-way tickets
        ...
        // ==========================================
        // run one extra iteration, so the cleanup code to pay LIRR and MNR fares and check for
        // subway exit
        ...
        for (int i = 0; i < patterns.size() + 1; i++) {
            ...
            if (i > 0) {
                // Don't do this at the end of the loop since there is short-circuit logic
                // (continue) below
            ...
            boolean thisPatternLirr =
                    NYCPatternType.LIRR_OFFPEAK.equals(patternType)
                            || NYCPatternType.LIRR_PEAK.equals(patternType);
            if (lirrBoardStops != null && (onStreetTransfer || !thisPatternLirr)) {
                // we have left the LIRR, either by transferring on street or riding a different
                // transit service.
                lirr =
                        new LIRRTransferAllowance(
                                lirrBoardStops,
                                lirrAlightStops,
                                lirrDirections,
                                lirrBoardTimes,
                ...
                int lirrDiscount =
                        getToLirrDiscountForMetrocardTransferSource(metrocardTransferSource);
                if (lirrDiscount > 0 && lirrBoardTimes.get(0) <= metrocardTransferExpiry) {
                    ...
                    // NB this may create issues if the transfer discounts are asymmetrical, by
                    // creating places where
                    // transfer allowances are negative. E.g. if a NICE bus gives a 1.75 discount
                    // off LIRR, and LIRR
                    // gives a 2.75 discount off local bus, riding the NICE bus before LIRR and
                    // foregoing the later discount
                    // makes the fare $1 higher than if you had discarded the NICE allowance
                    // beforehand.
                    // This also is slightly incorrect when a second ticket (rather than a via fare)
                    // was purchased for the
                    // LIRR, as in this case you would expect to get the second transfer, but that's
                    // a pretty minor edge case.
                    if (lirrMetrocardTransferSource != null) {
                        ...
                        metrocardTransferExpiry =
                                Math.min(
                                        lirr.mostRecentTicketPurchaseTime
                                                + NYCStaticFareData
                                                        .METROCARD_TRANSFER_VALIDITY_TIME_SECONDS,
                                        maxClockTime);
                    }
                }
                cumulativeFare += lirrFare;
                if (!lastIteration) { // don't clear LIRR transfer allowance information on last
                                      // iteration, save it for use in dominance
                    lirr = null;
                    lirrBoardStops = null;
            ...
            boolean thisPatternMnr =
                    NYCPatternType.METRO_NORTH_PEAK.equals(patternType)
                            || NYCPatternType.METRO_NORTH_OFFPEAK.equals(patternType);
            if (metroNorthBoardStop != -1 && (onStreetTransfer || !thisPatternMnr)) {
                TIntObjectMap<TIntIntMap> overrideFares =
                        metroNorthPeak ? mnrPeakFareOverrides : mnrOffPeakFareOverrides;
                int mnrFare;
                if (overrideFares.containsKey(metroNorthBoardStop)
                        && overrideFares
                                .get(metroNorthBoardStop)
                                .containsKey(metroNorthAlightStop)) {
                    mnrFare = overrideFares.get(metroNorthBoardStop).get(metroNorthAlightStop);
                } else {
                    mnrFare =
                            fareData.getMetroNorthFare(
                                    metroNorthBoardStop, metroNorthAlightStop, metroNorthPeak);
                }

                int mnrDiscount =
                        getToMnrDiscountForMetrocardTransferSource(metrocardTransferSource);
                ...
                if (!lastIteration) {
                    // Don't clear Metro-North transfer information before returning transfer
                    // allowance
            ...
            // All NICE routes have a list of allowable transfers, e.g. see the lower left corner of
            // this schedule:
            // http://www.nicebus.com/NICE/media/NiceBusPDFSchedules/NICE-n31_MapSchedule.pdf
            // Implementing these is too complicated, so we just assume that any transfer that can
            // be made on
            // foot is allowable. But clear the transfer allowance if another service, such as Long
            // Island Rail Road,
            ...
            if (NYCPatternType.METROCARD_NICE.equals(metrocardTransferSource)
                ...
                if (!(NYCPatternType.niceTransfers.contains(patternType)
                        ...
                        || (getToMnrDiscountForMetrocardTransferSource(metrocardTransferSource) > 0
                                && (NYCPatternType.METRO_NORTH_PEAK.equals(patternType)
                                        || NYCPatternType.METRO_NORTH_OFFPEAK.equals(
                                                patternType))))) {
                    // we are riding a service that doesn't provide a NICE discount. Clear the
                    // transfer allowance.
                    // This is correct even in the METROCARD_NICE_ONE_TRANSFER case, because if we
                    // transferred to another
                    // MetroCard service that _doesn't_ accept the NICE_ONE_TRANSFER transfer, a new
                    // ticket will be purchased and
            ...
            // This is to sort-of enforce that you can only get a second transfer if you board near
            // the ferry terminal.
            ...
            // This is to sort-of enforce that you can only get a second transfer if you board near
            // the SIR, and keep
            // the router from doubling back to lower manhattan on the ferry and transferring to a
            // bus.
            if (NYCPatternType.LOCAL_BUS_OR_SUBWAY_TO_SI_FERRY_TO_SIR.equals(
            ...
            // shouldn't matter, as riding any MetroCard service will clear it, but just for good
            // measure
            if (NYCPatternType.LOCAL_BUS_TO_SIR.equals(metrocardTransferSource)
                    && !NYCPatternType.STATEN_ISLAND_FERRY.equals(patternType)) {
            ...
            // LOCAL_BUS_TO_SI_FERRY, SUBWAY_TO_SI_FERRY: Only special when boarding SIR from Ferry,
            // otherwise behaves exactly like a local
            //   bus or subway transfer allowance. These allowances are cleared if the SI Ferry is
            // ridden twice, to prevent the router from doubling
            //   back using the ferry to get a local bus -> SIR -> local bus ride entirely on Staten
            // Island.

            // II.E. CHECK FOR SUBWAY SYSTEM EXIT
            // inSubwayPaidArea refers to whether we were in the subway paid area _before_ this ride
            // at this point.
            // Elvis has left the subway
            if (inSubwayPaidArea) {
                // If we're still riding the subway, check if it was a behind-gates transfer
                if (NYCPatternType.METROCARD_SUBWAY.equals(patternType))
                    inSubwayPaidArea =
                            hasBehindGatesTransfer(previousAlightStop, boardStop, fareData);
                // If we're not still riding the subway, well then, it is tautologically clear that
                // we have
                // left the subway.
            ...
            if (NYCPatternType.METROCARD_LOCAL_BUS.equals(patternType)) {
                if (boardTime <= metrocardTransferExpiry
                        &&
                        // Allow transfers from all the metrocard types except
                        // METROCARD_NICE_ONE_TRANSFER and the
                        // SUFFOLK ones, which do not allow transfers to the MTA
                        (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)
                                ||
                                // Special case: one more free transfer to local bus on Staten
                                // Island
                                // (bus/subway to SI Ferry to SIR implies we are now on Staten
                                // Island)
                                NYCPatternType.LOCAL_BUS_OR_SUBWAY_TO_SI_FERRY_TO_SIR.equals(
                                        metrocardTransferSource)
                                ||
                                // Special case: one more free transfer to local bus in Manhattan
                                // (bus to SIR to Ferry implies we are now in Manhattan)
                                NYCPatternType.LOCAL_BUS_TO_SIR_TO_SI_FERRY.equals(
                                        metrocardTransferSource)
                                ||
                                ...
                                || NYCPatternType.METROCARD_EXPRESS_BUS.equals(
                                        metrocardTransferSource)
                                ...
                                || NYCPatternType.STATEN_ISLAND_RWY.equals(
                                        metrocardTransferSource))) {
                    // use transfer - free transfers to local bus from any MetroCard transferable
                    // route
                    // clear transfer information
                    // Not going to implement prohibited bus-bus transfers, as they are mostly to
                    // prevent round-trips
            ...
            // III.B. MTA EXPRESS BUS
                ...
                if (boardTime <= metrocardTransferExpiry
                        &&
                        // Everything except NICE_ONE_TRANSFER, the SUFFOLK types, and the special
                        // types allowing two
                        ...
                        (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)
                                ...
                                || NYCPatternType.STATEN_ISLAND_RWY.equals(
                                        metrocardTransferSource))) {
                    // transfer from subway or local bus to express bus, pay upgrade fare and clear
                    // transfer allowance
                    ...
                    // Free express bus -> express bus transfer, see
                    // http://web.mta.info/nyct/fare/pdf/NYCTransit_Tariff.pdf
                    // page 33.
                    metrocardTransferSource = null;
                    metrocardTransferExpiry = maxClockTime;
                } else {
            ...
            // III.C. MTA SUBWAY
                ...
                if (boardTime <= metrocardTransferExpiry
                        &&
                        // Everything except NICE_ONE_TRANSFER and the SUFFOLK types, the
                        // intermediate or
                        // Staten Island-bound Staten Island special cases and the subway
                        // (free subway transfers are only allowed within the subway paid area, and
                        // are handled above)
                        (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)
                                ||
                                // It's okay to not ride the SIR after the ferry
                ...
                // we are now in the subway paid area, regardless of whether we boarded via a
                // transfer or not.
            ...
            // III.D. STATEN ISLAND RAILWAY
                ...
                // Fare is only paid on the SIR at St George and Tompkinsville, both for boarding
                // and alighting.
                ...
                if (nFarePayments == 0)
                    continue; // NO FARE INTERACTION, DO NOT UPDATE TRANSFER ALLOWANCES
                else {
                    if (boardTime <= metrocardTransferExpiry
                            &&
                            // this is all the metrocard types except METROCARD_NICE_ONE_TRANSFER
                            // and the
                            // SUFFOLK ones, which do not allow transfers to the MTA, as well as the
                            // SIR
                            // because we assume no free SIR-SIR transfers.
                            (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)
                                    || NYCPatternType.LOCAL_BUS_TO_SI_FERRY.equals(
                                            metrocardTransferSource)
                                    || NYCPatternType.SUBWAY_TO_SI_FERRY.equals(
                                            metrocardTransferSource)
                                    || NYCPatternType.METROCARD_EXPRESS_BUS.equals(
                                            metrocardTransferSource)
                                    || NYCPatternType.METROCARD_SUBWAY.equals(
                                            metrocardTransferSource)
                                    || NYCPatternType.METROCARD_NICE.equals(
                                            metrocardTransferSource))) {
                    ...
                    if (nFarePayments == 0) {
                        ...
                        if (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)) {
                            ...
                            // Island, because otherwise the transfer source would be
                            // LOCAL_BUS_TO_SI_FERRY
                            metrocardTransferSource = NYCPatternType.LOCAL_BUS_TO_SIR;
                        } else if (NYCPatternType.LOCAL_BUS_TO_SI_FERRY.equals(
                                        metrocardTransferSource)
                                || NYCPatternType.SUBWAY_TO_SI_FERRY.equals(
                                        metrocardTransferSource)) {
                            // allow one more free transfer to local bus
                            metrocardTransferSource =
                                    NYCPatternType.LOCAL_BUS_OR_SUBWAY_TO_SI_FERRY_TO_SIR;
                        } else {
                            metrocardTransferSource = null;
                            metrocardTransferExpiry = maxClockTime;
                        }
            ...
            // III.E. NICE BUS
            // Some special cases here to handle that you get two free transfers to the NICE, but
            // only
            // one to MTA services
            else if (NYCPatternType.METROCARD_NICE.equals(patternType)) {
                if (boardTime <= metrocardTransferExpiry
                        &&
                        // this is all the metrocard types except the special ones allowing a third
                        // transfer
                        // when the SI Ferry and SIR are involved
                        (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource)
                                ||
                                // Okay to board nice after riding the ferry (though I'm not sure
                                // it's possible)
                                NYCPatternType.LOCAL_BUS_TO_SI_FERRY.equals(metrocardTransferSource)
                                || NYCPatternType.SUBWAY_TO_SI_FERRY.equals(metrocardTransferSource)
                                || NYCPatternType.METROCARD_EXPRESS_BUS.equals(
                                ...
                                || NYCPatternType.METROCARD_SUBWAY.equals(metrocardTransferSource)
                                || NYCPatternType.METROCARD_NICE.equals(metrocardTransferSource)
                                || NYCPatternType.METROCARD_NICE_ONE_TRANSFER.equals(
                                        metrocardTransferSource)
                                || NYCPatternType.SUFFOLK.equals(metrocardTransferSource)
                                || NYCPatternType.SUFFOLK_ONE_TRANSFER.equals(
                                        metrocardTransferSource)
                                || NYCPatternType.STATEN_ISLAND_RWY.equals(
                                        metrocardTransferSource))) {
                    // use the transfer
                    if (NYCPatternType.METROCARD_NICE.equals(metrocardTransferSource)) {
                        ...
                        // consume any other transfer type, with special logic for Suffolk upgrade
                        // fares
                        if (NYCPatternType.SUFFOLK.equals(metrocardTransferSource)) {
                            // buy the transfer slip and the upgrade to the NICE fare, don't allow
                            // further transfers
                            cumulativeFare +=
                                    NYCStaticFareData.SUFFOLK_TRANSFER_SLIP
                                            + NYCStaticFareData.SUFFOLK_NICE_TRANSFER;
                        } else if (NYCPatternType.SUFFOLK_ONE_TRANSFER.equals(
                                metrocardTransferSource)) {
            ...
            // III.F. SUFFOLK COUNTY TRANSIT
                ...
                if (transferValid
                    ...
                    metrocardTransferSource =
                            NYCPatternType.SUFFOLK_ONE_TRANSFER; // update to one_transfer allowance
                    ...
                    // Buy an SCT ticket
                    // Note that Suffolk County Transit does not use the MetroCard system, but it
                    // simplifies implementation
                    // if we treat it like it does. This does prevent a user from holding a
                    // Metrocard transfer
                    // and a Suffolk transfer simultaneously, but I can't imagine why this would be
                    // useful.
                    // Suffolk <-> NICE is handled explicitly, and the other MetroCard services are
                    // so far
                    ...
                    metrocardTransferExpiry =
                            Math.min(
                                    boardTime
                                            + NYCStaticFareData
                                                    .SUFFOLK_TRANSFER_VALIDITY_TIME_SECONDS,
                                    maxClockTime);
                }
            ...
            // III.G. STATEN ISLAND FERRY
                ...
                // No fare interaction, as the Staten Island Ferry is free. But we do use the SI
                // ferry to "enforce"
                // the two-transfer rule for Local Bus -> SIR -> Bus/Subway in lower Manhattan and
                // vice-versa, so
                // update Metrocard transfer allowance appropriately
                if (NYCPatternType.LOCAL_BUS_TO_SIR.equals(metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.LOCAL_BUS_TO_SIR_TO_SI_FERRY;
                else if (NYCPatternType.METROCARD_LOCAL_BUS.equals(metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.LOCAL_BUS_TO_SI_FERRY;
                else if (NYCPatternType.METROCARD_SUBWAY.equals(metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.SUBWAY_TO_SI_FERRY;
                // prevent the router from riding the SI Ferry twice to get a free trip entirely on
                // Staten Island by doing bus -> SI Ferry -> SI Ferry -> SIR -> Bus
                // Don't take away the transfer allowance when doing this, just remove the special
                // "ferry" privileges
                // To get the special Staten Island transfer, you must ride the ferry an odd number
                // of times, which means
                // you must have started on Staten Island and ended in Manhattan, or vice-versa.
                else if (NYCPatternType.LOCAL_BUS_TO_SI_FERRY.equals(metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.METROCARD_LOCAL_BUS;
                else if (NYCPatternType.SUBWAY_TO_SI_FERRY.equals(metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.METROCARD_SUBWAY;
                else if (NYCPatternType.LOCAL_BUS_TO_SIR_TO_SI_FERRY.equals(
                        metrocardTransferSource))
                    metrocardTransferSource = NYCPatternType.LOCAL_BUS_TO_SIR;
            }

            // III.H. METRO-NORTH RAILROAD
            else if (NYCPatternType.METRO_NORTH_PEAK.equals(patternType)
                    || NYCPatternType.METRO_NORTH_OFFPEAK.equals(patternType)) {
                ...
                if (metroNorthBoardStop != -1) {
                    ...
                    if (thisRideDirection != metroNorthDirection
                            || thisRideLine != metroNorthLine) {
                        // we have changed direction or line. Pay for the previous ride, and reset
                        // the Metro-North transfer allowance
                        ...
                        if (overrideFares.containsKey(metroNorthBoardStop)
                            ...
                            mnrFare =
                                    overrideFares
                                            .get(metroNorthBoardStop)
                                            .get(metroNorthAlightStop);
                        } else {
                            mnrFare =
                                    fareData.getMetroNorthFare(
                                            metroNorthBoardStop,
                                            metroNorthAlightStop,
                                            metroNorthPeak);
                        }
                        ...
                        if (mnrDiscount > 0 && metroNorthBoardTime <= metrocardTransferExpiry) {
                            ...
                            if (mnrFare < 0) {
                                LOG.warn(
                                        "Discount on MNR is larger than MNR fare; assuming it's"
                                            + " free.");
                                mnrFare = 0;
                            }
                        } else {
                        ...
                        metroNorthAlightStop = alightStop;
                        metroNorthPeak |=
                                thisRidePeak; // any ride that involves a peak ride anywhere is a
                                              // peak fare.
                    }
                } else {
            ...
            // only allow pickups/dropoffs at the ferry terminal.
                ...
                // do nothing, but keep this here so we don't throw an unknown pattern type error
                // later
            }
            ...
            // TODO refactor to use pattern type
                ...
                if (lirrBoardStops == null) {
                    ...
                    if (lirrBoardStop == null)
                        throw new IllegalStateException(
                                "No LIRRStop found for transit layer stop " + boardStop);
                    if (lirrAlightStop == null)
                        throw new IllegalStateException(
                                "No LIRRStop found for transit layer stop " + alightStop);

                    lirrBoardStops.add(lirrBoardStop);
                    lirrBoardTimes.add(boardTime);
                    lirrAlightStops.add(lirrAlightStop);
                    lirrDirections.add(
                            LIRRTransferAllowance.LIRRDirection.forGtfsDirection(
                                    transitLayer.tripPatterns.get(pattern).directionId));
                    ...
                    if (boardStop != previousAlightStop) {
                        throw new IllegalStateException(
                                "LIRR transfer allowance preserved after on-street transfer!");
                    }
            ...
            // This has a special no-transfer fare, while other Westchester Bee-Line routes are just
            // MetroCard Local Buses
            // Don't touch transfer allowances here (although I suppose this might actually clear
            // you MetroCard transfer
            ...
            // III.M. AIRTRAIN JFK
            // Flat fare, don't touch transfer allowance. I suppose IRL this might clear the
            // transfer allowance since you
            // do pay with your MetroCard, but again, not documented
                ...
                // I assume that if you go from Howard Beach to Jamaica you pay twice... which is
                // why
                // these are separate ifs
                // (I'm also not sure you can get from Howard Beach to Jamaica without a transfer,
                // but if statements are cheap)
                if (fareData.airtrainJfkFareStops.contains(boardStop))
                    cumulativeFare += NYCStaticFareData.AIRTRAIN_JFK_FARE;
                if (fareData.airtrainJfkFareStops.contains(alightStop))
                    cumulativeFare += NYCStaticFareData.AIRTRAIN_JFK_FARE;
            } else {
        ...
        // IF WE HAVE AN ON-STREET TRANSFER, RECORD WHETHER WE'VE LEFT THE LIRR/METRO-NORTH/SUBWAY
        // FOR DOMINATION PURPOSES
        // don't consider a loop transfer to be an on-street transfer - the router will cunningly
        ...
        return new FareBounds(
                cumulativeFare,
                new NYCTransferAllowance(
                        lirr,
                        metrocardTransferSource,
                        metrocardTransferExpiry,
                        inSubwayPaidArea,
    ...
    public static boolean hasBehindGatesTransfer(int from, int to, NYCFareDataCache fareData) {
        if (from == to) return true; // same platform
            ...
            return previousFareArea != null
                    && thisFareArea != null
                    && previousFareArea == thisFareArea;
        }
    }
    ...
    private int getToLirrDiscountForMetrocardTransferSource(
