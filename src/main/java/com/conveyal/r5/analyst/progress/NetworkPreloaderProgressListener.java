...
/**
 * This implementaton of ProgressListener posts updates to the NetworkPreloader about progress on
 * preparing needed inputs for a particular request.
 */
public class NetworkPreloaderProgressListener implements ProgressListener {
    ...
    // Perhaps allow setting the logger field so log entries reflect the calling class instead of
    // ProgressListener.
    private static final Logger LOG =
            LoggerFactory.getLogger(NetworkPreloaderProgressListener.class);
    ...
    /**
     * Every how many work units we'll post an update. There is no point in doing this more often
     ...
     */
    public NetworkPreloaderProgressListener(
            NetworkPreloader networkPreloader, NetworkPreloader.Key key) {
    ...
    public void beginTask(String description, int totalElements) {
        ...
        // Integer division truncates downward. Add one to ensure nonzero update frequency (avoiding
        // divide by zero).
