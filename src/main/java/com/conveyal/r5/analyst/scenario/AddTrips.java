...
/**
 * Create a new trip pattern and add some trips to that pattern. The pattern may create new stops or
 * reuse existing ones from the base network. The newly created trips are true frequency trips, not
 * scheduled or exact_times frequencies.
 */
public class AddTrips extends Modification {
    ...
    /**
     * The timetables for this trip pattern. Note that these may be out of temporal order, it's up
     * to the consumer to get trips in order as required by the routing algorithm.
     */
    ...
    /**
     * If set to true, create both the forward pattern and a derived backward pattern as a matching
     * set.
     */
    ...
    public boolean resolve(TransportNetwork network) {
        if (stops == null || stops.size() < 2) {
            ...
            if (frequencies.isEmpty()) {
                errors.add(
                        "This modification should include at least one timetable/frequency entry.");
            }
    ...
    public boolean apply(TransportNetwork network) {
        ...
        // We will be creating a service for each supplied timetable, make a protective copy of the
        // list of services.
        transitLayer.services = new ArrayList<>(transitLayer.services);

        // TODO lots more to fill in here, need to have a way to just specify all needed info in
        // scenario editor.
        ...
        // No protective copy is made here because TransportNetwork.scenarioCopy already deep-copies
        // certain collections.
        ...
        if (bidirectional) {
            ...
            // Reverse the intStopIds in place. The string stopIds should not be used anymore at
            // this point.
            intStopIds.reverse();
            for (PatternTimetable ptt : frequencies) {
                // Reverse all the pattern timetables in place. Not sure how wise this is but it
                // works.
            ...
            generatePattern(
                    transitLayer,
                    this.directionId == 0 ? 1 : 0); // other direction ID for bidirection route
        }
        return false;
    }

    /**
     * This has been pulled out into a separate function so it can be called twice: once to generate
     * the forward pattern and once to generate the reverse pattern.
     *
     * @param transitLayer a protective copy of a transit layer whose existing tripPatterns list
     *     will be extended.
    ...
    private void generatePattern(TransitLayer transitLayer, int directionId) {
        ...
        for (PatternTimetable timetable : frequencies) {
            createSchedules(timetable, directionId, transitLayer.services)
                    .forEach(pattern::addTrip);
            if (timetable.firstDepartures == null) transitLayer.hasFrequencies = true;
            else transitLayer.hasSchedules = true;
        }
        // Ensure that generated trips are ordered as expected by R5, in case firstDepartures was
        // not ordered.
        // Validation in resolve() ensures there are at least two stops, so at least one departure
        // per trip.
        ...
        // We use the directionId method parameter rather than this.directionId. This allows two
        // patterns to be created with opposite directionIds when bidirectional=true.
    ...
    /**
     * A class representing a timetable from the incoming modification deserialized from JSON. TODO
     * rename to reflect usage in both AddTrips and AdjustFrequency.
     */
    public static class PatternTimetable implements Serializable {
        public static final long serialVersionUID = 1L;

        /**
         * The trip ID from which to copy travel and dwell times. Used only in AdjustFrequency
         * Modifications.
         */
        public String sourceTrip;

        /**
         * The travel times in seconds between adjacent stops. Used only in AddTrips, not
         * AdjustFrequency.
         */
        public int[] hopTimes;

        /**
         * The time in seconds the vehicle waits at each stop. Used only in AddTrips, not
         * AdjustFrequency.
         */
        ...
        /**
         * Times in seconds after midnight when trips will begin. If this field is non-null the
         * newly added trips will be scheduled trips rather than frequency-based, and you must not
         * provide the startTime, endTime, or headwaySecs fields. These departure times are not
         * necessarily sorted in ascending temporal order, it's up to the modification application
         * process to sort the resulting trip objects that are generated, as required for routing.
         */
        ...
        /**
         * ID for this frequency entry, so that it can be referred to for phasing purposes. May be
         * left null if there is no phasing from this entry.
         */
        ...
        /**
         * Sanity and range check the contents of this entry, considering the fact that
         * PatternTimetables may be used to create either frequency or scheduled trips, and they may
         * or may not include hop and dwell times depending on whether a new pattern is being
         * created.
         *
         * @param nStops the number of stops in the pattern to be generated, or -1 if a new pattern
         *     is not being generated and only schedule / frequency fields are to be validated, not
         *     hops and dwell times.
        ...
        protected List<String> validate(int nStops) {
            ...
            if (headwaySecs >= 0 || startTime >= 0 || endTime >= 0) {
                ...
                if (headwaySecs < 0 || startTime < 0 || endTime < 0) {
                    warnings.add(
                            "You must specify headwaySecs, startTime, and endTime together to"
                ...
                if (firstDepartures != null) {
                    warnings.add(
                            "You cannot specify firstDepartures (which creates scheduled trips)"
                                + " along with headwaySecs, startTime, or endTime (which create"
                                + " frequency trips");
                }
            } else {
                ...
                if (firstDepartures == null) {
                    warnings.add(
                            "You must specify either firstDepartures (which creates scheduled"
                                + " trips) or all of headwaySecs, startTime, and endTime (which"
                                + " create frequency trips");
                } else if (phaseFromTimetable != null
                        || phaseFromStop != null
                        || phaseSeconds != 0
                        || phaseAtStop != null) {
            ...
            if (phaseFromTimetable != null || phaseFromStop != null || phaseAtStop != null) {
                if (phaseFromTimetable == null || phaseFromStop == null || phaseAtStop == null) {
                    warnings.add(
                            "If one of phaseFromTimetable, phaseFromStop, phaseAtStop, or"
                                + " phaseSeconds is specified, all must be");
                }
            ...
            if (nStops >= 0) {
                ...
                if (hopTimes == null || hopTimes.length != nStops - 1) {
                    warnings.add(
                            "The number of hop times must be one less than the number of stops");
                }
            }
            return warnings;
        }
        ...
        /**
         * If this PatternTimetable contains phasing information, apply it to the given TripSchedule
         * destructively. Also set the frequency entry ID.
         */
        public void applyPhasing(TripSchedule sched) {
            ...
            if (firstDepartures == null) {
                ...
                sched.frequencyEntryIds =
                        new String[] {entryId != null ? entryId : UUID.randomUUID().toString()};
            }
        }
    }

    /**
     * Creates a gtfs-lib Service object based on the information in the given PatternTimetable,
     * which is usually part of a Modification deserialized from JSON.
     */
    public static Service createService(PatternTimetable timetable) {
        ...
        // TODO move this logic to a function on Calendar in gtfs-lib, e.g.
        // calendar.setFromDays(timetable.activeDaysOfWeek())
    ...
    /**
     * Creates an R5 TripSchedule object from a PatternTimetable object that was deserialized from
     * JSON. These represent a truly frequency-based family of trips (non-exact-times in GTFS
     * parlance). The supplied list of services will be extended with a new service, whose code will
     * be saved in the new TripSchedule. Make sure the supplied service list is a protective copy,
     * not the one from the original TransportNetwork!
     */
    public List<TripSchedule> createSchedules(
            PatternTimetable timetable, int directionId, List<Service> services) {
        ...
        // Convert the supplied hop and dwell times (which are relative to adjacent entries) to
        // arrival and departure
        // times (which are relative to the beginning of the trip or the beginning of the service
        // day).
        // Note that frequency here means "pure" unscheduled frequencies handled by Monte Carlo;
        // firstDepartures, like
        ...
        return IntStream.of(firstDepartures)
                .mapToObj(
                        firstDeparture -> {
                            ...
                            if (frequency) {
                                // Create an R5 frequency entry that will be attached to the new
                                // TripSchedule
