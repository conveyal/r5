...
/**
 * This modification selects all edges inside a given set of polygons and changes their
 * characteristics.
 */
public class ModifyStreets extends Modification {

    /**
     * A single multi-polygon, expressed as one array of [lat, lon] coordinates per polygon. All
     * edges entirely within the polygons will be selected. Later, we could add filtering of
     * selected edges, or selection of edges partially intersecting the polygons. We could
     * conceivably return the geometry of all the selected edges (up to a certain size) as an INFO
     * level field on the response.
     */
    public double[][][] polygons;

    /**
     * Overwrites the allowed modes for these streets with the given modes. The empty set of allowed
     * modes should be equivalent to removing the streets.
     */
    ...
    /**
     * After this modification is resolved against the TransportNetwork, this set will contain the
     * index numbers of all the edges that will be modified.
     */
    ...
    public boolean resolve(TransportNetwork network) {
        ...
        // Validate supplied polygon coordinates by loading them into JTS typed objects, ensuring
        // they look valid.
        ...
        for (double[][] polygon : polygons) {
            ...
            jtsPolygons.add(
                    geometryFactory.createPolygon(jtsCoordinates.toArray(new Coordinate[] {})));
        }
        MultiPolygon multiPolygon =
                geometryFactory.createMultiPolygon(jtsPolygons.toArray(new Polygon[] {}));
        ...
        Envelope fixedEnvelope =
                GeometryUtils.floatingWgsEnvelopeToFixed(multiPolygon.getEnvelopeInternal());
        ...
        candidateEdges.forEach(
                e -> {
                    ...
                    if (multiPolygon.intersects(edgeGeometryFloating)) {
                        // Mark for modification, which actually means marking it deleted and then
                        // making a new one.
        ...
        info.add(
                String.format(
                        "Will affect %d edges out of %d candidates.",
                        edgesInPolygon.size(), candidateEdges.size()));
    ...
    public boolean apply(TransportNetwork network) {
        ...
        // By convention we only index the forward edge in each pair, so we're iterating over
        // forward edges here.
        for (TIntIterator edgeIterator = edgesInPolygon.iterator(); edgeIterator.hasNext(); ) {
            ...
            // Our scenario EdgeStore cannot change existing edges, only delete them and add new
            // ones.
            // So first we mark all affected edges as deleted, then recreate them with new
            // characteristics.
    ...
    /**
     * Copy and modify the characteristics of a single edge in a pair. The shared characteristics of
     * the pair are copied / modified separately in the caller (the apply method). Our scenarios can
     * only extend existing edge lists, so modifying an edge is implemented as a soft delete + copy.
     */
    @JsonIgnore
