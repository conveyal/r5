...
public class RemoveTrips extends Modification {
    ...
    /**
     * The number of individual trips that were removed (not counting entire patterns). For logging
     * and error detection.
     */
    ...
    public boolean apply(TransportNetwork network) {
        ...
        if (routes != null) {
            ...
            transitLayer.tripPatterns =
                    transitLayer.tripPatterns.stream()
        ...
        LOG.info(
                "Removed {} entire patterns. Removed {} individual trips specified by ID.",
                nPatternsRemoved,
                nTripsRemoved);
    ...
    private TripPattern processPattern(TripPattern originalTripPattern) {
        ...
        newTripPattern.tripSchedules =
                originalTripPattern.tripSchedules.stream()
                        .filter(schedule -> !trips.contains(schedule.tripId))
                        .collect(Collectors.toList());
        nTripsRemoved +=
                originalTripPattern.tripSchedules.size() - newTripPattern.tripSchedules.size();
