...
public class DirectionUtils {
    ...
    static final double DIST = 10;

    // 180 degrees brads
    public static final byte m180 = radiansToBrads(Math.PI);

    /**
     * Returns the approximate azimuth from coordinate A to B in decimal degrees clockwise from
     * North, in the range (-PI to PI). The computation is exact for small delta between A and B.
     */
    public static double getAzimuth(Coordinate a, Coordinate b) {
        ...
        double x =
                (FastMath.cos(lat1) * FastMath.sin(lat2)
                        - FastMath.sin(lat1) * FastMath.cos(lat2) * FastMath.cos(dlng));
        if (FastMath.abs(x) < 1e-10 && FastMath.abs(y) < 1e-10) return Math.PI;
        double bearing = FastMath.atan2(y, x);
        return bearing; // (bearing < 0.0f) ? bearing + 360.0f: bearing;
    }

    /**
     * Computes the angle of the last segment of a LineString or MultiLineString in radians
     * clockwise from North in the range (-180, 180).
     *
        ...
        while (d < DIST && i >= 0) {
            seglength =
                    SphericalDistanceLibrary.fastDistance(
                            geometry.getCoordinateN(i), geometry.getCoordinateN(i + 1));
            if (d + seglength > DIST) {
    ...
    /**
     * Computes the angle of the first segment of a LineString or MultiLineString in radians
     * clockwise from North in the range (-PI, PI).
     *
     * @param geometry a LineString
     */
    ...
    /**
     * Converts binary radians to angle in degrees FIXME most of our operations can be done more
     * simply and efficiently directly in brads, exploiting overflow. A circle of 256 brads divides
     * into four sections of 64, with 32 on either side of a cardinal direction.
     */
