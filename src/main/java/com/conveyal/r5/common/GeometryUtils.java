...
public class GeometryUtils {
    ...
    /**
     * Haversine formula for distance on the sphere. We used to have a fastDistance function that
     * would estimate this quickly, but I'm not convinced we actually need it.
     *
     ...
     * (fixedLon1, fixedLat1) and return the fractional distance of the projected location along the
     * segment as a double in the range [0, 1]. All coordinates are fixed-precision geographic. Pass
     * in the cosine of the latitude to avoid repeatedly performing the expensive cosine operation.
     */
    ...
    /**
     * Given an envelope in fixed-point degrees, enlarge it by at least the given number of meters
     * in all directions. Intentionally slightly oversizes the rectangle by scaling longitudes based
     * on the latitude closest to the equator. Querying for objects inside the envelope will
     * overselect, which is a common characteristic of spatial indexes. You'll need to filter the
     * resulting set of selected objects using more accurate distances.
     */
    public static void expandEnvelopeFixed(Envelope envelope, double radiusMeters) {
        ...
        double floatingLat0 =
                fixedDegreesToFloating(
                        Math.min(Math.abs(envelope.getMaxY()), Math.abs(envelope.getMinY())));
        double yExpansion =
                VertexStore.floatingDegreesToFixed(
                        SphericalDistanceLibrary.metersToDegreesLatitude(radiusMeters));
        double xExpansion =
                VertexStore.floatingDegreesToFixed(
                        SphericalDistanceLibrary.metersToDegreesLongitude(
                                radiusMeters, floatingLat0));
