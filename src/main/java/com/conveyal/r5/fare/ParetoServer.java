...
public class ParetoServer {
    ...
    public String handle (Request req, Response res) throws IOException {
        ProfileRequest profileRequest = JsonUtils.objectMapper.readValue(req.body(), ProfileRequest.class);
        ...
        return JsonUtils.objectMapper.writeValueAsString(ret);
    }
                ...
                if (state.stop != -1 && (state.pattern != -1 || state.back != null)) {
                    if (state.pattern == -1) {
                        ...
                        LineString geom = GeometryUtils.geometryFactory.createLineString(new Coordinate[] {
                                new Coordinate(originStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR, originStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR),
                                new Coordinate(destStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR, destStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR),
                        });

                        legs.add(new ParetoTransferLeg(
                                originStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                originStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                destStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                destStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                geom,
                                originTime,
                                destTime,
                        ...
                        legs.add(new ParetoTransitLeg(network.transitLayer.routes.get(pattern.routeIndex),
                                ...
                                boardStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                boardStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                alightStopCoord.getY() / GeometryUtils.FIXED_DEGREES_FACTOR,
                                alightStopCoord.getX() / GeometryUtils.FIXED_DEGREES_FACTOR, shape, state.fare.transferAllowance));
                    }
