 ...
 *
 * <p>Each serialization or deserialization operation creates a completely new Kryo instance, so
 * there should be no issues with thread safety, as long as the object being serialized is not being
 * changed simultaneously.
 *
 * <p>Created by abyrd on 2018-11-08
 */
public abstract class KryoNetworkSerializer {
    ...
    /**
     * Set this to true to count instances and print a report including which serializer is handling
     * each class.
     */
    private static final boolean COUNT_CLASS_INSTANCES = false;

    /**
     * Factory method ensuring that we configure Kryo exactly the same way when saving and loading
     * networks, without duplicating code. We could explicitly register all classes in this method,
     * which would avoid writing out the class names the first time they are encountered and
     * guarantee that the desired serialization approach was used. Because these networks are so big
     * though, pre-registration should provide very little savings. Registration is more important
     * for small network messages.
     */
    private static Kryo makeKryo() {
        ...
        if (COUNT_CLASS_INSTANCES) {
            kryo =
                    new Kryo(
                            new InstanceCountingClassResolver(),
        ...
        // Handle references and loops in the object graph, do not repeatedly serialize the same
        // instance.
        kryo.setReferences(true);
        // Hash maps generally cannot be properly serialized just by serializing their fields.
        // Kryo's default serializers and instantiation strategies don't seem to deal well with
        // Trove primitive maps.
        // Certain Trove class hierarchies are Externalizable though, and define their own optimized
        // serialization
        // methods. addDefaultSerializer will create a serializer instance for any subclass of the
        // specified class.
        // The TPrimitiveHash hierarchy includes all the trove primitive-primitive and
        // primitive-Object implementations.
        kryo.addDefaultSerializer(TPrimitiveHash.class, ExternalizableSerializer.class);
        // We've got a custom serializer for primitive int array lists, because there are a lot of
        // them and the custom
        ...
        // The Kryo BitSet serializer in magro/kryo-serializers naively writes out a dense stream of
        // booleans.
        // BitSet's built-in Java serializer saves the internal bitfields, which is efficient. We
        // use that one.
        kryo.register(BitSet.class, new JavaSerializer());
        // Instantiation strategy: how should Kryo make new instances of objects when they are
        // deserialized?
        // The default strategy requires every class you serialize, even in your dependencies, to
        // have a zero-arg
        // constructor (which can be private). The setInstantiatorStrategy method completely
        // replaces that default
        // strategy. The nesting below specifies the Java approach as a fallback strategy to the
        // default strategy.
        kryo.setInstantiatorStrategy(
                new Kryo.DefaultInstantiatorStrategy(new SerializingInstantiatorStrategy()));
        return kryo;
    }
    ...
    /**
     * Read the given file and decode with Kryo into a new R5 TransportNetwork object. Transient
     * fields that were not serialized will be rebuilt (indexes and cached PointSet linkage).
     */
    public static TransportNetwork read(File file) throws Exception {
        ...
        LOG.info(
                "Loading {} file saved by R5 version {} commit {}",
                new String(header),
                version,
                commit);
        if (!BackendVersion.instance.version.equals(version)) {
            throw new RuntimeException(
                    String.format(
                            "File version %s is not compatible with this R5 version %s",
