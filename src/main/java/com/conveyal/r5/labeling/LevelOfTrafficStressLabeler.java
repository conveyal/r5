...
public class LevelOfTrafficStressLabeler {
    ...
    public void label (Way way, EnumSet<EdgeFlag> forwardFlags, EnumSet<EdgeFlag> backFlags) {
        ...
        if (ltsTagValue != null) {
            try {
                ...
                EdgeFlag ltsFlag = intToLts((int)lts);
        ...
        if (!forwardFlags.contains(EdgeFlag.ALLOWS_CAR) && !backFlags.contains(EdgeFlag.ALLOWS_CAR)) {
            // no cars permitted on this way, it is LTS 1
            // TODO on street bike lanes/cycletracks digitized as separate infrastructure?
            forwardFlags.add(EdgeFlag.BIKE_LTS_1);
            backFlags.add(EdgeFlag.BIKE_LTS_1);
            return;
        }
        ...
        EdgeFlag defaultLts = EdgeFlag.BIKE_LTS_3;

        // if it's small and slow, LTS 2
        if (lanes <= 3 && maxSpeed <= 25 * 1.61) defaultLts = EdgeFlag.BIKE_LTS_2;

        // assume that there aren't too many lanes if it's not specified
        if (lanes == Integer.MAX_VALUE && maxSpeed <= 25 * 1.61) defaultLts = EdgeFlag.BIKE_LTS_2;
        ...
        if (way.hasTag("highway", "unclassified") || way.hasTag("highway", "tertiary") || way.hasTag("highway", "tertiary_link")) {
            ...
            if (lanes <= 3 && Double.isNaN(maxSpeed)) defaultLts = EdgeFlag.BIKE_LTS_2;

            if (hasForwardLane) {
                forwardFlags.add(EdgeFlag.BIKE_LTS_2);
            }
            else {
            ...
            if (hasBackwardLane) {
                backFlags.add(EdgeFlag.BIKE_LTS_2);
            }
            else {
        ...
        if (!forwardFlags.contains(EdgeFlag.BIKE_LTS_1) && !forwardFlags.contains(EdgeFlag.BIKE_LTS_2) &&
                !forwardFlags.contains(EdgeFlag.BIKE_LTS_3) && !forwardFlags.contains(EdgeFlag.BIKE_LTS_4))
            forwardFlags.add(EdgeFlag.BIKE_LTS_4);

        if (!backFlags.contains(EdgeFlag.BIKE_LTS_1) && !backFlags.contains(EdgeFlag.BIKE_LTS_2) &&
                !backFlags.contains(EdgeFlag.BIKE_LTS_3) && !backFlags.contains(EdgeFlag.BIKE_LTS_4))
            backFlags.add(EdgeFlag.BIKE_LTS_4);
     }
    ...
    public void applyIntersectionCosts(StreetLayer streetLayer) {
        ...
        do {
            ...
            for (TIntIterator it = streetLayer.incomingEdges.get(v.index).iterator(); it.hasNext();) {
                e.seek(it.next());
                if (e.getFlag(EdgeFlag.BIKE_LTS_2)) maxLts = Math.max(2, maxLts);
                if (e.getFlag(EdgeFlag.BIKE_LTS_3)) maxLts = Math.max(3, maxLts);
                if (e.getFlag(EdgeFlag.BIKE_LTS_4)) maxLts = Math.max(4, maxLts);
            }
        ...
        for (TIntIntIterator it = vertexStresses.iterator(); it.hasNext();) {
            ...
            for (TIntIterator eit = streetLayer.incomingEdges.get(v.index).iterator(); eit.hasNext();) {
                ...
                int lts = it.value();
                if (e.getFlag(EdgeFlag.BIKE_LTS_2)) lts = Math.max(2, lts);
                if (e.getFlag(EdgeFlag.BIKE_LTS_3)) lts = Math.max(3, lts);
                if (e.getFlag(EdgeFlag.BIKE_LTS_4)) lts = Math.max(4, lts);

                // clear existing markings
                e.clearFlag(EdgeFlag.BIKE_LTS_1);
                e.clearFlag(EdgeFlag.BIKE_LTS_2);
                e.clearFlag(EdgeFlag.BIKE_LTS_3);
                e.clearFlag(EdgeFlag.BIKE_LTS_4);
    ...
    public static Integer ltsToInt (EdgeFlag flag) {
        switch (flag) {
    ...
    public static EdgeFlag intToLts (int lts) {
        if (lts < 2) return EdgeFlag.BIKE_LTS_1;
        else if (lts == 2) return EdgeFlag.BIKE_LTS_2;
        else if (lts == 3) return EdgeFlag.BIKE_LTS_3;
        else return EdgeFlag.BIKE_LTS_4;
    }
