...
/**
 * Assign traversal permissions to edges based on their tags in OpenStreetMap. see
 * https://wiki.openstreetmap.org/wiki/Computing_access_restrictions#Algorithm and also prior work
 * by Marko Burjek:
 ...
 *
 * <p>This class is abstract. You must make a country-specific subclass containing defaults for a
...
public abstract class TraversalPermissionLabeler {
    ...
    // This is immutable map of highway tag and boolean which is true if this tag has same road with
    // _link
    ...
    static {
        ...
        addPermissions(
                "footway|steps|platform|public_transport=platform|railway=platform|corridor",
                "access=no;foot=yes");
    }
    ...
    public RoadPermission getPermissions(Way way) {
        ...
        // check for one-way streets. Note that leaf nodes will always be labeled and there is no
        // unknown,
        ...
        if (dir.get(Node.FOOT) == OneWay.REVERSE)
        ...
        // but they need to be added also for example
        // highway=residential;bicycle=no;oneway=yes;bicycle:left=opposite_lane

        // If oneway is reversed (oneway=-1) opposite bicycle permission also need to be since they
        // are opposite rest of traffic
     ...
     *
     * <p>Removes wheelchair permissions on steps and adds it on wheelchair=yes or ramp:wheelchair
    ...
    /**
     * Adds Allows bike to backward direction if one of those is true: - cycleway= starts with
     * opposite (can be opposite_track or opposite_lane) - cycleway:left starts with opposite (can
     * be opposite_track or opposite_lane) - cycleway:right starts with opposite (can be
     * opposite_track or opposite_lane)
     *
     * @param way
     * @param backward
     */
     ...
     *
     * <p>AKA lane, track, shared_line
     *
     * @param way
     * @param forward
     * @param backward
     */
    private void applyDirectionalPermissions(
            Way way, EnumSet<EdgeStore.EdgeFlag> forward, EnumSet<EdgeStore.EdgeFlag> backward) {
        String cyclewayLeftTagValue = way.getTag("cycleway:left");
        String cyclewayRightTagValue = way.getTag("cycleway:right");
    ...
    private Label walk(EnumMap<Node, Label> tree, Node node) {
        do {
            ...
            if (tree.get(node) != Label.UNKNOWN) return tree.get(node);
    ...
    private void applySpecificPermissions(EnumMap<Node, Label> tree, Way way) {
        ...
        if (way.hasTag("access"))
            applyLabel(Node.ACCESS, Label.fromTag(way.getTag("access")), tree);
        ...
        if (way.hasTag("sidewalk")) {
            ...
            // For sidewalk direction doesn't matter since pedestrians can walk in both directions
            // on sidewalk
            if ("both".equalsIgnoreCase(sidewalk)
                    || "left".equalsIgnoreCase(sidewalk)
                    || "right".equalsIgnoreCase(sidewalk)) {
                applyLabel(Node.FOOT, Label.YES, tree);
            }
        }
        if (way.hasTag("vehicle"))
            applyLabel(Node.VEHICLE, Label.fromTag(way.getTag("vehicle")), tree);
        if (way.hasTag("bicycle"))
            applyLabel(Node.BICYCLE, Label.fromTag(way.getTag("bicycle")), tree);
        ...
        if (way.hasTag("cycleway:both"))
            applyLabel(Node.BICYCLE, Label.fromTag(way.getTag("cycleway:both")), tree);
        if (way.hasTag("motor_vehicle"))
            applyLabel(Node.CAR, Label.fromTag(way.getTag("motor_vehicle")), tree);
        // motorcar takes precedence over motor_vehicle
        if (way.hasTag("motorcar"))
            applyLabel(Node.CAR, Label.fromTag(way.getTag("motorcar")), tree);
    }
    ...
    protected <T> void applyLabel(Node node, T label, EnumMap<Node, T> tree) {
        if (label instanceof Label && label.equals(Label.UNKNOWN)) return;
    ...
    protected EnumMap<Node, Label> getTreeForWay(Way way) {
        ...
        if (highway != null) {
            ...
            EnumMap<Node, Label> defaultTree =
                    defaultPermissions.getOrDefault(
                            "highway=" + highway, defaultPermissions.get("highway=road"));
            tree.putAll(defaultTree);
            return tree;
        } else if (way.hasTag("railway", "platform")) {
            EnumMap<Node, Label> defaultTree =
                    defaultPermissions.getOrDefault(
                            "railway=platform", defaultPermissions.get("highway=road"));
            tree.putAll(defaultTree);
            return tree;
        } else if (way.hasTag("public_transport", "platform")) {
            EnumMap<Node, Label> defaultTree =
                    defaultPermissions.getOrDefault(
                            "public_transport=platform", defaultPermissions.get("highway=road"));
            tree.putAll(defaultTree);
            return tree;
        }
     ...
     *
     * <p>It splits tags on | and for each tag calls {@link #addPermission(String, String)}
     *
     * <p>This method is a bit stringly-typed since it's really accepting pipe- or
     * semicolon-separated lists crammed into Strings, but the tags are already being split on
     * equals as well. Look at the places where this method is called to see that this compromise
     * leads to more readable code.
     *
     * @param tags Tags are separated with | they can be specified with tag=value or just value
     *     where it is assumed that tag is highway.
     ...
     *
     * <p>It calls {@link #addTree(String, String)} once if highway doesn't have link counterpart.
    ...
    private static void addTree(String tag, String traversalPermissions) {
        EnumMap<Node, Label> currentTree =
                defaultPermissions.getOrDefault(tag, new EnumMap<>(Node.class));
        ...
        for (String permission : permissions) {
            ...
            if (currentLabel == Label.UNKNOWN) {
                throw new RuntimeException(
                        "permissions:" + permission + " invalid label for " + tag);
            }
    ...
    /**
     * Get a directional tree (for use when labeling one-way streets) where every node is labeled
     * bidirectional
     */
    private EnumMap<Node, OneWay> getDirectionalTree(Way way) {
        ...
        if (way.hasTag("oneway"))
            applyOnewayHierarchically(Node.ACCESS, OneWay.fromTag(way.getTag("oneway")), tree);
        if (way.hasTag("oneway:vehicle"))
            applyOnewayHierarchically(
                    Node.VEHICLE, OneWay.fromTag(way.getTag("oneway:vehicle")), tree);
        if (way.hasTag("oneway:motorcar"))
            applyOnewayHierarchically(
                    Node.CAR, OneWay.fromTag(way.getTag("oneway:motorcar")), tree);

        if (way.hasTag("oneway:bicycle"))
            applyOnewayHierarchically(
                    Node.BICYCLE, OneWay.fromTag(way.getTag("oneway:bicycle")), tree);
        ...
        if (way.hasTag("bicycle:backward")) {
            ...
            if (Label.fromTag(way.getTag("bicycle:backward")) == Label.NO) {
                if (sidepath == OneWay.REVERSE) {
                    // Seems it is sidepath in both direction meaning cycling is completely
                    // disallowed
                    LOG.error(
                            "Way has tags bicycle:forward=use_sidepath and"
                                + " bicycle:backward=use_sidepath please use bicycle=use_sidepath");
                }
                sidepath = OneWay.YES;
            }
        }
        ...
        // However there are there are some one way pedestrian paths, e.g. entrances with
        // turnstiles.
     ...
     * https://wiki.openstreetmap.org/wiki/Computing_access_restrictions#Transport_mode_hierarchy We
     * are using such a small subset of the tree on the wiki that we code up the parent/child
    ...
    protected enum Node {
        ...
        BICYCLE,
        CAR; // children of VEHICLE. CAR is shorthand for MOTOR_VEHICLE as we don't separately
             // support hazmat carriers, mopeds, etc.
    ...
    protected enum Label {
        ...
        private static boolean isNoThruTraffic(String access) {
            return "destination".equals(access)
                    ...
                    || "residents".equals(access)
                    || "resident".equals(access)
        ...
        public static Label fromTag(String tag) {
            ...
            if (isTagTrue(tag)
                    ...
                    || "unknown".equals(tag)
                    || "public".equals(tag)
                    || "permissive".equals(tag)
                    || "designated".equals(tag)
                    ...
                    // cycleway=crossing it should already have bicycle=yes from highway=cycleway or
                    // bicycle=yes
                    || "crossing".equals(tag)) {
                return YES;
            } else if (isTagFalse(tag)
                    || tag.equals("license")
                    || tag.equals("restricted")
                    || tag.equals("prohibited")
                    || tag.equals("emergency")
                    || "use_sidepath".equals(tag)
                    || "dismount".equals(tag)) {
                return NO;
    ...
    protected enum OneWay {
        ...
        public static OneWay fromTag(String tag) {
            ...
            if ("yes".equals(tag) || "true".equals(tag) || "1".equals(tag)) return YES;

            if ("-1".equals(tag) || "reverse".equals(tag)) return REVERSE;
