...
/**
 * This sets the flags on edges indicating what category of edge they are, e.g. stairs, bike path,
 * sidewalk.
 */
public class TypeOfEdgeLabeler {

    private boolean isCycleway(Way way, boolean back) {
        boolean bidirectionalCycleway =
                way.hasTag("highway", "cycleway")
                        || (way.hasTag("highway", "path")
                                && way.hasTag("bicycle", "designated")
                                && way.hasTag("foot", "designated"))
                        || way.hasTag("cycleway", "lane")
                        || way.hasTag("cycleway", "track");
        if (bidirectionalCycleway) {
            if (way.hasTag("oneway")) {
                if (TraversalPermissionLabeler.Label.fromTag(way.getTag("oneway"))
                        == TraversalPermissionLabeler.Label.YES) {
                    if (!back) {
                        return true;
                    } else {
                        return false;
        ...
        boolean has_cycleway_opposite =
                way.hasTag("cycleway", "opposite_lane") || way.hasTag("cycleway", "opposite_track");

        if (back) {
            String cycleway_left = way.getTag("cycleway:left");
            if (cycleway_left != null
                    && TraversalPermissionLabeler.Label.fromTag(cycleway_left)
                            == TraversalPermissionLabeler.Label.YES) {
                return true;
            }
            ...
            if (cycleway_right != null
                    && TraversalPermissionLabeler.Label.fromTag(cycleway_right)
                            == TraversalPermissionLabeler.Label.YES) {
                return true;
            }
    ...
    private boolean isSidewalk(Way way, boolean back) {
        ...
        if (way.hasTag("sidewalk")) {
            ...
            if (sidewalk.equals("both")) {
                return true;
            } else if (sidewalk.equals("none")
                    || sidewalk.equals("no")
                    || sidewalk.equals("false")) {
                return false;
            }
            if (!back) {
    ...
    /**
     * Adds Stairs, bike path, sidewalk and crossing flags to ways. This sets flags (passed in as
     * the second and third parameters) from the tags on the OSM Way (first parameter).
     */
    public void label(
            Way way,
        ...
        // Tunnels, covered roads and motorways are unlikely places for origins, destinations, or
        // park and rides.
