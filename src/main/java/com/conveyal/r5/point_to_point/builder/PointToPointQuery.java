...
public class PointToPointQuery {
    ...
    public ProfileResponse getPlan(ProfileRequest request) {
        ...
        if (request.hasTransit()) {
            ...
            Map<LegMode, TIntIntMap> accessTimes =
                    accessRouter.entrySet().stream()
                            .collect(
                                    Collectors.toMap(
                                            Map.Entry::getKey,
                                            e -> e.getValue().getReachedStops()));
            Map<LegMode, TIntIntMap> egressTimes =
                    egressRouter.entrySet().stream()
                            .collect(
                                    Collectors.toMap(
                                            Map.Entry::getKey,
                                            e -> e.getValue().getReachedStops()));
            ...
            /**
             * Orders first no transfers then one transfers 2 etc - then orders according to first
             * trip: - board stop - alight stop - alight time - same for one transfer trip
             */
            ...
            for (PathWithTimes path : usefullpathList) {
                ...
                if (LOG.isDebugEnabled()) {
                    ...
                    for (int i = 0; i < path.patterns.length; i++) {
                        // TransitSegment transitSegment = new
                        // TransitSegment(transportNetwork.transitLayer, path.boardStops[i],
                        // path.alightStops[i], path.patterns[i]);
                        if (!(((boardStop == path.boardStops[i]
                                && alightStop == path.alightStops[i])))) {
                            LOG.debug(
                        ...
                        TripPattern pattern =
                                transportNetwork.transitLayer.tripPatterns.get(path.patterns[i]);
                        if (pattern.routeIndex >= 0) {
                            RouteInfo routeInfo =
                                    transportNetwork.transitLayer.routes.get(pattern.routeIndex);
                            LOG.debug(
    ...
    private Map<LegMode, StreetRouter> findEgressPaths(ProfileRequest request) {
        ...
        for (LegMode mode : request.egressModes) {
            ...
            if (streetRouter.setOrigin(request.toLat, request.toLon)) {
                ...
                LOG.warn(
                        "MODE:{}, Edge near the origin coordinate wasn't found. Routing didn't"
                            + " start!",
                        mode);
            }
        }
        ...
        for (LegMode mode : request.directModes) {
            ...
            if (mode == LegMode.BICYCLE_RENT) {
                if (!transportNetwork.streetLayer.bikeSharing) {
                    LOG.warn(
                            "Bike sharing trip requested but no bike sharing stations in the"
                                + " streetlayer");
                    continue;
                }
                ...
                if (streetRouter != null) {
                    StreetRouter.State lastState =
                            streetRouter.getState(request.toLat, request.toLon);
                    if (lastState != null) {
                        streetPath =
                                new StreetPath(
                                        lastState,
                        ...
                        LOG.warn(
                                "MODE:{}, Edge near the destination coordinate wasn't found."
    ...
    private HashMap<LegMode, StreetRouter> findAccessPaths(ProfileRequest request) {
        ...
        for (LegMode mode : request.accessModes) {
            ...
            if (mode == LegMode.CAR_PARK) {
                streetRouter =
                        findParkRidePath(request, streetRouter, transportNetwork.transitLayer);
                if (streetRouter != null) {
                ...
                // Gets correct maxCar/Bike/Walk time in seconds for access leg based on mode since
     ...
     */
    public static StreetRouter findParkRidePath(
            ProfileRequest request, StreetRouter streetRouter, TransitLayer transitLayer) {
    ...
    /**
     * Combine the results of several street searches using different modes into a single map It
     * also saves with which mode was stop reached into stopModeMap. This map is then used to create
    ...
    private TIntIntMap combineMultimodalRoutingAccessTimes(
        ...
        for (Map.Entry<LegMode, StreetRouter> entry : routers.entrySet()) {
            ...
            router.getReachedStops()
                    .forEachEntry(
                            (stop, time) -> {
                                if (time > maxTimeFinal || time < minTimeFinal) return true;
                                // Skip stops that can't be used with wheelchairs if wheelchair
                                // routing is requested
                                if (request.wheelchair
                                        && !transportNetwork.transitLayer.stopsWheelchair.get(
                                                stop)) {
                                    return true;
                                }
                                ...
                                // There are penalties for using certain modes, to avoid bike/car
                                // trips that are only marginally faster
                                // than walking, so we use weights to decide which mode "wins" to
                                // access a particular stop.
        ...
        if (stopsFound > MAX_ACCESS_STOPS) {
            ...
            int cutoff =
                    timeList.get(
                            MAX_ACCESS_STOPS); // it needs to be same as MAX_ACCESS_STOPS since if
                                               // there are minimally MAX_ACCESS_STOPS + 1 stops the
                                               // indexes are from 0-MAX_ACCESS_STOPS
