 ...
 * JSON keys used.
 *
 * <p>These used to be command line parameters, but there were getting to be too many of them and
 * besides, we want to allow different graph builder configuration for each Graph.
 *
 * <p>TODO maybe have only one giant config file and just annotate the parameters to indicate which
...
public class TNBuilderConfig {
    ...
    /**
     * Generates nice HTML report of Graph errors/warnings (annotations). They are stored in the
     * same location as the graph.
     */
    public final boolean htmlAnnotations;

    /**
     * If number of annotations is larger then specified number annotations will be split in
     * multiple files. Since browsers have problems opening large HTML files.
     */
     ...
     * street. Perhaps this should be a runtime router parameter rather than a graph build
     * parameter.
     */
    ...
    /**
     * Path to bikeRental file currently only XML is support and file needs to be in same folder as
     * OSM files
     */
