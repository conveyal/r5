...
/**
 * Class used to represent transit paths for display to end users (and debugging). It is a group of
 * parallel arrays, with each position in the arrays representing a leg in the trip.
 *
 * <p>NOTE this has been superseded for most purposes by com.conveyal.r5.transit.path.Path
 */
@Deprecated
public class Path implements Cloneable {
    ...
    // FIXME assuming < 4 legs on each path, this parallel array implementation probably doesn't use
    // less memory than a List<Leg>.
    // It does effectively allow you to leave out the boardStopPositions and alightStopPositions,
    // but a subclass could do that too.
    public int[] patterns;
    ...
    /**
     * Used only in propagation for writing paths TODO update egress mode outside of path (e.g. in
     * wrapper or array parallel to perIterationTravelTimes in propagater)
     */
    ...
    public Path(RaptorState state, int stop) {
        ...
        while (state.previous != null) {
            // We copy the state at each stop from one round to the next. If a stop is not updated
            // in a particular
            // round, the information about how it was reached optimally will be found in a previous
            // round.
            ...
            checkState(
                    state.previous.bestNonTransferTimes[stop] >= state.bestNonTransferTimes[stop],
            ...
            // go to previous state before handling transfers as transfers are done at the end of a
            // round
            state = state.previous;
    ...
    @Override
    // We tried replacing this custom implementation with Objects.hash(...), but it did not produce
    // the expected
    ...
    public String toItineraryString(TransitLayer transitLayer) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // transitLayer.routes.get(transitLayer.tripPatterns.get(patterns[i]).routeIndex).route_short_name;
    ...
    public String[] toTripString(TransitLayer transitLayer) {
        ...
        for (int i = 0; i < length; i++) {
            var builder = new StringBuilder();
            RouteInfo route =
                    transitLayer.routes.get(transitLayer.tripPatterns.get(patterns[i]).routeIndex);
            builder.append(route.route_id)
                    ...
                    .append(" (")
                    .append(transitLayer.stopNames.get(boardStops[i]))
                    .append(") -> ")
                    .append(transitLayer.stopIdForIndex.get(alightStops[i]).split(":")[1])
                    .append(" (")
                    .append(transitLayer.stopNames.get(alightStops[i]))
                    .append(") alight ")
                    .append(
                            String.format(
                                    "%02d:%02d",
                                    Math.floorDiv(alightTimes[i], 3600),
                                    (int) (alightTimes[i] / 60.0 % 60)));
