...
/**
 * A path that also includes itineraries and bounds for all possible trips on the paths (even those
...
public class PathWithTimes extends Path {
    ...
    private void computeTimes(
        ...
        if (!accessTimes.containsKey(this.boardStops[0]))
            throw new IllegalArgumentException("Access times do not contain first stop of path!");
        if (!egressTimes.containsKey(this.alightStops[this.length - 1]))
        ...
        if (network.transitLayer.hasFrequencies) {
            ...
            throw new UnsupportedOperationException(
                    "Frequency-based trips are not yet supported in customer-facing profile"
                        + " routing");
        }
        ...
        TripPattern[] patterns =
                IntStream.of(this.patterns)
                        .mapToObj(p -> network.transitLayer.tripPatterns.get(p))
                        .toArray(s -> new TripPattern[s]);
        // find all possible times to board and alight each pattern
        // for each trip pattern, for each trip on that pattern, array of [depart origin, arrive
        // destination]
        ...
        for (int patIdx = 0; patIdx < patterns.length; patIdx++) {
            ...
            while (patterns[patIdx].stops[fromStopInPattern] != this.boardStops[pidx])
                fromStopInPattern++;
            int toStopInPattern = fromStopInPattern;
            while (patterns[patIdx].stops[toStopInPattern] != this.alightStops[pidx]) {
                // if we visit the board stop multiple times, board at the one closest to the alight
                // stop
                // TODO better handle duplicated stops/loop routes
                if (patterns[patIdx].stops[toStopInPattern] == this.boardStops[pidx])
                    fromStopInPattern = toStopInPattern;
                toStopInPattern++;
            }
            ...
            times[patIdx] =
                    patterns[patIdx].tripSchedules.stream()
                            .map(
                                    ts ->
                                            new int[] {
                                                ts.departures[finalFromStopInPattern],
                                                ts.arrivals[finalToStopInPattern]
                                            })
                            .toArray(s -> new int[s][]);
        }
        ...
        int firstTrip = 0;
        while (times[0][firstTrip][0]
                < req.fromTime + accessTime + FastRaptorWorker.BOARD_SLACK_SECONDS) firstTrip++;
        ...
        while (firstTrip < times[0].length) {
            ...
            for (int patIdx = 0; patIdx < this.patterns.length; patIdx++) {
                ...
                while (times[patIdx][trip][0] < time) {
                    trip++;
                    if (trip >= times[patIdx].length)
                        break
                                TIMES; // we've found the end of the times at which this path is
                                       // possible
                }
                ...
                if (patIdx < this.length - 1) {
                    ...
                    TIntList transfers =
                            network.transitLayer.transfersForStop.get(this.alightStops[patIdx]);
                    ...
                    if (this.alightStops[patIdx] != this.boardStops[patIdx + 1]) {
                        ...
                        if (transferTime == -1) {
                            throw new IllegalStateException(
                                    "Did not find transfer in transit network, indicates an"
                                        + " internal error");
                        }
                    } else
                        transferTime =
                                0; // no transfer time, we are at the same stop (board slack applied
                                   // below)

                    // TODO should board slack be applied at the origin stop? Is this done in
                    // RaptorWorker?
                    // See also below in computeStatistics
                    time =
                            times[patIdx][trip][1]
                                    + transferTime
                                    + FastRaptorWorker.BOARD_SLACK_SECONDS;
    ...
    /**
     * filter out itineraries that are dominated by other itineraries on this path (e.g. when the
     * first vehicle runs every 10 minutes, and the second every 60, no reason to include all
     * possible trips on the first vehicle).
     *
     * <p>Has the same effect as reverse optimization in classic OTP.
     */
    private void sortAndFilterItineraries() {
        ...
        for (Iterator<Itinerary> it = itineraries.iterator(); it.hasNext(); ) {
            ...
            // if the previously found itinerary arrives at the same time, don't save this one as it
            // is dominated
            if (prev != null
                    && prev.alightTimes[this.length - 1] == current.alightTimes[this.length - 1])
                it.remove();
            prev = current;
        }
    ...
    public static class Itinerary implements Comparable<Itinerary> {
        ...
        /**
         * The time you arrive at the board stop before each pattern, left 0 on the first leg as
         * it's undefined
         */
    ...
    // FIXME we are using a map with unorthodox definitions of hashcode and equals to make them
    // serve as map keys.
    // We should instead wrap PathWithTimes or copy the relevant fields into a PatternSequenceKey
    // class.
    ...
    public String dump(TransportNetwork network) {
        ...
        sb.append(
                String.format(
                        "min %d/avg %d/max %d %s ",
                        stats.min / 60, stats.avg / 60, stats.max / 60, accessMode.toString()));

        String[] routes =
                IntStream.of(patterns)
                        .mapToObj(
                                p -> {
                                    int routeIdx =
                                            network.transitLayer.tripPatterns.get(p).routeIndex;
                                    return String.format(
                                            "%s %d",
                                            network.transitLayer.routes.get(routeIdx)
                                                    .route_short_name,
                                            p);
                                })
