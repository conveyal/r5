...
import static com.conveyal.r5.rastercost.ElevationCostField.ELEVATION_SAMPLE_SPACING_METERS;
import static com.conveyal.r5.scenario.RasterCost.CostFunction.MINETTI;
import static com.conveyal.r5.scenario.RasterCost.CostFunction.SUN;
import static com.conveyal.r5.scenario.RasterCost.CostFunction.TOBLER;
...
public class RasterCost extends Modification {
    ...
    private RasterDataSourceSampler rasterSampler;
    ...
    public boolean prefetchFiles (FileStorage fileStorage) {
        try {
            ...
            File dataSourceFile = fileStorage.getFile(fileStorageKey);
            if (costFunction == SUN) {
                rasterSampler = new RasterDataSourceSampler(dataSourceFile, 1.0, false);
            } else {
                rasterSampler = new RasterDataSourceSampler(dataSourceFile, ELEVATION_SAMPLE_SPACING_METERS, false);
            }
        } catch (Exception e) {
    ...
    public boolean resolve (TransportNetwork network) {
        if (costFunction == TOBLER) {
            loader = new ElevationLoader(rasterSampler, new ToblerCalculator());
        } else if (costFunction == MINETTI) {
            loader = new ElevationLoader(rasterSampler, new MinettiCalculator());
        } else if (costFunction == SUN) {
            loader = new SunLoader(rasterSampler);
        }
        if (loader == null) {
