...
/**
 * This encapsulates the original/default turning and edge traversal cost calculations that were in
 * the street router. It uses hard-wired values for the amount of time it takes to complete a turn.
 */
public class BasicTraversalTimeCalculator implements TraversalTimeCalculator {
    ...
    // These are all specified for drive-on-right countries, and reversed in drive-on-left
    // countries.
    // TODO re-express as "turn against traffic" and "turn with traffic" to make this less
    // confusing.
    ...
    public boolean
            driveOnRight; // TODO instead of a field, this should be a different implementation
                          // class
    ...
    /**
     * Quantify the difficulty of turning from the fromEdge onto the toEdge with the given
     * streetMode of transport. Quantities should be time-like with units of seconds or perceived
     * seconds. TODO pull this out into an interface to allow generalization to data from
     * generalized cost tags
     */
    @Override
    public int turnTimeSeconds(int fromEdge, int toEdge, StreetMode streetMode) {
        if (streetMode == StreetMode.CAR) {
            ...
            if (angle < 27) return STRAIGHT_ON;
            else if (angle < 153) return driveOnRight ? LEFT_TURN : RIGHT_TURN;
            else if (angle < 207) return U_TURN;
            else if (angle < 333) return driveOnRight ? RIGHT_TURN : LEFT_TURN;
            else return STRAIGHT_ON;
        }
        return 0;
    }
    ...
    /**
     * Compute the angle between two edges, positive, from 0 to 2 * Pi. The angle is
     * _counterclockwise_ because that's how it's done in JTS.
     */
    public double computeAngle(int fromEdge, int toEdge) {
        ...
        // figure out the angle. All below computations in radians. Add 2 * pi so everything is
        // positive.
        ...
        while (distanceSoFar < 10 && coordIndex < coords.length) {
            distanceSoFar +=
                    GeometryUtils.distance(
                            coords[coordIndex - 1].x,
                            coords[coordIndex - 1].y,
                            coords[coordIndex].x,
                            coords[coordIndex].y);
            coordIndex++;
        }
