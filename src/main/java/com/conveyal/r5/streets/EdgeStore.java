...
/**
 * This stores all the characteristics of the edges in the street graph layer of the transport
 * network. The naive way to do this is making one object per edge, and storing all those edges in
 * one big list or in outgoing and incoming edge lists in each vertex. However, representing the
 * street graph as an object graph has some disadvantages, including devoting a large amount of
 * space to pointers, making serialization more complex, and possible memory locality effects on
 * speed.
 *
 * <p>Originally we considered using a library that would simulate compound value types (C structs).
 * A column store with parallel arrays is better than a struct simulation because: 1. it is less
 * fancy, 2. it is auto-resizing (not fixed size), 3. I've tried both and they're the same speed.
 *
 * <p>This problem will be solved when Java JEP 169 (value/inline types) finally becomes part of the
 * language, especially when inline types are available in generic types like ArrayList. The "record
 * types" now in JVM preview would improve readability and maintainability but are still reference
 * classes, so not worth the migration.
 *
 * <p>Edges come in pairs that have the same origin and destination vertices and the same
 * geometries, but reversed. Therefore many of the arrays are only half as big as the number of
 * edges. All even numbered edges are forward, all odd numbered edges are reversed.
 *
 * <p>Typically, somewhat more than half of street segment edges have intermediate points (ie. their
 * geometry contains points other than the two endpoints). Therefore it's more efficient to add a
 * complete dense column for references to intermediate point arrays, instead of using a sparse
 * HashMap to store values only for those edges that have intermediate points.
 *
 * <p>For geometry storage I tried several methods. Full Netherlands load in 4GB of heap: Build time
 * is around 8 minutes. 2.5-3GB was actually in use. List of int arrays: 246MB serialized, 5.2 sec
 * write, 6.3 sec read. List of int arrays, full lists (not only intermediates): 261MB, 6.1 sec
 * write, 6.2 sec read. Indexes into single contiguous int array: 259MB, 5 sec write, 7.5 sec read.
...
public class EdgeStore implements Serializable {
    ...
    /**
     * CAR speeds, one speed for each edge. Separate entries for forward and backward edges. rounded
     * (m/s * 100) (2 decimal places) Saved speed is mostly the same as speed saved as m/s * 1000 it
     * differs in second decimal place and is 0.024% smaller This way of saving speeds is 3.5%
     * smaller then previous (saving 7 decimal places)
     */
    public TShortList speeds;

    /**
     * The index of the origin vertex of each edge pair in the forward direction. One entry for each
     * edge pair.
     */
    public TIntList fromVertices;

    /**
     * The index of the destination vertex of each edge pair in the forward direction. One entry for
     * each edge pair.
     */
    ...
    /**
     * Geometries. One entry for each edge pair. These are packed lists of lat, lon, lat, lon... as
     * fixed-point integers, and don't include the endpoints (i.e. don't include the intersection
     * vertices, only intermediate points).
     */
    public List<int[]> geometries;

    /**
     * The compass angle at the start of the edge geometry (binary radians clockwise from North).
     * Internal representation is -180 to +179 integer degrees mapped to -128 to +127 (brads) One
     * entry for each edge pair.
     */
    ...
    /**
     * When applying scenarios, we don't duplicate the entire set of edges and vertices. We extend
     * them, treating the baseline graph as immutable. We have to be careful not to change or delete
     * any elements of that baseline graph which is shared between all threads. All edges at or
     * above the index firstModifiableEdge can be modified, all lower indexes should be treated as
     * immutable. All edges from firstModifiableEdge up to nEdges() are temporary and are in a
     * separate spatial index. This field also serves as an indication that a scenario is being
     * applied and this EdgeStore is a protective copy, whenever firstModifiableEdge &gt; 0.
     */
    public int firstModifiableEdge = 0;

    /**
     * When applying a scenario, we can't touch the baseline graph which is shared between all
     * threads. Whenever we must delete one of these immutable edges (e.g. when splitting a road to
     * connect a new stop) we instead record its index in temporarilyDeletedEdges, meaning it should
     * be ignored as if it did not exist in this thread. TODO: ignore these edges when routing, not
     * only during spatial index queries.
     */
    public TIntSet temporarilyDeletedEdges = null;

    /**
     * This method will tell you whether a scenario has been applied to this EdgeStore, i.e. whether
     * its lists have been extended. There's one case where this method will fail: using Scenarios
     * to create street networks from a blank slate, where there are no edges before the scenario is
     * applied.
     *
     * @return true if this EdgeStore has already been extended, and can therefore be modified
     *     without affecting the baseline graph shared between all threads.
     */
    public boolean isExtendOnlyCopy() {
        // FIXME this definition will fail if we ever allow scenrios on top of completely empty
        // street networks.
    ...
    /**
     * Stores and retrieves per-edge costs, either specified in input data or derived from standard
     * OSM tags. For now this may be null, indicating that no per-edge times are available and
     ...
     */
    public static final transient EnumSet<EdgeFlag> PERMISSION_FLAGS =
            EnumSet.of(EdgeFlag.ALLOWS_PEDESTRIAN, EdgeFlag.ALLOWS_BIKE, EdgeFlag.ALLOWS_CAR);

    /**
     * When we create new edges, we must assign them OSM IDs. By convention user-generated OSM IDs
     * not in the main OSM database are negative. FIXME it's weird to store a positive ID, increment
     * it in a positive direction, but always negate it before using it.
     */
    ...
    public EdgeStore(VertexStore vertexStore, StreetLayer layer, int initialSize) {
        ...
        // There are separate flags and speeds entries for the forward and backward edges in each
        // pair.
        flags = new TIntArrayList(initialSize);
        speeds = new TShortArrayList(initialSize);
        // Vertex indices, geometries, and lengths are shared between pairs of forward and backward
        // edges.
    ...
    /**
     * Edge flags are various boolean values (requiring a single bit of information) that can be
     * attached to each edge in the street graph. They are each assigned a bit number from 0...31 so
     * they can all be packed into a single integer field for space and speed reasons.
     */
    public enum EdgeFlag {

        // Street categories.
        // FIXME some street categories are mutually exclusive and should not be flags, just narrow
        // numbers.
        // This could be done with static utility functions that write the low N bits of a number
        // into a given position.
        // Maybe reserve the first 4-5 bits (or a whole byte, and 16 bits for flags) for mutually
        // exclusive edge types.
        UNUSED(
                0), // This flag is deprecated and currently unused. Use it for something new and
                    // interesting!
        BIKE_PATH(1),
        SIDEWALK(2),
        CROSSING(3),
        ...
        SLOPE_OVERRIDE(13),
        /**
         * An edge that links a transit stop to the street network; two such edges should not be
        ...
        // If this flag is present, the edge is good idea to use for linking. Excludes runnels,
        // motorways, and covered roads.
        LINKABLE(20),

        // The highest five bits of the flags field represent the bicycle level of traffic stress
        // (LTS) for this street.
        // See
        // http://transweb.sjsu.edu/PDFs/research/1005-low-stress-bicycling-network-connectivity.pdf
        // The comments below on the LTS flag for each level 1 through 4 are pasted from that
        // document.
        // FIXME bicycle LTS should not really be flags, the categories are mutually exclusive and
        // can be stored in 2 bits.

        /**
         * If this flag is set, then its LTS has been loaded from OSM tags and we will not apply
         * further processing to infer it. We jump to flag number 27 here to group all five LTS
         * flags together at the high end of the 32-bit integer flags field.
         */
         ...
         * operating space outside the zone into which car doors are opened. Intersections are easy
         ...
         * through lane and a rightturn lane, it is configured to give cyclists unambiguous priority
         * where cars cross the bike lane and to keep car speed in the right-turn lane comparable to
         ...
         * moderately low speed. Crossings may be longer or across higher-speed roads than allowed
         * by LTS 2, but are still considered acceptably safe to most adult pedestrians.
         */
        BIKE_LTS_3(30),

        /**
         * A level of stress beyond LTS3. (this is in fact the official definition. -Ed.) Also known
         * as FLORIDA_AVENUE.
         */
        BIKE_LTS_4(31);

        /**
         * In each enum value this field should contain an integer with only a single bit switched
         * on (a power of two).
         */
    ...
    /**
     * This creates the bare topological edge pair with a length. Flags, detailed geometry, etc.
     * must be set subsequently using an edge cursor. This avoids having a tangle of different edge
     * creator functions for different circumstances.
     *
     * @return a cursor pointing to the forward edge in the pair, which always has an even index.
     */
    public Edge addStreetPair(
            int beginVertexIndex, int endVertexIndex, int edgeLengthMillimeters, long osmID) {
        // The first new edge to be created will have an index equal to the number of existing
        // edges.
        int forwardEdgeIndex = nEdges();
        // The caller supplies an OSM ID less than 0 to indicate that the edges being created are
        // not derived
        ...
        if (beginVertexIndex < 0 || beginVertexIndex >= vertexStore.getVertexCount()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Attempt to begin edge pair at nonexistent vertex %s",
                            beginVertexIndex));
        }
        ...
        if (endVertexIndex < 0 || endVertexIndex >= vertexStore.getVertexCount()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Attempt to end edge pair at nonexistent vertex %s", endVertexIndex));
        }
        ...
        // angles needs to be calculated here since some edges don't get additional geometry (P+R,
        // transit, etc.)
    ...
    void startTurnRestriction(StreetMode streetMode, boolean reverseSearch, StreetRouter.State s1) {
        if (reverseSearch) {
            ...
            // Turn restrictions only apply to cars for now. This is also coded in canTurnFrom, so
    ...
    /**
     * Inner class that serves as a cursor: points to a single edge in this store, and can be moved
     * to other indexes. TODO make this a separate class so the outer class reference is explicit
     * (useful in copy functions)
     */
    public class Edge {
         ...
         *
         * @return true if we have not advanced past the end of the list (there is an edge at the
         *     new position).
         */
        ...
        public void seek(int pos) {
            ...
            if (pos >= nEdges())
                throw new ArrayIndexOutOfBoundsException(
                        "Attempt to seek beyond end of edge store");
        ...
        /**
         * This function should not exist. This is a hack until we stop using an inner class and
         * it's possible to access the outer EdgeStore field from Edge functions.
         */
        ...
        /**
         * This is inefficient, it's just a stopgap until we refactor to eliminate the need for Sets
         * of EdgeFlag
         */
        ...
        /**
         * @return the car speed on this edge, taking live traffic updates into account if requested
        ...
        // This is really ugly, we should store speeds in mm/sec or something. There is an
        // outstanding PR for this.
        ...
        /**
         * Copy the flags and speeds from the supplied Edge cursor into this one. Doesn't copy OSM
         * ids since those are set when edge is created. This is a hack and should be done some
         * other way.
         */
        ...
        /**
         * Calculate the speed appropriately given the ProfileRequest and traverseMode and the
         * current wall clock time. Note: this is not strictly symmetrical, because in a forward
         * search we get the speed based on the time we enter this edge, whereas in a reverse search
         * we get the speed based on the time we exit the edge.
         *
         * <p>If driving speed is based on max edge speed. (or from traffic if traffic is supported)
         *
         * <p>Otherwise speed is based on wanted walking, cycling speed provided in ProfileRequest.
         */
        public float calculateSpeed(ProfileRequest options, StreetMode traverseStreetMode) {
            if (traverseStreetMode == null) {
                // Do we really want to do this? Why not just let the NPE happen if the parameter is
                // missing?
                return Float.NaN;
        ...
        // TODO move most or all of this logic into the router to avoid passing search state like
        // req and timeCalculator
        public StreetRouter.State traverse(
            ...
            // A new instance representing the state after traversing this edge. Fields will be
            // filled in later.
            ...
            // We allow two links in a row if this is a first state (negative back edge or no
            // backState
            // Since at least P+R stations are connected to graph with only LINK edges and otherwise
            // search doesn't work
            // Since backEdges are set from first part of multipart P+R search
            if ((s0.backEdge >= 0)
                    && (s0.backState != null)
                    && getFlag(EdgeFlag.LINK)
                    && getCursor(s0.backEdge).getFlag(EdgeFlag.LINK)) {
            ...
            if (streetMode == StreetMode.WALK) {
                ...
                // If biking is not allowed on this edge, or if the traffic stress is too high, walk
                // the bike.
                ...
                if (req.bikeTrafficStress > 0 && req.bikeTrafficStress < 4) {
                    ...
                    if (req.bikeTrafficStress < 3 && getFlag(EdgeFlag.BIKE_LTS_3))
                        tryWalking = true;
                    if (req.bikeTrafficStress < 2 && getFlag(EdgeFlag.BIKE_LTS_2))
                        tryWalking = true;
                }
                if (tryWalking) {
                    if (!getFlag(EdgeFlag.ALLOWS_PEDESTRIAN)) {
            ...
            // Negative backEdge means this state is not the result of traversing an edge (it's the
            // start of a search).
            if (s0.backEdge >= 0) {
                if (req.reverseSearch) {
                    turnTimeSeconds =
                            timeCalculator.turnTimeSeconds(getEdgeIndex(), s0.backEdge, streetMode);
                } else {
                    turnTimeSeconds =
                            timeCalculator.turnTimeSeconds(s0.backEdge, getEdgeIndex(), streetMode);
                }
            }
            ...
            // Make sure we don't have states that don't increment weight/time, otherwise we could
            // create loops.
            // This should not happen since we always round upward (ceil function), perhaps replace
            // with assertions.
        ...
        /**
         * Can we turn onto this edge from this state? Also copies still-applicable restrictions
         * forward.
         */
        public boolean canTurnFrom(
                StreetRouter.State s0, StreetRouter.State s1, boolean reverseSearch) {
            // Turn restrictions only apply to cars for now. This is also coded in traverse, so
            // change it both places
            // if/when it gets changed.
            if (s0.turnRestrictions != null && s0.streetMode == StreetMode.CAR) {
                ...
                for (TIntIntIterator it = s1.turnRestrictions.iterator(); it.hasNext(); ) {
                    ...
                    if (reverseSearch) {
                        // In reverse search order of from/to and viaEdges is changed since we
                        // search from toEdge to fromEdge
        ...
        /**
         * Cut the packed array of intermediate coordinates in two at the specified segment index,
         * non-destructively (i.e. returning copies). The original array of the edge's intermediate
         * coordinates does not include the first and last points of the edge (which are supplied by
         * vertices), so the segment at index N ends at intermediate point N, which occupies array
         * indexes N2 and N2+1 in the packed array. Intermediate points before the split point
         * (which is on the specified segment), ending with the beginning of the split segment,
         * should be in the first return array. Intermediate points after the split point, starting
         * with the end of the split segment, should be in the second return array. The first or
         * second return array can be empty if we are splitting at the first or last segment
         * respectively, or if there are no intermediate coordinates.
         *
         * @return Pair with element a being int[] of intermediate coordinates before split, and
         *     element b being int[] of intermediate coordinates after split.
         */
        ...
        public void setGeometry(List<Node> nodes) {
            ...
            if (nodes.size() <= 2) {
                ...
                // Angles also need to be calculated here since when splitting edges some edges
                // currently loose geometry
         ...
         *
         * <p>Angles are saved as binary radians clockwise from North. 0 is 0 -180° = -128 180° =
         * -180° = -128 178° = 127
         *
         * <p>First and last angles are calculated between first/last and point that is at least 10
         * m from it according to line distance
         *
         * @see DirectionUtils
         *     <p>TODO: calculate angles without converting to the Linestring and back
         */
        ...
        public int getOutAngle() {
            ...
            if (isBackward()) {
                angle =
                        DirectionUtils.bradsToDegree(
                                (byte) (inAngles.get(pairIndex) - DirectionUtils.m180));
                return angle;
            } else {
        ...
        public int getInAngle() {
            ...
            if (isBackward()) {
                angle =
                        DirectionUtils.bradsToDegree(
                                (byte) (outAngles.get(pairIndex) - DirectionUtils.m180));
                return angle;
            } else {
        ...
        /**
         * Returns LineString geometry of edge Uses from/to vertices for first/last node and nodes
         * from geometries for middle nodes
         *
         * <p>TODO: it might be better idea to return just list of coordinates
         *
         * @return
         */
        public LineString getGeometry() {
            ...
            int size =
                    coords == EMPTY_INT_ARRAY
                            ? 2
                            :
                            // division with two since coordinates are in same array saved as lat,
                            // lon,lat etc.
                            (coords.length / 2) + 2;
            ...
            // NB it initially appears that we are reversing the from and to vertex twice, but keep
            // in mind that
            // getFromVertex returns the from vertex of the edge _pair_ rather than that of a
            // particular edge.
            // This is the from vertex when we are on a forward edge, and the to vertex when we are
            // on a back edge.
            ...
            if (coords != null) {
                for (int i = 1; i < c.length - 1; i++) {
                    ...
                    c[i] =
                            new Coordinate(
                                    ilon / VertexStore.FIXED_FACTOR,
                                    ilat / VertexStore.FIXED_FACTOR);
                }
            }
        ...
        /**
         * Call a function on every segment in this edge's geometry. Always iterates forward over
         * the geometry, whether we are on a forward or backward edge.
         */
        public void forEachSegment(SegmentConsumer segmentConsumer) {
            ...
            segmentConsumer.consumeSegment(
                    s, prevFixedLat, prevFixedLon, vertex.getFixedLat(), vertex.getFixedLon());
        }

        /**
         * Call a function for every point on this edge's geometry, including the beginning end end
         * points. Always iterates forward over the geometry, whether we are on a forward or
         * backward edge.
         */
        ...
        public String toString() {
            String base =
                    String.format(
                            "Edge from %d to %d. Length %f meters, speed %f kph.",
         ...
         *
         * <p>It looks like walk,bike,car or none. If any mode of transport is not allowed it is
         * missing in return value.
         *
         * @return
         */
        ...
        /**
         * @return whether this edge may be modified. It may not be modified if it is part of a
         *     baseline graph that has been extended by a scenario. It may be modified if it's part
         *     of a baseline graph in the process of being built, or if it was created as part of
        ...
        /**
         * Set the flags for all on-street modes of transportation to "true", so that any mode can
         * traverse this edge.
         */
        ...
        /**
         * Set the flags for all on-street modes of transportation to "false", so no modes can
         * traverse this edge.
         */
        ...
        /**
         * Set flags on the current edge allowing traversals by searches with any of the given
         * streetModes enabled.
         */
        ...
        /**
         * Set a flag on the current edge allowing traversals by searches with the given streetMode
         * enabled.
         */
    ...
    /**
     * @return an Edge object that can be moved around in the EdgeStore to get a view of any edge in
     *     the graph. This object is not pointing at any edge upon creation, so you'll need to call
     *     seek() on it to select an edge.
     */
    ...
    public static interface SegmentConsumer {
        public void consumeSegment(
                int index, int fixedLat0, int fixedLon0, int fixedLat1, int fixedLon1);
    }
    ...
    /**
     * @return the total number of individual directed edges in the graph, including temporary ones
     *     from Scenarios. Forward and backward edges in pairs are counted separately.
     */
    public int nEdges() {
        return flags.size();
    }
    ...
    /**
     * @return the number of forward+backward edge pairs in the graph, including temporary ones from
     *     applied Scenarios. This should always be always nEdges/2.
     */
    ...
    /**
     * Returns a semi-deep copy of this EdgeStore for use when applying Scenarios. Mutable objects
     * and collections will be cloned, but their contents will not. The lists containing the edge
     * characteristics will be copied in such a way that all threads applying Scenarios will share
     * the same baseline data, and only extend those lists. We don't use clone() and make sure every
     * field copy is explicit below, avoiding any unintentional shallow-copying of collections or
    ...
    public EdgeStore extendOnlyCopy(StreetLayer copiedStreetLayer) {
        ...
        // The Edge store references a vertex store, and the StreetLayer should also hold the same
        // reference.
        // So the StreetLayer that makes this copy needs to grab a pointer to the new extend only
        // VertexStore
        copy.vertexStore = vertexStore.extendOnlyCopy();
        copy.flags = new TIntAugmentedList(flags);
        // This is a deep copy, we should do an extend-copy but need a new class for that. Can we
        // just use ints?
    ...
    /**
     * If this EdgeStore has has a Scenario applied, it may contain temporary edges that are not in
     * the baseline network. The edges added temporarily by a Scenario should always be the numbers
     * from firstModifiableEdge to nEdges. Call the supplied function with the index number of every
     * such temporarily added edge in this EdgeStore.
     */
    ...
    /**
     * If this EdgeStore has has a Scenario applied, some edges in the baseline network may be
     * "removed" by hiding them. Call the supplied function with the index number of every such
     * temporarily deleted edge in this EdgeStore.
     */
    public void forEachTemporarilyDeletedEdge(IntConsumer consumer) {
        if (this.isExtendOnlyCopy()) {
            temporarilyDeletedEdges.forEach(
                    edge -> {
                        consumer.accept(edge);
                        return true;
                    });
    ...
    /**
     * Call the supplied function on the index number of every temporarily added edge in this
     * EdgeStore, then on every temporarily deleted edge. This is a convenience function, equivalent
     * to calling forEachTemporarilyAddedEdge followed by forEachTemporarilyDeletedEdge.
     */
