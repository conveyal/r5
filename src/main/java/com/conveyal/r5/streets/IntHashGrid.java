...
/**
 * A spatial index using a 2D fast long hashtable from the Trove library. This is a modified version
 ...
 *
 * <p>Objects to index are placed in all grid bins touching the bounding envelope. We *do not store*
 * any bounding envelope for each object: we will therefore return false positives when querying,
 * and it's up to the caller to filter them out (with whatever knowledge it has on the location of
 * the object).
 *
 * <p>Note: For performance reasons, write operations are not synchronized, synchronization must be
 * handled by the caller. Read-only operations are thread-safe though.
 *
...
public class IntHashGrid implements Serializable {
    ...
    /**
     * Insert a linestring into the index. NB: the line string uses real-world float coordinates,
     * not fixed coordinates. This function keeps long and angular line strings from winding up in
     * many unnecessary cells by inserting each segment individually and splitting long segments
     * into pieces.
     *
     * <p>We could use a rasterization algorithm, but just splitting the line segments up into
     * manageable pieces works as well and is easier to follow, at the expense of slower insert
    ...
    public final void insert(LineString geom, final int item) {
        ...
        for (int i = 0; i < coord.length - 1; i++) {
            ...
            if (segments > 1000 || segments < 0)
                LOG.warn(
                        "Huge number of segments ({}) for edge, or possible int overflow)",
                        segments);
            ...
            for (int s = 0; s < segments; s++) {
                ...
                Coordinate c1 =
                        new Coordinate(
                                VertexStore.floatingDegreesToFixed(
                                        coord[i].x + dX * segFrac * (s + 1)),
                                VertexStore.floatingDegreesToFixed(
                                        coord[i].y + dY * segFrac * (s + 1)));
    ...
    /**
     * The spatial index can and will return false positives, but should not produce false
     * negatives. We return the unfiltered results including false positives. That is, this
     * overselects and MUST BE FILTERED.
     *
    ...
    public String toString() {
        return String.format(
                "IntHashGrid %d x %d, %d bins allocated, %d objs, %d entries (avg %.2f entries/bin,"
                    + " %.2f entries/object)",
                this.xBinSize,
                this.yBinSize,
                this.nBins,
