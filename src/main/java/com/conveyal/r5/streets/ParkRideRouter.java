...
public class ParkRideRouter extends StreetRouter {
    ...
    /**
     * From map of found P+Rs creates: - map of transitStopIndex duration which is used in
     * getReachedStops for raptor transit routing - map of streetVertexTransitStop P+R - to know
     * from which P+R we got to this stop
     *
     * <p>If stop appears multiple times, stop with shortest access time (time to get to P+R +
     * switch time + to stop) is saved
     *
     * @param carParks
    ...
    public void addParks(TIntObjectMap<State> carParks, TransitLayer transitLayer) {
        ...
        carParks.forEachValue(
                (state) -> {
                    int parkRideStreetVertexIdx = state.vertex;
                    int timeToParkRide = state.getDurationSeconds();
                    TIntObjectMap<State> parkRideTransitStops =
                            parkRideLocationsMap.get(parkRideStreetVertexIdx).closestTransfers;
                    // for each transit stop reached from this P+R
                    parkRideTransitStops.forEachEntry(
                            (toStop, pr_state) -> {
                                int distanceMillimeters = pr_state.distance;
                                int stopStreetVertexIdx =
                                        transitLayer.streetVertexForStop.get(toStop);
                                int timeToStop =
                                        (int) (distanceMillimeters / walkSpeedMillimetersPerSecond);
                                int totalTime =
                                        timeToParkRide
                                                + timeToStop
                                                + PointToPointQuery.CAR_PARK_DROPOFF_TIME_S;
                                // Adds time to to get to this stop and saves from which P+R we get
                                // from STOP
                                ...
                                if (!transitStopIndexDurationMap.containsKey(toStop)) {
                                    ...
                                    transitStopStreetIndexParkRideState.put(
                                            stopStreetVertexIdx, pr_state);
                                    streetVertexDurationMap.put(stopStreetVertexIdx, totalTime);
                                    // ELSE we only add time and update P+R if new time is shorter
                                    // then previously saved one
                                } else {
    ...
    /**
     * This uses stops found in {@link StopVisitor} if transitStopSearch is true and DOESN'T search
     * in found states for stops
     *
     * @return a map from transit stop indexes to their distances from the origin (or whatever the
     *     dominance variable is). Note that the TransitLayer contains all the information about
     *     which street vertices are transit stops.
     */
    @Override
     ...
     *
     * <p>Gets path from P+R to requested stop. (Path was found during graph build stage)
     *
     * @param vertexIndex
     */
    @Override
    public State getStateAtVertex(int vertexIndex) {
        // TODO: calculate correct distance and duration since currently only walk part has distance
        // and duration added
    ...
    /**
     * Returns travel time to this vertex. Only returns time to stops, since only those times are
     * saved
     *
     * @param vertexIndex
     * @return
