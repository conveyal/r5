...
/**
 * This stores edge characteristics that affect the perceived difficulty or danger of traversing
 * edges. From these characteristics, we can produce costs per edge in units of subjective-seconds
 * (effective time), or in units of subjective-meters (effective distance) if they must be scaled by
 * speeds. Costs can be pre-calculated where they don't need to be scaled by speeds.
 *
 * <p>Currently this requires special tags in the OSM input data, which must be generated by the
 * data provider using external scripts. This mechanism could eventually be extended, with R5
 * deriving these characteristics from standard OSM tags that are used in a wider range of places.
 *
 * <p>Input data has forward/backward tags, because OSM ways represent both directions of a road. R5
 * edges are directed, so all costs are "forward".
 *
 * <p>Instances of this class are for a single mode of travel; several instances can be used for
...
public class SingleModeTraversalTimes implements Serializable {
    ...
    // The effective length of the edge in meters, accounting for slope, which will be scaled by
    // speed.
    // This assumes a linear response to speed at increasing slopes, which is probably not accurate.
    // Eventually
    // fixed time-like costs that should not be scaled by speed (e.g. due to danger) should be
    // handled separately.
    // TODO slow down stairs, update above comment after final decision to store multipliers or
    // perceived lengths
    TDoubleList perceivedLengthMultipliers = new TDoubleArrayList();

    // The expected wait times to turn left or right or go straight through the intersection after
    // the edge.
    // These are stored as times because they are not scaled by user-specified speed so are
    // effectively pre-divided.
    ...
    public void summarize(String prefix) {
        LOG.info(
                "{} length multipliers: {}",
                prefix,
                Stats.of(perceivedLengthMultipliers.toArray()));
    ...
    /**
     * Interface for classes that calculate generalized costs for a single edge. This could be a
     * data class rather than an interface if we were not using the flyweight / column-store
     * pattern. TODO data class rather than interface, we're just instantiating zillions of
     * "providers" instead of Records.
     */
    public interface Supplier {
        /**
         * @return the factor by which the edge length should be scaled to account for slopes and
         *     other impedance when walking.
         */
        double perceivedLengthMultipler();
        /**
         * @return the expected amount of time it will take to make the given turn _after_
    ...
    public void setOneEdge(int edge, Supplier supplier) {
        // TODO could pre-multiply by edgeLengthMm passed in from caller, which makes street
        // modifications a little trickier
        setOneEdge(
                edge,
    ...
    /**
     * The generalized cost formulae provided to us are in terms of perceived distance. We want our
     * output to be in perceived time. When making this conversion, speed settings should affect
     * certain costs that are perceived relative to the length of the road; other costs like turn
     * costs should be converted to time using a standardized speed because they will not decrease
     * for a faster walker / biker. For the time being though, we return only distance units which
     * are scaled by the speed in the
     *
     * <p>Note that this method matches the implicit interface of TurnCostCalculator. Perhaps
     * GeneralizedCosts can be split out into a custom TurnCostCalculator and WeightCalculator. TODO
     * check toEdge vs. fromEdge parameter order, considering forward/reverse searches
     *
     * <p>Note that TurnEffectiveTime and TraversalEffectiveDistance could be precomputed to reduce
     * runtime computation. This would also allow setting them directly from tags on the input data,
     * or in scenario modifications. Those two array fields would not clutter the EdgeStore
     * significantly, and could be nulled when not present. Also note that some of these are not
     * effective/perceived times but real clock times - e.g. for left turns. Therefore we may want
     * to have an interface that computes turnTimes as well as turnCosts; and an interface that
     * computes edge traversalTimes as well as traversalCosts (which may be length scaling factors
     * or constants).
     */
    public int turnTimeSeconds(int fromEdge, int toEdge) {
        // Only find nonzero turn cost if there is a previous edge (we are not on the first edge
        // transition)
        // and if that previous edge appears within this generalized cost table.
        if (fromEdge < 0 || fromEdge >= this.size()) {
            return 0;
        }
        // This seems inefficient, we could fetch the angles without cursors (e.g.
        // edgeStore.angleBetweenEdges()).
        EdgeStore.Edge e = edgeStore.getCursor();
        e.seek(toEdge);
        int inAngle = e.getInAngle();
        e.seek(fromEdge);
        int outAngle = e.getOutAngle();
        // The degrees conversions are inefficient, we should be using brads (2^8 per rotation)
        // directly
    ...
    private double perceivedTraversalMeters(EdgeStore.Edge edge) {
        ...
        if (edge.edgeIndex < this.size()) {
            // This is performing a lot of extra multiplication. The perceived distances could be
            // premultiplied, but
            // that would require somehow getting the edge lengths into the GeneralizedCosts factory
            // methods.
    ...
    /**
     * Copy traversal time multiplier and turn times from an old edge to a new one. The multiplier
     * will be overwritten with the supplied one if the parameter is non-null; For use only on
     * scenario StreetLayers.
     */
