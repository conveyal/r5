...
/**
 * Represents a potential split point along an existing edge, retaining some geometric calculation
 * state so that once the best candidate is found more detailed calculations can continue.
 */
public class Split {
    ...
    // We must use a long because squaring a typical search radius in fixed-point _does_ cause
    // signed int32 overflow.
    public long distanceToEdge_squaredFixedDegrees =
            Long.MAX_VALUE; // squared distance from given point to the split, in degrees
    ...
    /**
     * Accumulated distance from the beginning vertex of the edge geometry up to the split point
     * (point on the edge closest to the point to be linked)
     */
    public int distance0_mm = 0;

    /**
     * Accumulated distance from the end vertex of the edge geometry up to the split point (point on
     * the edge closest to the point to be linked)
     */
    ...
    /**
     * Copy all the fields in another Split into this one. This avoids creating large amounts of
     * tiny short-lived objects. Does not copy distanceToEdge_mm, because this is only set at the
     * very end of the operation, on the winning Split.
     */
    ...
    private static GeodeticCalculator distanceCalculator =
    ...
    public static Split find(
        ...
        // After this conversion, the entire geometric calculation is happening in fixed precision
        // int degrees.
        ...
        // Use longs for radii and their square because squaring the fixed-point radius _will_
        // overflow a signed int32.
        long radiusFixedLat =
        ...
        candidateEdges.forEach(
                e -> {
                    ...
                    // Do not consider linking to edges that are links to streets from transit
                    // stops, P+Rs, and bike shares.
                    // These edges allow all modes to traverse, but may be connected to roads with
                    // more restrictive permissions.
                    // On a given edge pair both directions will have the same flag.
                    if (edge.getFlag(EdgeStore.EdgeFlag.LINK)) return true;

                    // If either direction of the current edge doesn't allow the specified mode of
                    // travel, skip it.
                    // It is arguably better to skip it only if BOTH directions forbid the specified
                    // mode (see commented block
                    // below). This system has odd effects in areas with lots of one-way streets or
                    // divided roads.
                    // TODO Really, we want to allow linking to two different edge-pairs in such
                    // cases but that is more complex.
                    // Do not consider linking to edges that are not marked "linkable". This
                    // excludes e.g. tunnels and motorways.
                    if (!edge.allowsStreetMode(streetMode)
                            || !edge.getFlag(EdgeStore.EdgeFlag.LINKABLE)) {
                        return true;
                    }
                    edge.advance();
                    if (!edge.allowsStreetMode(streetMode)
                    ...
                    // The distance to this edge is the distance to the closest segment of its
                    // geometry.
                    edge.forEachSegment(
                            (seg, fixedLat0, fixedLon0, fixedLat1, fixedLon1) -> {
                                ...
                                curr.frac =
                                        GeometryUtils.segmentFraction(
                                                fixedLon0, fixedLat0, fixedLon1, fixedLat1,
                                                fixedLon, fixedLat, cosLat);
                                // Project to get the closest point on the segment.
                                // Note: the fraction is scaleless, xScale is accounted for in the
                                // segmentFraction function.
                                curr.fixedLon =
                                        (int) (fixedLon0 + curr.frac * (fixedLon1 - fixedLon0));
                                curr.fixedLat =
                                        (int) (fixedLat0 + curr.frac * (fixedLat1 - fixedLat0));
                                // Find squared distance to edge (avoid taking square root, which is
                                // slow)
                                ...
                                if (curr.distanceToEdge_squaredFixedDegrees
                                        < squaredRadiusFixedLat) {
                                    if (curr.distanceToEdge_squaredFixedDegrees
                                            < best.distanceToEdge_squaredFixedDegrees) {
                                        // Update the best segment if we've found something closer.
                                        best.setFrom(curr);
                                    } else if (curr.distanceToEdge_squaredFixedDegrees
                                                    == best.distanceToEdge_squaredFixedDegrees
                                            && curr.edge < best.edge) {
                                        // Break distance ties by favoring lower edge IDs. This
                                        // makes destination linking
                                        // deterministic where centroids are equidistant to edges
                                        // (see issue #159).
                                        best.setFrom(curr);
                                    }
                                }
        ...
        // We found an edge. Iterate over its segments again, accumulating distances along its
        // geometry.
        // The distance calculations involve square roots so are deferred to happen here, only on
        // the selected edge.
        ...
        edge.forEachSegment(
                (seg, fLat0, fLon0, fLat1, fLon1) -> {
                    ...
                    // lengthAfter should be total length minus lengthBefore, which ensures splits
                    // do not change total lengths.
                    if (seg <= best.seg) {
        ...
        // FIXME perhaps we should be using the sphericalDistanceLibrary here, or the other way
        // around.
        // The initial edge lengths are set using that library on OSM node coordinates, and they are
        // slightly different.
        // We are using a single cosLat value at the linking point, instead of a different value at
        // each segment.
        ...
        if (best.distance0_mm > edge.getLengthMm()) {
            // This mistake happens because the linear distance calculation we're using comes out
            // longer than the
            // spherical distance. The graph remains coherent because we force the two split edge
            // lengths to add up
        ...
        // To speed up computation above, square roots were avoided and
        // distanceToEdge_squaredFixedDegrees was
        // calculated using fixed point degrees. We now want to calculate the distance in
        // millimeters, for routing.
        // To do so, we take the square root of distanceToEdge_squaredFixedDegrees, convert to
        // floating point degrees
        // latitude then multiply by the metersPerDegreeLat factor above and 1000 to convert to
        // millimeters.
        ...
        double distanceToEdge_floatingDegrees =
                VertexStore.fixedDegreesToFloating(distanceToEdge_fixedDegrees);
        best.distanceToEdge_mm = (int) (distanceToEdge_floatingDegrees * metersPerDegreeLat * 1000);
        return best;
    }

    /**
     * Find a split on a particular edge. FIXME this contains way too much duplicate code. We can
     * reuse the code in Split.find(). we just need a way to supply an edge or edges, instead of
    ...
    public static Split findOnEdge(double lat, double lon, EdgeStore.Edge edge) {
        ...
        edge.forEachSegment(
                (seg, fixedLat0, fixedLon0, fixedLat1, fixedLon1) -> {
                    ...
                    curr.frac =
                            GeometryUtils.segmentFraction(
                                    fixedLon0, fixedLat0, fixedLon1, fixedLat1, fixedLon, fixedLat,
                                    cosLat);
                    // Project to get the closest point on the segment.
                    // Note: the fraction is scaleless, xScale is accounted for in the
                    // segmentFraction function.
                    ...
                    curr.distance0_mm =
                            (int)
                                    ((lengthBefore_fixedDeg[0] + length * curr.frac)
                                            * metersPerDegreeLat
                                            * 1000);
