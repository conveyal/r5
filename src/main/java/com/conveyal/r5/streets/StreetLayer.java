 ...
 *
 * <p>Is is currently implemented as a column store.
 *
 * <p>We could also use something like https://github.com/RichardWarburton/slab which simulates Java
 * objects in a chunk of memory that could be mapped to a file. This could be useful for routing
 * across continents or large countries.
 *
 * <p>While the transit search is very fast (probably because it tends to search in order over
 * contiguous arrays) the street searches can be surprisingly slow. We suspect this is due to the
 * vertices being in somewhat random order in memory. The solution would be to order the vertices in
 * memory according to their proximity in the graph, then sort edges according to from-vertex order.
 *
 * <p>Really what you want to do is embed the distance metric defined by the graph in 1D space. This
 * is a 'metric embedding' or multidimensional scaling, analagous to force-directed graph layout.
 * http://ceur-ws.org/Vol-733/paper_pacher.pdf You could do something similar using their geographic
 * coordinates (Morton-code-sort the vertices).
 */
public class StreetLayer implements Serializable, Cloneable {
    ...
    /**
     * Minimum allowable size (in number of vertices) for a disconnected subgraph; subgraphs smaller
     * than these will be removed. There are several reasons why one might have a disconnected
     * subgraph. The most common is poor quality OSM data. However, they also could be due to areas
     * that really are disconnected in the street graph, and are connected only by transit. These
     * could be literal islands (Vashon Island near Seattle comes to mind), or islands that are
     * isolated by infrastructure (for example, airport terminals reachable only by transit or
     * driving, for instance BWI or SFO).
     */
    public static final int MIN_SUBGRAPH_SIZE = 40;

    /**
     * The radius below which we will not split a street, and will instead connect to an existing
     * intersection. i.e. if the requested split point is less than this distance from an existing
     * vertex (edge endpoint) we'll just return that existing endpoint.
     */
    private static final int SNAP_RADIUS_MM = 5 * 1000;

    /**
     * The radius of a circle in meters within which to search for nearby streets. This should not
     * necessarily be a constant, but even if it's made settable it should be stored in a field on
     * this class to avoid cluttering method signatures. Generally you'd set this once at startup
     * and always use the same value afterward. 1.6km is really far to walk off a street. But some
     * places have offices in the middle of big parking lots.
     */
    public static final double LINK_RADIUS_METERS = 1600;

    /**
     * Searching for streets takes a fair amount of computation, and the number of streets examined
     * grows roughly as the square of the radius. In most cases, the closest street is close to the
     * search center point. If the specified search radius exceeds this value, a mini-search will
     * first be conducted to check for close-by streets before examining every street within the
     * full specified search radius.
     */
    public static final int INITIAL_LINK_RADIUS_METERS = 300;

    // Edge lists should be constructed after the fact from edges. This minimizes serialized size
    // too.
    ...
    /**
     * Spatial index of temporary edges from a scenario. We used to not have this, and we used to
     * return all temporarily added edges in every spatial index query (because spatial indexes are
     * allowed to over-select, and are filtered). However, we now create scenarios with thousands of
     * temporary edges (from thousands of added transit stops), so we keep two spatial indexes, one
     * for the baseline network and one for the scenario additions.
     */
    private transient IntHashGrid temporaryEdgeIndex;

    // Key is street vertex index, value is BikeRentalStation (with name, number of bikes, spaces id
    // etc.)
    public TIntObjectMap<BikeRentalStation> bikeRentalStationMap;
    public TIntObjectMap<ParkRideParking> parkRideLocationsMap;

    // TODO these are only needed when building the network, should we really be keeping them here
    // in the layer?
    ...
    private transient TraversalPermissionLabeler permissionLabeler =
            new USTraversalPermissionLabeler();
    ...
    /**
     * Turn restrictions can potentially affect (include) several edges, so they are stored here and
     * referenced by index within all edges that are affected by them. TODO what if an edge is
     * affected by multiple restrictions?
     */
    public List<TurnRestriction> turnRestrictions = new ArrayList<>();

    /**
     * The TransportNetwork containing this StreetLayer. This link up the object tree also allows us
     * to access the TransitLayer associated with this StreetLayer of the same TransportNetwork
     * without maintaining bidirectional references between the two layers.
     */
    public TransportNetwork parentNetwork = null;

    /**
     * A string uniquely identifying the contents of this StreetLayer among all StreetLayers. When
     * no scenario has been applied, this field will contain the ID of the enclosing
     * TransportNetwork. When a scenario has modified this StreetLayer, this field will be changed
     * to that scenario's ID. We need a way to know what information is in the network independent
     * of object identity, which is lost in a round trip through serialization. This also allows
     * re-using cached linkages for several scenarios as long as they don't modify the street
     * network.
     */
    public String scenarioId;

    /**
     * Some StreetLayers are created by applying a scenario to an existing StreetLayer. All the
     * contents of the base StreetLayer are not copied, they are wrapped to make them extensible.
     * These are called "scenario copies". If this StreetLayer is such a scenario copy, this field
     * points to the original StreetLayer it was based upon. Otherwise this field should be null.
     */
     ...
     * Negative wait times mean the service is not available at a particular location. If this
     * reference is null, no wait time is applied. Note that this is a single field, rather than a
     * collection: we only support one set of polygons for one mode.
     */
    ...
    public static final EnumSet<EdgeStore.EdgeFlag> ALL_PERMISSIONS =
            EnumSet.of(
                    ...
                    EdgeStore.EdgeFlag.NO_THRU_TRAFFIC_BIKE,
                    EdgeStore.EdgeFlag.NO_THRU_TRAFFIC_PEDESTRIAN,
     ...
     *
     * <p>Routable ways are highways (unless they are raceways or highway rest_area/services since
     * those are similar to landuse tags Or public_transport platform or railway platform unless its
     * usage tag is tourism
     *
     * <p>In both cases roads need to exists in reality aka don't have: - construction, - proposed,
     * - removed, - abandoned - unbuilt tags
     *
     * <p>Both construction tagging schemes are supported tag construction=anything and
     * highway/cycleway=construction same with proposed.
     *
     * @param way
     * @return
     */
    private static boolean isWayRoutable(Way way) {
        ...
        (way.hasTag("highway")
                        ...
                        // Those two are areas which are places around highway (similar to landuse
                        // tags they aren't routable)
                        highway.equals("services")
                                || highway.equals("rest_area")
                                // highway=conveyor is obsoleted tag for escalator and is actually
                                // routable
    ...
    private static boolean actuallyExistsInReality(String highway, Way way) {
        return !("construction".equals(highway)
                || "abandoned".equals(highway)
                || "removed".equals(highway)
                || "proposed".equals(highway)
                || "propossed".equals(highway)
                || "unbuilt".equals(highway)
                || way.hasTag("construction")
                || way.hasTag("proposed"));
    }
    ...
    void loadFromOsm(OSM osm, boolean removeIslands, boolean saveVertexIndex) {
        ...
        // TEMPORARY HACK: create a GeneralizedCosts object to hold costs from preprocessed OSM
        // data, and indicate that
        ...
        for (Map.Entry<Long, Way> entry : osm.ways.entrySet()) {
            ...
            int nEdgesCreated = 0;
            int beginIdx = 0;
            // Break each OSM way into topological segments between intersections, and make one edge
            // pair per segment.
        ...
        LOG.info(
                "Made {} vertices and {} edges.", vertexStore.getVertexCount(), edgeStore.nEdges());
        ...
        if (removeIslands) {
            ...
            // due to bike walking, walk must go before bike, see comment in TarjanIslandPruner
            // javadoc
        ...
        // TODO handle multipolygon relations that are park and rides (e.g. parking structures with
        // holes)
        osm.relations.entrySet().stream()
                .filter(e -> e.getValue().hasTag("type", "restriction"))
                .forEach(e -> this.applyTurnRestriction(e.getKey(), e.getValue()));
     ...
     * <p>It uses OSM Mapdb
     *
     * <p>Uses {@link #getName(long, Locale)}
     *
     ...
     * Gets way name from OSM name tag
     *
     * <p>TODO: generate name on unnamed ways (sidewalks, cycleways etc.)
     *
     ...
     *
     * <p>Tags are returned as tag=value separated with ;
     *
     * <p>AKA same format that {@link Way#setTagsFromString(String)} accepts
     *
    ...
    public String getWayTags(EdgeStore.Edge edge) {
        ...
        if (way != null && !way.hasNoTags()) {
            return way.tags.stream().map(OSMEntity.Tag::toString).collect(Collectors.joining(";"));
        }
        return null;
    }
    ...
    private void buildParkAndRideAreas(List<Way> parkAndRideWays) {
        ...
        for (Way way : parkAndRideWays) {
            ...
            ParkRideParking parkRideParking =
                    new ParkRideParking(centerVertex, centroid.y, centroid.x, way);
            ...
            nearbyEdges.forEach(
                    eidx -> {
                        ...
                        // Check every edge found by the spatial index (which may overselect),
                        // looking for ones that
                        // intersect the road.
                        // The intersection of a park and ride linestring and an edge should be one
                        // or more points.
                        // Potentially a line if the road is running along the edge of the parking
                        // lot.
                                ...
                                if (single instanceof Point) {
                                    ...
                                    if (singleCoords.length > 0) {
                                        ...
                                        if (singleCoords.length > 1) {
                                            connectParkAndRide(
                                                    centerVertex, coords[coords.length - 1], e);
                                        }
                                    }
                                }
                            }
    ...
    private void buildParkAndRideNodes(List<Node> nodes) {
        ...
        for (Node node : nodes) {
            ...
            ParkRideParking parkRideParking =
                    new ParkRideParking(vidx, node.getLat(), node.getLon(), node);
            parkRideLocationsMap.put(vidx, parkRideParking);

            int targetWalking =
                    getOrCreateVertexNear(node.getLat(), node.getLon(), StreetMode.WALK);
            if (targetWalking == -1) {
                LOG.warn(
                        "Could not link park and ride node at ({}, {}) to the street network.",
                        node.getLat(),
                        node.getLon());
                continue;
            }
            ...
            // If both CAR and WALK links would connect to the same edge we can skip new useless
            // edge
    ...
    private void connectParkAndRide(int centerVertex, Coordinate coord, EdgeStore.Edge edge) {
        ...
        EdgeStore.Edge created =
                edgeStore.addStreetPair(
                        centerVertex,
                        targetVertex,
                        1,
                        -1); // basically free to enter/leave P&R for now.
        // allow link edges to be traversed by all, access is controlled by connected edges
        created.allowAllModes();
        created.setFlag(EdgeStore.EdgeFlag.LINK);
    ...
    /**
     * Given a turn restriction relation from OSM, find the affected edges in our street layer and
     * create a turn restriction object to store this information. The restriction is added to the
     * network and associated with the edges it affects, so the method need not return anything.
     *
    ...
    private void applyTurnRestriction(long osmRelationId, Relation restrictionRelation) {
        ...
        if (restrictionRelation.getTag("restriction").startsWith("no_")) only = false;
            ...
            LOG.error(
                    "Restriction {} has invalid restriction tag {}, skipping",
                    osmRelationId,
                    restrictionRelation.getTag("restriction"));
            return;
        }
        ...
        for (Relation.Member member : restrictionRelation.members) {
            if ("from".equals(member.role)) {
                if (from != null) {
                    LOG.error(
                            "Turn restriction {} has multiple 'from' members, skipping.",
                            osmRelationId);
                    return;
                }
                if (member.type != OSMEntity.Type.WAY) {
                    LOG.error(
                            "Turn restriction {} has a 'from' member that is not a way, skipping.",
                            osmRelationId);
                    return;
                }
                from = member;
                ...
                if (to != null) {
                    LOG.error(
                            "Turn restriction {} has multiple 'to' members, skipping.",
                            osmRelationId);
                    return;
                }
                if (member.type != OSMEntity.Type.WAY) {
                    LOG.error(
                            "Turn restriction {} has a 'to' member that is not a way, skipping.",
                            osmRelationId);
                    return;
                }
                to = member;
            ...
            // Osmosis may produce situations where referential integrity is violated, probably at
            // the edge of the
            // bounding box where half a turn restriction is outside the box.
            if (member.type == OSMEntity.Type.WAY) {
                if (!osm.ways.containsKey(member.id)) {
                    LOG.warn(
                            "Turn restriction relation {} references nonexistent way {}, dropping"
                                + " this relation",
                            osmRelationId,
                            member.id);
                    return;
                ...
                if (!osm.nodes.containsKey(member.id)) {
                    LOG.warn(
                            "Turn restriction relation {} references nonexistent node {}, dropping"
                                + " this relation",
                            osmRelationId,
                            member.id);
                    return;
        ...
        if (from == null || to == null || via.isEmpty()) {
            LOG.error(
                    "Invalid turn restriction {}, does not have from, to and via, skipping",
                    osmRelationId);
            return;
        }
        ...
        if ((hasViaWays && hasViaNodes) || (hasViaNodes && via.size() > 1)) {
            LOG.error(
                    "via must be single node or one or more ways, skipping restriction {}",
                    osmRelationId);
            return;
        }
        ...
        if (hasViaNodes) {
            ...
            if (vertex == -1) {
                LOG.warn(
                        "Vertex {} not found to use as via node for restriction {}, skipping this"
                            + " restriction",
                        via.get(0).id,
                        osmRelationId);
                return;
                    ...
                    .forEach(
                            eidx -> {
                                ...
                                if (e.getOSMID() == fromWayId) {
                                    if (fromEdge[0] != -1) {
                                        LOG.error(
                                                "From way enters vertex {} twice, restriction {} is"
                                                    + " therefore ambiguous, skipping",
                                                vertex,
                                                osmRelationId);
                                        bad[0] = true;
                                        return false;
                    ...
                    .forEach(
                            eidx -> {
                                ...
                                if (e.getOSMID() == toWayId) {
                                    if (toEdge[0] != -1) {
                                        LOG.error(
                                                "To way exits vertex {} twice, restriction {} is"
                                                    + " therefore ambiguous, skipping",
                                                vertex,
                                                osmRelationId);
                                        bad[0] = true;
            ...
            // The restriction's via member(s) are ways, which is more tricky than a restriction via
            // a single node.
            Way fromWay = osm.ways.get(from.id);
            long[][] viaNodes =
                    via.stream().map(m -> osm.ways.get(m.id).nodes).toArray(i -> new long[i][]);
            Way toWay = osm.ways.get(to.id);

            // We need to convert from an unordered set of OSM ways to an ordered sequence of R5
            // edges, where the
            // edges may be smaller than the ways. We do a search, finding a path through our street
            // graph that touches
            ...
            // via.size() - 1 because we've already explored one via way where we transferred from
            // the the from way
                ...
                for (int statePos = 0; statePos < previousNodes.size(); statePos++) {
                    ...
                    for (long node : way.nodes) {
                        ...
                        for (int viaPos = 0; viaPos < viaNodes.length; viaPos++) {
                            ...
                            // We don't handle complicated cases (which may or may not exist) that
                            // have loops.
                            ...
                            for (long viaNode : osm.ways.get(viaWayId).nodes) {
                                if (viaNode == node) {
                                    long[] newNodes =
                                            Arrays.copyOf(previousNodes.get(statePos), round + 2);
                                    long[] newWays =
                                            Arrays.copyOf(previousWays.get(statePos), round + 2);
                ...
                for (long node : finalWay.nodes) {
                    for (long toNode : toWay.nodes) {
                        if (node == toNode) {
                            if (pathNodes != null) {
                                LOG.error(
                                        "Turn restriction {} has ambiguous via ways (multiple paths"
                                            + " through via ways between from and to), skipping",
                                        osmRelationId);
                                return;
                            }
            ...
            if (pathNodes == null) {
                LOG.error(
                        "Invalid turn restriction {}, no way from from to to via via, skipping",
                        osmRelationId);
                return;
            }
            ...
            // edges affected by this turn restriction. Make a list in case something goes awry when
            ...
            for (int nidx = pathNodes.length - 1; nidx > 0; nidx--) {
                ...
                incomingEdges
                        ...
                        .forEach(
                                eidx -> {
                                    ...
                                    if (e.getOSMID() == wayId) {
                                        if (edge[0] != -1) {
                                            // TODO we've already started messing with data
                                            // structures!
                                            LOG.error(
                                                    "To way exits vertex {} twice, restriction {}"
                                                        + " is therefore ambiguous, skipping",
                                                    vertex,
                                                    osmRelationId);
                                            bad[0] = true;
                                            return false;
                ...
                if (edge[0] == -1) {
                    LOG.warn(
                            "Did not find via way {} for restriction {}, skipping",
                            wayId,
                            osmRelationId);
                    return;
                }
     ...
     * "only" restrictions are a mess to implement in reverse, so "only" restrictions are converted
     * to (possibly multiple) "no" restrictions when they're reversed.
     *
     * <p>These converted restrictions are added to StreetLayer.turnRestrictions (the list of all
     * turn restrictions in the network) and then to EdgeStore.turnRestrictionsReverse (associating
     * them with their toEdge).
     *
     * <p>"No" turn restrictions, whether they were originally "no" restrictions or were converted
     * from "only" turn restrictions, are associated with their toEdge (instead of fromEdge) since
     * this method is handling restrictions to be used in reverse searches.
     */
    ...
    /**
     * Get or create mapping from a global long OSM ID to an internal street vertex ID, creating the
     * vertex as needed.
     *
     * @return the internal ID for the street vertex that was found or created, or -1 if there was
     *     no such OSM node.
     */
    private int getVertexIndexForOsmNode(long osmNodeId) {
        ...
        if (vertexIndex == -1) {
            ...
            if (node == null) {
                LOG.warn(
                        "OSM data references an undefined node. This is often the result of"
                            + " extracting a bounding box in Osmosis without the completeWays"
                            + " option.");
            } else {
    ...
    /**
     * Calculate length from a list of nodes. This is done in advance of creating an edge pair
     * because we need to catch potential length overflows before we ever reserve space for the
     * edges.
     *
     * @return the length of the edge in millimeters, or -1 if that length will overflow a 32 bit
     *     int
     */
    ...
    /**
     * Make an edge for a sub-section of an OSM way, typically between two intersections or leading
     * up to a dead end.
     */
    private void makeEdgePair(Way way, int beginIdx, int endIdx, Long osmID) {
        ...
        // FIXME these sets of flags should probably not leak outside the permissions/stress/etc.
        // labeler methods
        ...
        if (Collections.disjoint(forwardFlags, ALL_PERMISSIONS)
                && Collections.disjoint(backFlags, ALL_PERMISSIONS)) {
            LOG.debug("Way has no permissions skipping!");
            return;
        }

        // Set forward and backward edge flags from OSM Way tags. The flags will later be stored in
        // the EdgeStore.
        ...
        Edge newEdge =
                edgeStore.addStreetPair(
                        beginVertexIndex, endVertexIndex, edgeLengthMillimeters, osmID);
        ...
        // If per-edge traversal time factors are being recorded for this StreetLayer, store these
        // factors for the
        ...
        if (edgeStore.edgeTraversalTimes != null) {
            try {
                ...
                LOG.error(
                        "Continuing to load but ignoring generalized costs due to exception: {}",
                        ex.toString());
    ...
    public void indexStreets() {
        ...
        // Skip by twos, we only need to index forward (even) edges. Their odd companions have the
        // same geometry.
    ...
    /**
     * Rather than querying the spatial index directly, going through this method will ensure that
     * any temporary edges not in the index are also visible. Temporary edges, created when applying
     * a scenario in a single thread, are not added to the spatial index, which is read by all
     * threads. Note: the spatial index can and will return false positives, but should not produce
     * false negatives. We return the unfiltered results including false positives because
     * calculating the true distance to each edge is quite a slow operation. The caller must
     * post-filter the set of edges if more distance information is needed, including knowledge of
     * whether an edge passes inside the query envelope at all. FIXME Document whether we're putting
     * all edges, or just forward edges into this index.
     *
    ...
    /**
     * The edge lists (which edges go out of and come into each vertex) are derived from the edges
     * in the EdgeStore. So any time you add edges or change their endpoints, you need to rebuild
     * the edge index. TODO some way to signal that a few new edges have been added, rather than
     * rebuilding the whole lists.
     */
    ...
    /**
     * Find an existing street vertex near the supplied coordinates, or create a new one if there
     * are no vertices near enough. Note that calling this method is potentially destructive (it can
     * modify the street network).
     *
     * <p>This uses {@link #findSplit(double, double, double, StreetMode)} and {@link Split} which
     * require the spatial index to already be built. In other works {@link #indexStreets()} needs
     * to be called before this is used.
     *
     * <p>TODO potential refactor: rename this method Split.perform(), and store a ref to
     * streetLayer in Split.
     *
     ...
     * @return the index of a street vertex very close to the supplied location, or -1 if no such
     *     vertex could be found or created.
     */
    public int getOrCreateVertexNear(double lat, double lon, StreetMode streetMode) {
        ...
        // The split is somewhere along a street away from an existing intersection vertex. Make a
        // new splitter vertex.
        int newVertexIndex = vertexStore.addVertexFixed((int) split.fixedLat, (int) split.fixedLon);
        int oldToVertex =
                edge
                        .getToVertex(); // Hold a copy of the to vertex index, because it may be
                                        // modified below.
        P2<int[]> geoms = edge.splitGeometryAfter(split.seg);
        if (edge.isMutable()) {
            // The edge we are going to split is mutable.
            // We're either building a baseline graph, or modifying an edge created within the same
            // scenario.
            // Modify the existing bidirectional edge pair to serve as the first segment leading up
            // to the split point.
            ...
            // We must be applying a scenario, and this edge is part of the baseline graph shared
            // between threads.
            // Preserve the existing edge pair, creating a new edge pair to lead up to the split.
            // The new edge will be added to the edge lists later (the edge lists are a transient
            // index).
            // We add it to a temporary spatial index specific to this scenario, rather than the
            // base spatial index
            // which is shared between all scenarios on this network.
            EdgeStore.Edge newEdge0 =
                    edgeStore.addStreetPair(
                            edge.getFromVertex(),
                            newVertexIndex,
                            split.distance0_mm,
                            edge.getOSMID());
            // Copy the flags and speeds for both directions, making the new edge like the existing
            // one.
            newEdge0.copyPairFlagsAndSpeeds(edge);
            newEdge0.setGeometry(geoms.a);
            // Add the new edges to a temporary spatial index that is associated with only this
            // scenario.
            // we need to build this on the fly so that it is possible to split a street multiple
            // times; otherwise,
            // once a street had been split once, the original edge would be removed from
            // consideration
            // (StreetLayer#getEdgesNear filters out edges that have been deleted) and the new edge
            // would not yet be in
            // the spatial index for consideration. Havoc would ensue.
            temporaryEdgeIndex.insert(newEdge0.getEnvelope(), newEdge0.edgeIndex);

            // Exclude the original split edge from all future spatial index queries on this
            // scenario copy.
            // This should allow proper re-splitting of a single edge for multiple new transit
            // stops.
        ...
        // The new edge will be added to the edge lists later (the edge lists are a transient
        // index).
        EdgeStore.Edge newEdge1 =
                edgeStore.addStreetPair(
                        newVertexIndex, oldToVertex, split.distance1_mm, edge.getOSMID());
        ...
        // One way to do this: make a vertex flag for splitter vertices. When at a splitter vertex,
        // don't consider
        // traversing edges that have the same OSM ID but the opposite direction (i.e. are even when
        // the previous
    ...
    // By convention we only index the forward edge since both edges in a pair have the same
    // geometry.
    // TODO expand to handle temp / non-temp automatically? This indexing can't be done at the end
    // to allow re-splitting.
    ...
    /**
     * Perform destructive splitting of edges FIXME: currently used only in P+R. This methods should
     * probably be changed to resuse code from getOrCreateVertexNear.
     */
    public int splitEdge(Split split) {
        // We have a linking site. Find or make a suitable vertex at that site.
        // Retaining the original Edge cursor object inside findSplit is not necessary, one object
        // creation is harmless.
        Edge edge = edgeStore.getCursor(split.edge);
        ...
        edge.setGeometry(
                Collections
                        .EMPTY_LIST); // Turn it into a straight line for now. FIXME split edges
                                      // should have geometries

        // Make a second, new bidirectional edge pair after the split and add it to the spatial
        // index.
        ...
        // turn restrictions on the forward edge go to the new edge's forward edge. Turn
        // restrictions on the back edge stay
        // where they are
        edgeStore
                .turnRestrictions
                .removeAll(split.edge)
                .forEach(ridx -> edgeStore.turnRestrictions.put(newEdge.edgeIndex, ridx));

        return newVertexIndex;
        // TODO store street-to-stop distance in a table in TransitLayer. This also allows adjusting
        // for subway entrances etc.
    }

    /**
     * Create a street-layer vertex representing a transit stop, and connect that new vertex to the
     * street network if possible. The vertex will be created and assigned an index whether or not
     * it is successfully linked to the streets. This is intended for transit stop linking. It
     * always creates a new vertex in the street layer exactly at the coordinates provided. You can
     * be sure to receive a unique vertex index each time it's called on the same street layer. Once
     * it has created this new vertex, it will look for the nearest edge in the street network and
     * link the newly created vertex to the closest point on that nearby edge. The process of
     * linking to that edge may or may not create a second new splitter vertex along that edge. If
     * the newly created vertex is near an intersection or another splitter vertex, the existing
     * vertex will be reused. So in sum, this will create one or two new vertices, and all necessary
     * edge pairs to properly connect these new vertices. TODO store street-to-stop distance in a
     * table in TransitLayer, or change the link edge length. This also allows adjusting for subway
     * entrances etc.
     *
    ...
    public int createAndLinkVertex(double lat, double lon) {
        ...
        int length_mm =
                (int)
                        (GeometryUtils.distance(
                                        lat, lon, streetVertex.getLat(), streetVertex.getLon())
                                * 1000);
        ...
        // In practice, mode permissions will be controlled by whatever street edges lead up to
        // these link edges.
     ...
     * vertices or edges. This is a nondestructive operation: it simply finds a candidate split
     * point without modifying anything. This function starts with a small search envelope and
     * expands it as needed under the assumption that most search points will be close to a road.
     * TODO favor transit station platforms and pedestrian paths when requested
     *
     * @param lat latitude in floating point geographic coordinates (not fixed point int
     *     coordinates)
     * @param lon longitude in floating point geographic coordinates (not fixed point int
     *     coordinates)
     * @param streetMode a mode of travel that the street must allow
     * @return a Split object representing a point along a sub-segment of a specific edge, or null
     *     if there are no streets nearby allowing the specified mode of travel.
     */
    public Split findSplit(double lat, double lon, double radiusMeters, StreetMode streetMode) {
        ...
        // If no split point was found by the first search (or no search was yet conducted) search
        // with the full radius.
        if (split == null) {
     ...
     * record the connection between the two.
     */
    public void associateStops(TransitLayer transitLayer) {
        for (Stop stop : transitLayer.stopForIndex) {
            ...
            transitLayer.streetVertexForStop.add(
                    stopVertex); // This is always a valid, unique vertex index.
            // The inverse stopForStreetVertex map is a transient, derived index and will be built
            // later.
        }
    }
    ...
    /**
     * We intentionally avoid using clone() on EdgeStore and VertexStore so all field copying is
     * explicit and we can clearly see whether we are accidentally shallow-copying any collections
     * or data structures from the base graph. StreetLayer has a lot more fields and most of them
     * can be shallow-copied, so here we use clone() for convenience.
     *
     * @param willBeModified must be true if the scenario to be applied will make any changes to the
     *     new StreetLayer copy. This allows some optimizations (the lists in the StreetLayer will
     *     not be wrapped).
     * @return a copy of this StreetLayer to which Scenarios can be applied without affecting the
     *     original StreetLayer.
     *     <p>It's questionable whether the willBeModified optimization actually affects routing
     *     speed, but in theory it saves a comparison and an extra dereference every time we use the
     *     edge/vertex stores. TODO check whether this actually affects speed. If not, just wrap the
     *     lists in every scenario copy. Why would you clone the StreetLayer at all if it's not
     *     going to be modified? Because there are circular references between the street and
     *     transit layers, so if you don't clone both, you could end up at the wrong transit or
     *     street layer by chaining together those references.
     */
    public StreetLayer scenarioCopy(TransportNetwork newScenarioNetwork, boolean willBeModified) {
        ...
        if (willBeModified) {
            ...
            // Indicate that the content of the new StreetLayer will be changed by giving it the
            // scenario's scenarioId.
            // If the copy will not be modified, scenarioId remains unchanged to allow cached
            // pointset linkage reuse.
            copy.scenarioId = newScenarioNetwork.scenarioId;
            copy.edgeStore = edgeStore.extendOnlyCopy(copy);
            // The extend-only copy of the EdgeStore also contains a new extend-only copy of the
            // VertexStore.
    ...
    public void associateBikeSharing(TNBuilderConfig tnBuilderConfig) {
        ...
        BikeRentalBuilder bikeRentalBuilder =
                new BikeRentalBuilder(new File(tnBuilderConfig.bikeRentalFile));
        ...
        for (BikeRentalStation bikeRentalStation : bikeRentalStations) {
            int streetVertexIndex =
                    getOrCreateVertexNear(
                            bikeRentalStation.lat, bikeRentalStation.lon, StreetMode.WALK);
            if (streetVertexIndex > -1) {
                numAddedStations++;
    ...
    public StreetLayer clone() {
        try {
            ...
            throw new RuntimeException(
                    "This exception cannot happen. This is why I love checked exceptions.");
        }
    }

    /**
     * @return true if this StreetLayer was created by a scenario, and is therefore wrapping a base
     *     StreetLayer.
     */
    ...
    /**
     * Create a geometry in FIXED POINT DEGREES containing all the points on all edges created or
     * removed by the scenario that produced this StreetLayer, buffered by radiusMeters. This is a
     * MultiPolygon or GeometryCollection. When there are no created or removed edges, returns an
     * empty geometry rather than null, because we test whether transit stops are contained within
     * the resulting geometry.
     */
    public Geometry scenarioEdgesBoundingGeometry(int radiusMeters) {
        List<Polygon> geoms = new ArrayList<>();
        Edge edge = edgeStore.getCursor();
        edgeStore.forEachTemporarilyAddedOrDeletedEdge(
                e -> {
                    edge.seek(e);
        ...
        // We can't just make a multipolygon as the component polygons may not be disjoint. Unions
        // are pretty quick though.
        // The UnaryUnionOp gets its geometryFactory from the geometries it's operating on.
        // We need to supply one in case the list is empty, so it can return an empty geometry
        // instead of null.
    ...
    /**
     * Create a geometry in FIXED POINT DEGREES containing all the points on all edges created by
     * the scenario that produced this StreetLayer. Returns null if the resulting geometry would
     * contain no points, to allow short-circuiting around slow operations that would use the
     * result.
     */
    public Geometry addedEdgesBoundingGeometry() {
        List<Geometry> geoms = new ArrayList<>();
        Edge edge = edgeStore.getCursor();
        edgeStore.forEachTemporarilyAddedEdge(
                e -> {
                    edge.seek(e);
                    // Should we simply only put linkable edges in the temp edge index? We could
                    // even name it accordingly.
                    // When iterating all scenario edges for egress cost table rebuilding, we don't
        ...
        // The component polygons may not be disjoint, so rather than making a multipolygon compute
        // the union.
        Geometry result = new UnaryUnionOp(geoms, GeometryUtils.geometryFactory).union();
        if (result.isEmpty()) {
            return null;
    ...
    public static void logFixedPointGeometry(String label, Geometry fixedPointGeometry) {
        if (fixedPointGeometry == null) {
            ...
            String geoJson =
                    new GeometryJSON().toString(fixedDegreeGeometryToFloating(fixedPointGeometry));
            if (geoJson == null) {
    ...
    /**
     * Finds all the P+R stations in given envelope. This might overselect (doesn't filter the
     * objects from the spatial index) but it's only used in visualizations.
     *
                    ...
                    eidx -> {
                        ...
                        if (e.getFlag(EdgeStore.EdgeFlag.LINK)) {
                            ...
                            if (v.getFlag(VertexStore.VertexFlag.PARK_AND_RIDE)) {
                                ParkRideParking parkRideParking =
                                        parkRideLocationsMap.get(e.getFromVertex());
    ...
    /**
     * Finds all the bike share stations in given envelope. Might overselect from the spatial index
     * but this is only used for visualizations.
     *
     * @param env Envelope in float degrees
     * @return BikeRentalStations, or empty list if none are found or there are no bike stations in
     *     the graph.
     */
                    ...
                    eidx -> {
                        ...
                        if (v.getFlag(VertexStore.VertexFlag.BIKE_SHARING)) {
                            BikeRentalStation bikeRentalStation =
                                    bikeRentalStationMap.get(e.getFromVertex());
    ...
    /**
     * For the given location and mode of travel, get an object representing the available on-demand
     * mobility service, including pick-up delay and which stops it will take you to. We currently
     * only support one StreetMode per pickup delay polygon collection. If the supplied mode matches
     * the wait time polygons' mode, return the pickup delay (or -1 for no service). Otherwise,
     * return an object representing a 0 second delay.
     *
     ...
     */
    public PickupWaitTimes.AccessService getAccessService(
            double lat, double lon, StreetMode streetMode) {
    ...
    public boolean edgeIsDeletedByScenario(int p) {
        return edgeStore.temporarilyDeletedEdges != null
                && edgeStore.temporarilyDeletedEdges.contains(p);
    }
