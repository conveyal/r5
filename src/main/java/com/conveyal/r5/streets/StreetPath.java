...
public class StreetPath {
    ...
    private LinkedList<RoutingState> states;

    private LinkedList<Integer> edges;
    private RoutingState lastState;
    private RoutingState firstState;
    private StreetLayer streetLayer;
    private int distance;
    private int duration;

    public StreetPath(RoutingState s, StreetLayer streetLayer, boolean reverseSearch) {
        ...
        if (reverseSearch) {
            this.lastState = s.reverse(streetLayer.edgeStore);
        }
         ...
         */
        for (RoutingState cur = lastState; cur != null; cur = cur.backState) {
            states.addFirst(cur);
            if (cur.backEdge != -1) {
     ...
     */
    public StreetPath(RoutingState lastState, StreetRouter streetRouter, LegMode mode) {
        this(lastState, streetRouter.streetLayer, false);
        //First streetPath is part of path from last bicycle station to the end destination on foot
        if (mode == LegMode.BICYCLE_RENT) {
            RoutingState endCycling = getStates().getFirst();
            ...
            if (lastState != null) {
                ...
                RoutingState startCycling = getStates().getFirst();
            ...
            RoutingState carPark = getStates().getFirst();
            //So we need to search for driving part in previous streetRouter
            RoutingState carState = streetRouter.previousRouter.getStateAtVertex(carPark.vertex);
            if (carState != null) {
                add(carState, true);
            } else {
    ...
    public LinkedList<RoutingState> getStates() {
        return states;
    }
    ...
    public Edge getEdge(Integer edgeIdx) {
        return streetLayer.getEdgeCursor(edgeIdx);
    }
     ...
     */
    public void add(RoutingState lastState, boolean updateDistanceTime) {
        boolean first = true;
        /*
         * Starting from latest (time-wise) state, copy states to the head of a list in reverse
         * chronological order. List indices will thus increase forward in time, and backEdges will
        ...
        if (updateDistanceTime) {
            ...
            distance += lastState.distance;
            duration += lastState.getDurationSeconds()+ CAR_PARK_DROPOFF_TIME_S;
        }
    }
}
