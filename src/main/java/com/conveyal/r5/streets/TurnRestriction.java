...
public class TurnRestriction implements Serializable {
     ...
     *
     * <p>TODO: make this on the fly without copying arrays
     *
     * @param viaEdges
     */
     ...
     *
     * <p>If function is called on NO TURN restriction empty list is returned since no remapping is
     * needed
     *
     * <p>This is since only turn restrictions aren't supported in reverse street search.
     *
     * <p>For example ONLY turn restriction from EN+1 to ENW over EW is remapped to: NO TURN
     * restrictions: EN+1 to EN, EN+1 to EE, EN+1 to ES, EN+1 to ENE and to EN+1 to EW+1 over EW,
     * EN+1 to ESW over EW
     *
     * @param streetLayer
     * @return list of new NO TURN restrictions which are semantically the same as one ONLY TURN
     *     restriction
     */
    List<TurnRestriction> remap(StreetLayer streetLayer) {
        ...
        if (only) {
            ...
            for (int posInTurnRestriction = -1;
                    posInTurnRestriction < viaEdges.length;
                    posInTurnRestriction++) {
                next[0] =
                        posInTurnRestriction + 1 < viaEdges.length
                ...
                // Goes over all outgoing CAR/BIKE traversable edges of toVertex of currentEdgeIdx
                // an adds NO TURN turn restriction
                // Unless edge is next in current turn restriction
                streetLayer
                        .outgoingEdges
                        .get(fromEdgeToVertex)
                        .forEach(
                                eidx -> {
                                    if (eidx == next[0]) {
                                        return true;
                                    ...
                                    e.seek(eidx);
                                    // filter car and bicycle traversible and add turn restriction
                                    // from from and on this edges which aren't via/to
                                    if (e.getFlag(EdgeStore.EdgeFlag.ALLOWS_CAR)
                                            || e.getFlag(EdgeStore.EdgeFlag.ALLOWS_BIKE)) {
                                        TurnRestriction restriction = new TurnRestriction();
                                        restriction.fromEdge = fromEdge;
                                        // Adds via edges which are via edges in original turn
                                        // restriction up to this point
                                        if (finalPosInTurnRestriction >= 0) {
                                            restriction.viaEdges =
                                                    Arrays.copyOfRange(
                                                            viaEdges,
                                                            0,
                                                            finalPosInTurnRestriction + 1);
                                        }
