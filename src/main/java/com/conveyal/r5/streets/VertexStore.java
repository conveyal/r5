...
/**
 * Store a large number of vertices in parallel arrays, providing some abstraction to view them as
 * Vertex objects. This class is a workaround for Java's lack of compound value types. It is the
 * equivalent of an array of Vertex structs, but transposed to be a column store. Not because the
 * work we're doing operates frequently on entire columns, just because the quirks of Java make
 * parallel arrays more memory efficient than lists of objects. It would be nice if this kind of
 * functionality was built into the language but it's not too terrible to hand-roll it. It does also
 * have the advantage of allowing a field to be dropped entirely (its array field is null) which is
...
public class VertexStore implements Serializable {
    ...
    public static final double FIXED_FACTOR =
            1e7; // we could just reuse the constant from osm-lib Node.
    ...
    /**
     * Used when converting fixed-point latitude and longitude to floating-point from Split. The
     * parameter datatype is double even though it is a fixed-point representation.
     */
    ...
    /**
     * Given a Geometry in fixed-point latitude and longitude, return a copy converted to floating
    ...
    public static Geometry fixedDegreeGeometryToFloating(Geometry fixedGeometry) {
        ...
        wgsResult.apply(
                (CoordinateFilter)
                        c -> {
                            c.x = fixedDegreesToFloating(c.x);
                            c.y = fixedDegreesToFloating(c.y);
                        });
        return wgsResult;
    }
    ...
    public enum VertexFlag {
        ...
        // TODO Add Stop and split vertex flags. This allows blocking U-turns at splits and
        // eliminating the special stop finder visitor
    ...
    /**
     * Makes a copy of this VertexStore that can have vertices added to it, but cannot otherwise be
     * modified. This is done efficiently by wrapping the existing lists holding the various vertex
     * characteristics.
     *
     * <p>We don't use clone() to make sure every field is explicitly copied below, avoiding any
     * unintentional shallow-copying of collections or referenced data structures.
     */
    public VertexStore extendOnlyCopy() {
        ...
        // This is a deep copy, we should do an extend-copy but need a new TByteArrayList class for
        // that.
        // I would like to make sure the extend-only system works well before proliferating classes
        // of this kind.
