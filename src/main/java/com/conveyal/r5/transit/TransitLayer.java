...
/**
 * A key simplifying factor is that we don't handle overnight trips. This is fine for analysis at
 * usual times of day.
 */
public class TransitLayer implements Serializable, Cloneable {

    /**
     * Maximum distance to record in distance tables, in meters. Set to 3.5 km to match OTP
     * GraphIndex.MAX_WALK_METERS but TODO should probably be reduced after Kansas City project.
     */
    ...
    /**
     * Distance limit for transfers, meters. Set to 1km which is slightly above OTP's 600m (which
     * was specified as 1 m/s with 600s max time, which is actually somewhat less than 600m due to
     * extra costs due to steps etc.
     */
    ...
    /**
     * The time zone in which this TransportNetwork falls. It is read from a GTFS agency. It
     * defaults to GMT if no valid time zone can be found.
     */
    ...
    // Inverse map of stopIdForIndex, reconstructed from that list (not serialized). No-entry value
    // is -1.
    ...
    // FIXME we may currently be storing weight or time to reach other stop, which we did to avoid
    // floating point division. Instead, store distances in millimeters, and divide by speed in
    // mm/sec.
    ...
    // TODO there is probably a better way to do this, but for now we need to retain stop object for
    // linking to streets
    public transient List<Stop> stopForIndex = new ArrayList<>();

    // The coordinates of a place roughly in the center of the transit network, for centering maps
    ...
    /**
     * For each transit stop, an int-int map giving the walking distance to every reachable street
     * vertex from that stop. This is the result of running a distance-constrained street search
     * outward from every stop in the graph. If these tables are present, we serialize them when
     * persisting a network to disk to avoid recalculating them upon re-load. However, these tables
     * are only computed when the network is first built in certain code paths used for analysis
     * work. The tables are not necessary for basic point-to-point routing. Serializing this table
     * makes network files much bigger and makes our checks to ensure that scenario application does
     * not damage base graphs slower.
     */
    public List<TIntIntMap> stopToVertexDistanceTables;

    /**
     * The TransportNetwork containing this TransitLayer. This link up the object tree also allows
     * us to access the StreetLayer associated with this TransitLayer in the same TransportNetwork
    ...
    /**
     * A string uniquely identifying the contents of this TransitLayer among all TransitLayers. When
     * no scenario has been applied, this field will contain the ID of the containing
     * TransportNetwork. When a scenario has modified this StreetLayer, this field will be changed
     * to the scenario's ID. We need a way to know what transit information is in the layer
     * independent of object identity, which is lost in a round trip through serialization. This
     * also allows re-using cached information for multiple scenarios that don't modify the transit
     * network. (This never happens yet, but will when we allow street editing.)
     */
    public String scenarioId;

    /**
     * Load a GTFS feed with full load level. The feed is not closed after being loaded. TODO
     * eliminate "load levels"
     */
    ...
    /**
     * Load data from a GTFS feed. Call multiple times to load multiple feeds. The feed is not
     * closed after being loaded.
     */
    public void loadFromGtfs(GTFSFeed gtfs, LoadLevel level) throws DuplicateFeedException {
        ...
        // Load stops.
        // ID is the GTFS string ID, stopIndex is the zero-based index, stopVertexIndex is the index
        // in the street layer.
        ...
        for (Stop stop : gtfs.stops.values()) {
            ...
            // This is only used while building the TransitNetwork to look up StopTimes from the
            // same feed.
        ...
        // Load service periods, assigning integer codes which will be referenced by trips and
        // patterns.
        TObjectIntMap<String> serviceCodeNumber = new TObjectIntHashMap<>(20, 0.5f, -1);
        gtfs.services.forEach(
                (serviceId, service) -> {
        ...
        // Group trips by stop pattern (including pickup/dropoff type) and fill stop times into
        // patterns.
        // Also group trips by the blockId they belong to, and chain them together if they allow
        // riders to stay on board
        // the vehicle from one trip to the next, even if it changes routes or directions. This is
        // called "interlining".
        gtfs.findPatterns();
        ...
        for (String tripId : gtfs.trips.keySet()) {
            ...
            try {
                ...
                LOG.warn(
                        "First and last stops do not both have times specified on trip {} on route"
                            + " {}, skipping this as interpolation is impossible",
                        trip.trip_id,
                        trip.route_id);
                continue TRIPS;
            }
            ...
            for (StopTime st : stopTimes) {
                ...
                if (previousDeparture > st.arrival_time || st.arrival_time > st.departure_time) {
                    LOG.warn(
                            "Negative-time travel at stop {} on trip {} on route {}, skipping this"
                                + " trip as it will wreak havoc with routing",
                            st.stop_id,
                            trip.trip_id,
                            trip.route_id);
                    continue TRIPS;
                ...
                if (previousDeparture
                        == st.arrival_time) { // Teleportation: arrive at downstream stop
                                              // immediately after departing upstream
                    // often the result of a stop_times input with time values rounded to the
                    // nearest minute.
                    // TODO check if the distance of the hop is reasonably traveled in less than 60
                    // seconds, which may vary by mode.
                    nZeroDurationHops++;
                }
            ...
            if (nStops == 0) {
                LOG.warn(
                        "Trip {} on route {} {} has no stops, it will not be used",
                        trip.trip_id,
                        trip.route_id,
            ...
            if (tripPattern == null) {
                tripPattern =
                        new TripPattern(
                                String.format("%s:%s", gtfs.feedId, route.route_id),
                                stopTimes,
                ...
                if (level == LoadLevel.FULL) {
                    ...
                    if (trip.shape_id != null && SAVE_SHAPES) {
                        ...
                        if (shape == null)
                            LOG.warn(
                                    "Shape {} for trip {} was missing",
                                    trip.shape_id,
                                    trip.trip_id);
                        else {
                            // TODO this will not work if some trips in the pattern don't have
                            // shapes
                            tripPattern.shape = shape.geometry;

                            // project stops onto shape
                            boolean stopsHaveShapeDistTraveled =
                                    StreamSupport.stream(stopTimes.spliterator(), false)
                                            .noneMatch(st -> Double.isNaN(st.shape_dist_traveled));
                            boolean shapePointsHaveDistTraveled =
                                    DoubleStream.of(shape.shape_dist_traveled)
                            ...
                            if (stopsHaveShapeDistTraveled && shapePointsHaveDistTraveled) {
                                ...
                                LocationIndexedLineInLocalCoordinateSystem line =
                                        new LocationIndexedLineInLocalCoordinateSystem(
                                                shape.geometry.getCoordinates());
                            ...
                            for (int i = 0; i < locations.length; i++) {
                                ...
                                tripPattern.stopShapeFraction[i] =
                                        (float) locations[i].getSegmentFraction();
                            }
                        }
                    }
                }
        ...
        LOG.info(
                "Done creating {} trips on {} patterns.",
                nTripsAdded,
                tripPatternForPatternId.size());
        ...
        // Chain together trips served by the same vehicle that allow transfers by simply staying on
        // board.
        // Elsewhere this is done by grouping by (serviceId, blockId) but this is not supported by
        // the spec.
                ...
                .forEach(
                        (blockId, trips) -> {
                            TripSchedule[] schedules =
                                    trips.toArray(new TripSchedule[trips.size()]);
        ...
        // Otherwise it is set to first valid agency timezone and warning is shown if agencies have
        // different timezones
        if (gtfs.agency.size() == 0) {
            ...
            for (Agency agency : gtfs.agency.values()) {
                ...
                try {
                    ...
                    LOG.error(
                            "Agency {} in GTFS with timezone '{}' wasn't found in timezone database"
                                + " reason: {}",
                            agency.agency_name,
                    ...
                    LOG.error(
                            "Agency {} in GTFS has timezone in wrong format:'{}'. Expected format:"
                                + " area/city ",
                            agency.agency_name,
                            agency.agency_timezone);
                    // timezone will be set to GMT if it is still empty after for loop
                ...
                if (timeZone == null) {
                    LOG.info(
                            "TransportNetwork time zone set to {} from agency '{}' and"
                                + " agency_timezone:{}",
                            tz,
                            agency.agency_name,
                            agency.agency_timezone);
                    ...
                    LOG.error(
                            "agency time zone {} differs from TransportNetwork time zone: {}. This"
                                + " will be problematic.",
                            tz,
                            timeZone);
                }
            ...
            if (timeZone == null) {
                timeZone = ZoneId.of("GMT");
                LOG.warn(
                        "No agency in graph had valid timezone; API request times will be"
                            + " interpreted as GMT.");
            }
        }
        ...
        //        Multimap<T2<String, Integer>, TripPattern> patternsForRouteDirection =
        // HashMultimap.create();
        //        tripPatterns.forEach(tp -> patternsForRouteDirection.put(new T2(tp.routeId,
        // tp.directionId), tp));
        //        for (T2<String, Integer> routeAndDirection : patternsForRouteDirection.keySet()) {
        //            RouteTopology topology = new RouteTopology(routeAndDirection.first,
        // routeAndDirection.second, patternsForRouteDirection.get(routeAndDirection));
        //        }
    ...
    private void findCenter(Collection<Stop> stops) {
        ...
        // Stops is a HashMap so size() is fast. If it ever becomes a MapDB BTree, we may want to do
        // this differently.
    ...
    public void rebuildTransientIndexes() {
        ...
        for (int patternIdx = 0; patternIdx < tripPatterns.size(); patternIdx++) {
            ...
            for (int tripScheduleIdx = 0;
                    tripScheduleIdx < pattern.tripSchedules.size();
                    tripScheduleIdx++) {
                ...
                for (int frequencyEntryIdx = 0;
                        ...
                        frequencyEntryIdx++) {
                    frequencyEntryIndexForId.put(
                            schedule.frequencyEntryIds[frequencyEntryIdx],
    ...
    /**
     * Run a distance-constrained street search from every transit stop in the graph. Store the
     * distance to every reachable street vertex for each of these origin stops. If a scenario has
     * been applied, we need to build tables for any newly created stops and any stops within
     * transfer distance or access/egress distance of those new stops. In that case a rebuildZone
     * geometry should be supplied. If rebuildZone is null, a complete rebuild of all tables will
     * occur for all stops.
     *
     * @param rebuildZone the zone within which to rebuild tables in FIXED-POINT DEGREES, or null to
     *     build all tables.
     */
    public void buildDistanceTables(Geometry rebuildZone) {
        ...
        if (rebuildZone != null) {
            LOG.info(
                    "Selectively finding distances for only those stops potentially affected by"
                        + " scenario application.");
        }

        LambdaCounter buildCounter =
                new LambdaCounter(
                        LOG,
                        getStopCount(),
                        1000,
        ...
        // Working in parallel, create a new list containing one distance table for each stop index,
        // optionally
        // skipping stops falling outside the specified geometry.
        stopToVertexDistanceTables =
                IntStream.range(0, getStopCount())
                        .parallel()
                        .mapToObj(
                                stopIndex -> {
                                    if (rebuildZone != null) {
                                        // Skip existing or new stops outside the zone that may be
                                        ...
                                        if (p == null || !rebuildZone.contains(p)) {
                                            ...
                                            return stopIndex < stopToVertexDistanceTables.size()
                                                    ? stopToVertexDistanceTables.get(stopIndex)
                                                    : null;
                                        }
                                    }
    ...
    /**
     * Perform a single on-street WALK search from the specified transit stop. Return the distance
     * in millimeters to every reached street vertex.
     *
    ...
    public TIntIntMap buildOneDistanceTable(int stop) {
        ...
        if (originVertex == -1) {
            ...
            LOG.warn(
                    "Stop {} has not been linked to the street network, cannot build a distance"
                        + " table for it.",
                    stop);
            return null;
        }
        ...
        // The values in this map will be distances in millimeters since that is our dominance
        // function.
    ...
    // Mark all services that are active on the given day. Trips on inactive services will not be
    // used in the search.
    ...
    // TODO setStreetLayer which automatically links and records the streetLayer ID in a field for
    // use elsewhere?
    ...
    /**
     * Log some summary information about the contents of the layer that might help with spotting
     * errors or bad data.
     */
        ...
        for (String routeId : patternsForRoute.keySet()) {
            ...
            System.out.println(
                    String.join(
                            ",",
                            routeId,
                            Integer.toString(nPatterns),
                            Integer.toString(nTrips),
                            Integer.toString(nStops)));
        }
    }
    ...
    public static TransitModes getTransitModes(int routeType) {
        ...
        /**
         * else if (routeType >= 1600 && routeType < 1700){ //Self drive return TransitModes.CAR; }
         */
    ...
    public TransitLayer scenarioCopy(TransportNetwork newScenarioNetwork, boolean willBeModified) {
        ...
        if (willBeModified) {
            // Protectively copy all the lists that will be affected by adding new stops to the
            // network.
            // See StopSpec.materializeOne(). We would really only need to do this for modifications
            // that create new stops.
            ...
            // To indicate that this layer is different than the one it was copied from, record the
            // scenarioId of
            // the scenario that modified it. If the scenario will not affect the contents of the
            // layer, its
    ...
    /**
     * Finds all the transit stops in given envelope and returns it. Stops also have mode which is
     * mode of route in first pattern that this stop is found in. Stop coordinates are jittered
     * {@link com.conveyal.r5.point_to_point.PointToPointRouterServer#jitter(VertexStore.Vertex)}.
     * Meaning they are moved a little from their actual coordinate so that multiple stops at same
     * coordinate can be seen.
     *
     * <p>This will overselect from the spatial index but this is only used for visualizations.
     *
    ...
    public Collection<com.conveyal.r5.api.util.Stop> findApiStopsInEnvelope(Envelope env) {
        ...
        TIntSet nearbyEdges =
                this.parentNetwork.streetLayer.spatialIndex.query(VertexStore.envelopeToFixed(env));

        nearbyEdges.forEach(
                eidx -> {
                    e.seek(eidx);
                    if (e.getFlag(EdgeStore.EdgeFlag.LINK)
                            && stopForStreetVertex.containsKey(e.getFromVertex())) {
                        int stopIdx = stopForStreetVertex.get(e.getFromVertex());

                        com.conveyal.r5.api.util.Stop stop =
                                new com.conveyal.r5.api.util.Stop(stopIdx, this, true, true);
                        stops.add(stop);
                    }
                    return true;
                });
    ...
    /**
     * Find all the transit stops inside the given Geometry. Filtering is performed - this should
     * not overselect. This should include stops added by the scenario, minus those removed by the
     * scenario.
     *
    ...
    public TIntSet findStopsInGeometry(Geometry geometry) {
        ...
        // This should get all the edges including ones added by a scenario, minus those removed by
        // a scenario.
    ...
    /**
     * For the given pattern index, returns the GTFS routeId. If includeName is true, the returned
     * string will also include a route_short_name or route_long_name (if they are not null).
     */
    ...
    /**
     * For the given stop index, returns the GTFS stopId (stripped of R5's feedId prefix) and, if
     * includeName is true, stopName.
     */
    public String stopString(int stopIndex, boolean includeName) {
        ...
        String stop =
                stopIdForIndex.get(stopIndex) == null
                        ? "[new]"
                        : stopIdForIndex.get(stopIndex).split(":")[1];
