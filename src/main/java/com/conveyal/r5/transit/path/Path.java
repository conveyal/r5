...
/**
 * Fully specified door-to-door itinerary (at a specific departure time and with specific wait
 * times, which may be derived from random frequency offsets). These itineraries are an optional
 * result from Raptor searches, and they can be reduced/summarized in different ways (e.g.
 * de-duplicating repeated PatternSequences or RouteSequences) TODO rename to Itinerary, compare to
 * PathWithTimes?
 */
public class Path implements Cloneable {
    ...
    // One wait time for each transit leg boarded. This is tracked outside the pathTemplate, because
    // initial wait
    // will depends on the specific departure time (and subsequent waits may depend on random
    // frequency offsets used
    ...
    public Path(RaptorState state, int stop) {
        ...
        while (state.previous != null) {
            ...
            // Step back to previous state before handling transfers, as transfers are done at the
            // end of a round
            state = state.previous;
        ...
        if (length == 0) LOG.error("Transit path computed without a transit segment!");

        // We traversed up the tree (working backward in time) but the user wants to see paths down
        // the tree
