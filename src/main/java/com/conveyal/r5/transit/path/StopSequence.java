...
/**
 * A door-to-door path, i.e. access/egress characteristics and transit legs (keyed on
 * characteristics including per-leg in-vehicle times but not specific trips/patterns/routes), which
 * may be repeated at different departure times.
 *
 * <p>Instances are constructed initially from transit legs, with access and egress set in
...
public class StopSequence {
     ...
     *
     * @param bestAccessOptions map with the optimal access time/mode to reach each stop in the
     *     network
     */
    ...
    /**
     * Set the time spent transferring between stops, which is not stored in our Raptor
     * implementation but can be calculated by subtracting the other components of travel time from
     * the total travel time
     */
    public int transferTime(PathResult.Iteration iteration) {
        if (access == null
                && egress == null
                && iteration.waitTimes.size() == 0
                && rideTimesSeconds == null) {
