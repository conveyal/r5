...
import com.conveyal.eventbus.HandleRegionalEvent;
import com.conveyal.eventbus.HandleSinglePointEvent;
...
import static com.conveyal.r5.analyst.PerTargetPropagater.SECONDS_PER_MINUTE;
import static com.conveyal.r5.analyst.TravelTimeSurfaceResultsFormat.GEOTIFF;
...
public class AnalysisWorker implements Component {
     ...
     */
    private final WebMercatorGridPointSetCache gridPointSetCache = new WebMercatorGridPointSetCache();
    ...
    public AnalysisWorker (
        ...
        this.networkPreloader = new NetworkPreloader(transportNetworkCache, gridPointSetCache);
    ...
    protected void handleOneRegionalTask (RegionalTask task) throws Throwable {
        ...
        if (task.makeTauiSite) {
            ...
            if (oneOriginResult.travelTimes.anyCellReached()) {
                ...
                saveTauiData(task, timesFileName, persistenceBuffer);
            } else {
    ...
    public static class GridJsonBlock {
        ...
        public List<SerializablePathIterations> pathSummaries;
    ...
    public static void addJsonToGrid (
        ...
        JsonUtils.objectMapper.writeValue(outputStream, jsonBlock);
    ...
    public List<RegionalTask> getSomeWork () {
        ...
        httpPost.setEntity(JsonUtils.objectToJsonHttpEntity(workerStatus));
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200 && responseEntity != null) {
                ...
                return JsonUtils.lenientObjectMapper.readValue(responseEntity.getContent(),
    ...
    public void saveTauiMetadata (AnalysisWorkerTask analysisWorkerTask, TransportNetwork network) {
        try {
            ...
            saveTauiData(analysisWorkerTask, "request.json", buffer);
            ...
            saveTauiData(analysisWorkerTask, "warnings.json", buffer);
            ...
            saveTauiData(analysisWorkerTask, "transitive.json", buffer);
        } catch (Exception e) {
    ...
    public void saveTauiData (AnalysisWorkerTask task, String fileName, PersistenceBuffer buffer) {
        FileStorageKey key = new FileStorageKey(FileCategory.TAUI, String.join("/", task.jobId, fileName));
        fileStorage.moveIntoStorage(key, buffer);
    }
}
