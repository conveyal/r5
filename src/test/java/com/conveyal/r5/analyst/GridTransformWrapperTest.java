...
/**
 * Test the class that aligns grids of different dimensions at the same zoom level, unifying their
 * 1D indexes.
 */
class GridTransformWrapperTest {
    ...
    void testTwoAdjacentGrids() {
        ...
        // Translate the pixel weights relative to the right-hand grid into supergrid x and y
        // coordinates.
        List<Grid.PixelWeight> rightTranslatedWeights =
                weights.stream()
        ...
        for (int i = 0; i < superSuperCellCount; i++) {
            ...
            assertEquals(
                    superCount, mergedCount, "Supergrid should contain the sum of left and right");
        }
        assertEquals(weights.stream().mapToDouble(pw -> pw.weight).sum() * 3, totalWeight);
    }

    /**
     * Ensure that we refuse to create a transform wrapper that attempts to transform across zoom
     * levels.
     */
    @Test
    void testMismatchedZoomLevels() {
        ...
        assertThrows(
                ...
                () -> new GridTransformWrapper(webMercatorExtents, grid));
    }
