...
public abstract class DistributionTester {
    ...
    /**
     * Assert that the five percentiles given in the array (5, 25, 50, 75, and 95) appear to be
     * drawn from values uniformly distributed from the given min to max.
     */
    public static void assertUniformlyDistributed(int[] sortedPercentiles, int min, int max) {
        ...
        for (int p = 0; p < PERCENTILES.length; p++) {
            ...
            assertEquals(
                    ...
                    actual,
                    1,
                    "Travel time off by more than one at percentile " + percentile);
        }
    }
    ...
    public static void assertExpectedDistribution(Distribution expectedDistribution, int[] values) {
        for (int p = 0; p < PERCENTILES.length; p++) {
            ...
            assertEquals(
                    ...
                    actual,
                    1,
                    "Travel time off by more than one at percentile " + PERCENTILES[p]);
        }
    }
}
