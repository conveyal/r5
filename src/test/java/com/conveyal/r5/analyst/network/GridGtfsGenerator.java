...
/**
 * Create a MapDB backed GTFS object from a GridLayout, not necessarily to be written out as a
 * standard CSV/ZIP feed, but to be fed directly into the R5 network builder. Used to create
 * networks with predictable characteristics in tests.
 */
public class GridGtfsGenerator {
    ...
    // If mergeStops is true, certain stops will be created multiple times but IDs will collide on
    // insertion.
    ...
    public void addTripsForRoute(GridRoute route, boolean back) {
        ...
        for (int intialDeparture = start;
                intialDeparture < end;
                intialDeparture += headway, tripIndex++) {
            Trip trip = new Trip();
            ...
            for (int stopSequence = 0; stopSequence < route.nStops; stopSequence++) {
                ...
                feed.stop_times.put(
                        new Fun.Tuple2<>(stopTime.trip_id, stopTime.stop_sequence), stopTime);
