...
/**
 * This is a collection of tests using roads and transit lines laid out in a large perfect grid in
 * the desert. These networks have very predictable travel times, so instead of just checking that
 * results don't change from one version to the next of R5 (a form of snapshot testing) this checks
 * that they match theoretically expected travel times given headways, transfer points, distances,
 * common trunks and competing lines, etc.
 *
 * <p>TODO Option to align street grid exactly with sample points in WebMercatorGridPointSet to
 * eliminate walking time between origin and destination and transit stops or street intersections.
 * Also check splitting.
 */
public class SimpsonDesertTests {
    ...
    public void testGridScheduled() throws Exception {
        ...
        // Now to verify the results. We have only our 5 percentiles here, not the full list of
        // travel times.
        // They are also arranged on a grid. This grid does not match the full extents of the
        // network, rather it
        // matches the extents set in the task, which must exactly match those of the opportunity
        // grid.
        Coordinate destLatLon = gridLayout.getIntersectionLatLon(40, 40);
        // Here is a bit of awkwardness where WebMercatorGridPointSet and Grid both extend PointSet,
        // but don't share
        // their grid referencing code, so one would have to be converted to the other to get the
        // point index.
        int pointIndex =
                new WebMercatorGridPointSet(task.getWebMercatorExtents())
                        .getPointIndexContaining(destLatLon);

        int[] travelTimePercentiles = oneOriginResult.travelTimes.getTarget(pointIndex);

        // Transit takes 30 seconds per block. Mean wait time is 10 minutes. Any trip takes one
        // transfer.
        // 20+20 blocks at 30 seconds each = 20 minutes. Two waits at 0-20 minutes each, mean is 20
        // minutes.
        // Board slack is 2 minutes. With pure frequency routes total should be 24 to 64 minutes,
        // median 44.
        // However, these are not pure frequencies, but synchronized such that the transfer wait is
        // always 10 minutes.
        // So scheduled range is expected to be 2 minutes slack, 0-20 minutes wait, 10 minutes ride,
        // 10 minutes wait,
    ...
    /**
     * Similar to above, but using frequency routes which should increase uncertainty waiting for
     * second ride.
     */
    @Test
    public void testGridFrequency() throws Exception {
        ...
        // Frequency range is expected to be 2x 2 minutes slack, 2x 0-20 minutes wait, 2x 10 minutes
        // ride,
    ...
    /**
     * Similar to frequency case above, but with two different alternative paths. The availability
     * of multiple alternative paths should also reduce the variance of the distribution.
     */
    @Test
    public void testGridFrequencyAlternatives() throws Exception {
        ...
        // gridLayout.addVerticalFrequencyRoute(30, 20); // This route should have no effect on
        ...
        // FIXME convolving new Distribution(2, 10) with itself and delaying 20 minutes is not the
        // same
        ...
        Distribution observed =
                Distribution.fromTravelTimeResult(oneOriginResult.travelTimes, pointIndex);
    ...
    /**
     * Write travel times to GeoTiff. Convenience method to help visualize results in GIS while
     * developing tests.
     */
