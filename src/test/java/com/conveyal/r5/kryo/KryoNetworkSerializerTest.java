...
/**
 * Tests that serialization and deserialization of TransportNetworks functions as expected and does
 * not corrupt objects. Created by abyrd on 2018-11-05
 */
public class KryoNetworkSerializerTest {

    /**
     * We do not cross-check the result using the checksum, because the checksum is performed using
     * serialization.
     */
    @Test
    public void testRoundTrip() throws Exception {
        ...
        // Test that the network is identical to itself. This is a test that the ObjectDiffer works
        // and is configured
        ...
        // Re-load the saved network, and confirm that the re-loaded graph is identical to the built
        // one.
        // Reading the file also rebuilds transient indexes and primes the linkage cache with any
        // saved linkage.
    ...
    /**
     * Create an ObjectDiffer configured to work on R5 TransportNetworks. Make some exclusions for
     * classes that are inherently transient or contain unordered lists we can't yet compare. Apply
     * the ObjectDiffer to two TransportNetworks and assert that there are no differences between
     * them.
     */
    private static void assertNoDifferences(TransportNetwork a, TransportNetwork b) {
        ...
        // FIXME these should not be fields on the resultant objects, they are only used when
        // building the layer.
        ...
        // Skip the somewhat unnecessary spatial index field on PointSets - these contain unordered
        // lists
        ...
        // It and the map are keyed on street layers, which have identity equality so don't compare
        // properly here.
        objectDiffer.ignoreFields("linkageCache", "linkageMap");
        objectDiffer.useEquals(BitSet.class);
        // IntHashGrid contains unordered lists of elements in each bin. Lists are compared as
        // ordered.
        objectDiffer.ignoreClasses(IntHashGrid.class);
        // The class ch.qos.logback.classic.Logger holds some reflection data using Java proxy
        // objects
        objectDiffer.ignoreClasses(Logger.class);
        // If the root objects are identity-equal, enable special testing mode that will compare the
        // entire tree anyway.
        if (a == b) {
