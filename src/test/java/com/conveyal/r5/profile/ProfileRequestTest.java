...
/** Some date and time related tests for profile requests. Created by mabu on 2.12.2015. */
public class ProfileRequestTest {
    ...
    public void testEmptyGetFromTimeDate() {
        Instant expected =
                ZonedDateTime.now(profileRequest.zoneId).truncatedTo(ChronoUnit.DAYS).toInstant();
    ...
    public void testEmptyFromTime() {
        ...
        Instant expected =
                ZonedDateTime.of(2015, 10, 5, 0, 0, 0, 0, profileRequest.zoneId).toInstant();
        Instant got = Instant.ofEpochMilli(profileRequest.getFromTimeDate());
        assertEquals(expected, got);
    }
    ...
    public void testFull() {
        ...
        Instant expected =
                ZonedDateTime.of(2015, 1, 5, 6, 22, 0, 0, profileRequest.zoneId).toInstant();
        Instant got = Instant.ofEpochMilli(profileRequest.getFromTimeDate());
        assertEquals(expected, got);
    }
}
